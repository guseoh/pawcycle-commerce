...
- PR·Issue·review·CI의 동적 원격 상태가 필요하면 `github-mcp-readonly` Skill로 읽기 evidence path를 구성한다.
...
4. GitHub 동적 상태가 필요하면 `docs/runbook/github-mcp-agent.md`와 `github-mcp-readonly` Skill의 최소 읽기 Tool만 사용한다.
5. 변경 영향에 필요한 검증, AI 리뷰의 유효 지적과 남은 위험을 확인한다.
6. `docs/runbook/lean-harness.md` 조건에 따라 QA·보고서·인수인계 필요 여부를 판단한다.
7. 병합 권고·보류·반려 근거와 사용자 결정 항목을 보고하고 자동 병합하지 않는다.
...
- GitHub 읽기 작업에서 예상하지 않은 쓰기·병합·삭제·권한 확대가 필요하다.
...
- GitHub MCP 업무 흐름·Tool Allowlist·보안 경계는 `docs/runbook/github-mcp-agent.md`를 따른다.
