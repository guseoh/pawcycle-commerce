...
성공 결과 retention은 최초 `completed_at`부터 30일이다. cleanup cutoff는 주입한 `Clock`의 현재 instant에서 30일을 뺀 UTC 시각이며 `completed_at < cutoff`만 table별 양의 `batchSize` 범위에서 삭제한다. 정확히 cutoff인 row와 `completed_at IS NULL` row는 삭제하지 않고, 생성·관리 삭제 수를 별도로 반환한다. replay는 retention을 연장하지 않으며 cleanup으로 삭제된 key는 이후 새 요청으로 처리될 수 있다. Scheduler, 운영 batch size, retry/backoff와 alert는 이 설계에서 확정하지 않는다.

V4는 두 idempotency table에 `completed_at`과 cleanup index를 추가한다. 기존 row는 `response_status`가 2xx이고 `response_body`가 존재하는 성공 완료 결과만 migration 실행 시점의 단일 UTC 시각으로 backfill해 30일 grace period를 부여하며, reservation 등 완료 여부를 판정할 수 없는 row는 null로 보존한다.
