...
| QA-F006-017 | PS-003 로그인 복귀 | 상품 ID 0을 `returnTo`로 로그인 | same-origin `/products` fallback | 실제 브라우저 URL | 실행 |
| QA-F006-018 | PS-003 로그인 복귀 | 구독 ID 0을 `returnTo`로 로그인 | same-origin `/products` fallback | 실제 브라우저 URL | 실행 |
...
정상 화면 keyboard-only 흐름과 skip link Enter 이동은 사용자 수동 관찰로 보완했지만 자동 회귀 테스트를 대체하지 않는다. GET 장애 화면의 재시도 keyboard 접근성과 session 만료는 승인된 안전 재현 수단이 없어 실제 브라우저 미재현 위험으로 남는다.
