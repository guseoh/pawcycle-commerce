...
Subscription detail의 `nextDelivery`에 `addOns[]`, `addOnTotalKrw`, `orderTotalKrw`를 추가한다. Add-on 항목은 `skuId`, `productId`, `productName`, `skuName`, `quantity`, `unitPriceKrw`, `lineAmountKrw`를 제공하며 current/pending Snapshot에는 넣지 않는다. ACTIVE·`SCHEDULED` next Schedule은 기존 `availableActions`에 `SET_NEXT_DELIVERY_ADDON`을 항상 추가하고, 실제 Add-on이 있으면 `REMOVE_NEXT_DELIVERY_ADDON`도 추가한다. `ORDER_STOCK_UNAVAILABLE` HELD에서는 실제 Add-on이 있을 때 `availableActions`에 `REMOVE_NEXT_DELIVERY_ADDON`과 `CANCEL`만 노출한다. RESCHEDULE·PAUSE·RESUME·CHANGE_DELIVERY_CYCLE은 같은 Add-on을 보존하고, SKIP은 새 next Schedule로 이동하며, CANCEL은 미소비 Add-on을 삭제한다.
