...
가격, 할인, 총 결제 금액, 쿠폰, 재고, 품절, 배송 예정일, 구독 상태는 첫 MVP API 응답에 포함하지 않는다.
...
| 구독 생성 | POST | `/api/subscriptions` | 필요 | `201 Created` | `AUTH_REQUIRED`, `SKU_NOT_FOUND`, `SKU_NOT_SUBSCRIBABLE`, `VALIDATION_FAILED` |
...
| REQ-SUB-004 | AC-SUB-004-01 외 | `nextOrderDate`, `createdDate`, `YYYY-MM-DD` 후보 | `created_date`, `next_order_date` | BE, FE | 날짜 계산·표시 기준 | Proposed |
...
- API 날짜 `YYYY-MM-DD` 후보를 화면에서 `YYYY. M. D.`로 표시한다.
