...
사용자가 2026-07-13에 D1~D8 추천안을 수정 없이 명시적으로 승인했다. 승인 근거는 PR 병합이 아니라 위 사용자 입력이며, 이 문서는 API-003 구독 범위의 Backend 구현 입력으로 사용할 수 있다. `API-001`과 `DATA-002` 원본 문서 전체는 계속 `Proposed` 상태다.

## 사용자 승인 결과 요약

| ID | 결정 | 추천안 요약 | 상태 |
| --- | --- | --- | --- |
| D1 | API와 성공 상태 | `POST /api/subscriptions` 201, 목록·상세 GET 200 | `Approved` |
| D2 | 생성 요청·응답 | 필수 `skuId`, `quantity`, `deliveryCycleWeeks`; 응답 `subscriptionId`, `nextOrderDate` | `Approved` |
| D3 | 목록·상세 응답 | 중첩 Product/SKU 요약, 목록 `subscriptionId DESC`, 상세에 가격·생성일 추가 | `Approved` |
| D4 | 인증·소유권 | session principal의 `memberId` 사용, 미존재·타인 소유·비숫자 ID 동일 404 | `Approved` |
| D5 | 입력·도메인·안전 오류 | 입력 오류는 단일 `VALIDATION_FAILED`, 도메인·endpoint별 안전 오류 분리 | `Approved` |
| D6 | 동일 조건·중복 요청 | 동일 조건 복수 구독 허용, 요청 1회당 1건, MVP 멱등성 저장소 없음 | `Approved` |
| D7 | 구독 물리 데이터 | 새 Flyway migration, 최소 필드·FK·CHECK·`(member_id, id)` 인덱스 | `Approved` |
| D8 | Backend 구현·검증 | API 3개와 migration을 한 PR에서 구현·테스트, FE 완료 후 통합 QA | `Approved` |

승인에 사용된 정확한 사용자 입력은 다음과 같다.

```text
API-003 D1~D8 전체 추천안 승인
```

선택되지 않은 각 D의 실질적인 대안은 승인되지 않았으며 구현 입력이 아니다.
...
추천안을 수정 없이 승인했다. 성공 상태·URI 대안은 선택하지 않았다.
...
추천안을 수정 없이 승인했다. 생성 성공 응답 확대 대안은 선택하지 않았다.
...
추천안을 수정 없이 승인했다. 응답 필드·정렬 대안은 선택하지 않았다.
...
- QA: 미존재·타인 소유·비숫자 ID의 status·body와 유의미한 timing 차이에서 구독 존재·소유자 정보를 추론할 수 없는지 검증한다. 요청 URL에는 계약상 `subscriptionId`가 포함되므로 URL 자체를 비노출 판정 기준으로 사용하지 않는다.
- 로그: application log에는 원문 `memberId`·`subscriptionId`를 남기지 않고 식별자를 마스킹한다. request path를 기록하는 access log의 path parameter 마스킹은 배포 환경 통합 QA에서 별도로 검증한다.
...
추천안을 수정 없이 승인했다. 소유권 실패를 구분하는 대안은 선택하지 않았다.
...
추천안을 수정 없이 승인했다. API-001의 분리 validation code 대안은 선택하지 않았다.
...
timeout 후 사용자가 재시도하거나 client·proxy가 요청을 반복하면 중복 구독이 생성될 수 있다. 사용자는 이 MVP 위험을 명시적으로 승인했다.

### 승인 결과

추천안을 수정 없이 승인했다. 동일 조건 unique와 Idempotency-Key 대안은 선택하지 않았다.
...
- 상태, soft delete, 가격 snapshot, 감사 시각(`created_at`, `updated_at`), 결제·배송·주문 FK, 동일 조건 unique와 측정 근거 없는 추가 인덱스는 추가하지 않는다.
...
대용량 회원별 목록에서 인덱스 효과는 실제 데이터로 다시 측정해야 한다. 감사 시각이 후속 요구사항으로 필요해지면 type·nullability·기본값·갱신 책임을 새 결정으로 확정해야 한다.

### 승인 결과

추천안을 수정 없이 승인했다. 추가 필드·제약·인덱스 대안은 선택하지 않았다.
...
- Discord 상세 알림 개선 SRE 작업은 API-003 마무리 후 별도 역할 범위에서 병렬 착수할 수 있다.
...
추천안을 수정 없이 승인했다. API별 작업·QA 분할 대안은 선택하지 않았다.
...
- D2·D3 JSON은 승인된 PS-002·DOMAIN-001·UX-001·PS-003 표시 요구를 넘지 않는다. `UX-001`은 D2·D3의 승인 UX 기준선이며 단순 참고 Proposed 문서가 아니다.
...
- D8은 승인된 D1~D7을 하나의 Backend 구현 입력으로 전달하며 Backend Engineer가 통합 구현을 시작할 수 있다.

## 승인 후 실행 순서

1. 구독 Backend 3개 API, migration과 테스트를 하나의 수직 작업으로 구현한다.
2. Discord 상세 알림 개선 SRE 작업을 별도 역할에서 병렬 착수한다.
...
D1~D8은 모두 `Approved`다. 2026-07-13 사용자 입력 `API-003 D1~D8 전체 추천안 승인`을 근거로 API-003 구독 범위의 Backend 구현을 시작할 수 있다. PR 병합은 승인 근거가 아니며 API-001·DATA-002 원본 전체는 계속 `Proposed`다.
