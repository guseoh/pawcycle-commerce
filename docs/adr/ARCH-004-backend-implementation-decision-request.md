...
첫 Backend 구현에서는 JPA, MySQL JDBC driver, Flyway, Spring Security, `spring-security-test`를 도입 후보로 요청하고, Testcontainers와 OpenAPI 검증 도구는 다음 단계 후보로 분리한다.
...
`ARCH-004-DEC-002`: 첫 Backend 구현에서 JPA, MySQL JDBC driver, Flyway, Spring Security, `spring-security-test` 도입을 허용하고 Testcontainers와 OpenAPI 도구는 별도 결정으로 분리해도 되는지 승인해 주세요.
...
A를 선택하고 첫 파일은 `V1__create_first_mvp_tables.sql` 같은 단순한 이름으로 시작하도록 요청한다.
...
`ARCH-004-DEC-004`: Flyway를 도입하고 첫 Backend 구현에서 최초 schema migration을 작성해도 되는지 승인해 주세요.
...
AUTH-001은 세션 기반인지 token 기반인지 결정하지 않았다. 로그인 화면과 인증 저장 방식도 아직 없다.
...
`ARCH-004-DEC-007`: 첫 MVP 인증 방식은 session 기반으로 시작해도 되는지 승인해 주세요.
...
Session 기반을 선택한다면 CSRF를 사용하고, cookie는 `HttpOnly`, 운영 환경 `Secure`, `SameSite=Lax` 후보로 둔다. principal은 최소 `memberId`와 표시·로그인 식별에 필요한 값만 포함한다.
...
`ARCH-004-DEC-008`: session cookie, CSRF 사용, 최소 `memberId` principal 구조를 첫 인증 구현 기준으로 삼아도 되는지 승인해 주세요.
...
A를 선택하고 OpenAPI 도구는 첫 구현 뒤 계약 검증 필요가 커질 때 별도로 검토한다.
...
`ARCH-004-DEC-009`: `API-001`의 5개 API 후보를 첫 Backend Controller 계약 기준으로 사용해도 되는지 승인해 주세요.
...
Repository Validation은 Backend `test/build`와 Frontend `npm ci/lint/build`를 실행한다. MySQL, Flyway migration 적용, Security test, API contract 검증은 아직 없다.

선택지:

| 선택지 | 설명 | 장점 | 단점 | 후속 영향 |
| --- | --- | --- | --- | --- |
| A. 현재 CI 유지 + 새 단위/Controller/Security test 포함 | Gradle test/build 안에서 신규 테스트를 실행한다. | workflow 변경 없이 시작할 수 있다. | 실제 MySQL migration 검증은 부족하다. | `.github/**` 변경이 없다. |
...
첫 Backend 구현 PR에서는 A를 우선 요청하고, MySQL service container, Testcontainers, OpenAPI 검증은 별도 SRE 또는 후속 기술 결정으로 분리한다.
...
`ARCH-004-DEC-012`: 첫 Backend 구현에서는 기존 Repository Validation 안의 Gradle test/build를 활용하고, MySQL/Testcontainers/OpenAPI CI 확장은 별도 작업으로 분리해도 되는지 승인해 주세요.
...
| CSRF/cookie/principal | CSRF 사용, HttpOnly/SameSite cookie 후보, 최소 `memberId` principal |
| API 계약 | `API-001` 5개 API 후보 사용 |
| 오류 JSON | `code`, `message`, `fieldErrors` 공통 구조 |
| 아키텍처 | feature package + domain/application/infra/api 분리 |
| CI | 첫 구현 PR에서는 기존 Gradle/npm 검증 유지, DB/OpenAPI 확장은 별도 작업 |
...
- session 기반 인증은 CSRF와 cookie 속성을 함께 다루지 않으면 보호 API 보안 기준이 약해진다.
