...
| 8 | CSRF 활성화, cookie·principal·BCrypt 기준 | Approved, 전달 계약 세부값 Decision Required |
...
- 승인 내용: CSRF 보호를 활성화하고 상태 변경 요청에 CSRF token을 요구한다. 공개 GET 상품 API에는 token을 요구하지 않는다. session cookie는 HttpOnly를 사용하고 SameSite 기본 후보는 Lax이며 운영 HTTPS에서는 Secure를 사용한다. principal은 최소 `memberId`만 제공하고 JPA Entity 전체를 담지 않는다. 비밀번호는 Spring Security `PasswordEncoder`와 `BCryptPasswordEncoder`로 검증한다.
- 승인 근거: 사용자 승인 입력 8, 13, 16과 ARCH-005 결정 항목 8·보완 결정 항목 4.
- 적용 범위: 브라우저 session 인증, 로그인·로그아웃 상태 변경, 현재 회원 식별, 개발·운영 cookie 차이 문서화.
- 제외 범위: CSRF 전체 비활성화, session id 응답 body 노출, 평문 비밀번호 저장·비교·로그 출력, Entity principal, 클라이언트 memberId 신뢰.
- 구현 시 검증 조건: CSRF token 누락·오류 요청은 거부되고 올바른 token 요청은 처리돼야 한다. cookie 속성과 환경별 차이를 확인하고, principal에서 `memberId`를 얻는 Security 테스트를 작성한다. 민감정보가 로그에 남지 않는지 확인한다.
- 관련 요구사항·설계 문서: `AUTH-001`, `ARCH-001`, `AGENTS.md` Secret 관리, Spring Security 구현 작업.
- 위험: CSRF token 획득 방식과 정확한 header/cookie 이름이 아직 계약으로 정해지지 않았다. SameSite=Lax도 사용자 입력에서 기본 후보로 표현됐으므로 실제 배포 흐름과 함께 확정해야 한다.
...
CSRF 활성화와 header 전달 방향은 Approved다. 다음 값은 아직 정해지지 않았다.

- 최초 token 획득 endpoint 또는 노출 방식
- CSRF token 이름과 request header 이름
- cookie를 사용할 경우 cookie 이름과 JavaScript 접근 가능성
- token 갱신·만료 시 클라이언트 처리
- 로그인·로그아웃 요청에서 token을 얻는 순서

상태 변경 API 구현 전에 Backend와 Frontend가 같은 계약을 사용하도록 보완해야 한다.
...
- SameSite=Lax 후보의 최종 설정과 개발·운영 차이
