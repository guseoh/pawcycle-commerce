...
SSM prefix는 실행 입력으로만 전달한다. script는 네 parameter를 각각 `--with-decryption`으로 조회하고, 하나라도 누락·빈 값·조회 실패면 현재 runtime symlink를 바꾸지 않고 실패한다. 성공 시 `mysql.env`, `backend.env`, `.complete`를 mode `600`으로 만든 뒤 `current` symlink를 원자적으로 교체한다. 값은 stdout에 출력하지 않는다.
...
materialize가 만든 과거 `.bundle.*`에는 Secret이 남는다. 새 `current` bundle과 배포 성공을 확인한 뒤에만 root 사용자가 현재 symlink의 실제 대상이 아닌 과거 bundle을 정확히 식별해 별도 승인된 보안 정리 절차로 제거한다. 경로가 불명확하면 삭제하지 않는다.
