...
- 실제 운영 검증: 대상 release `b9cf3cf51c5ffd4b85c6eafc78706ed079e299d6`의 활성화와 재부팅 복구를 사용자/Tech Lead가 확인함. 상세 결과와 미확정 항목은 `docs/reports/OPS-010/sre-report.md`에 기록
...
2. 현재 정상 SHA가 있으면 `git diff --quiet <current-sha> <target-sha> -- infra/production`에 해당하는 계약 검사를 먼저 수행한다. commit 부재·Git 오류·차이 발견 시 image pull과 container·release state 변경 전에 중단한다.
3. Compose config를 검증하고 digest로 고정된 MySQL·Nginx를 pull한다.
4. 현재 정상 SHA가 있으면 그 release의 두 application image도 다시 pull·검증해 복귀 가능성을 먼저 확보한다.
5. 대상 Backend·Frontend의 정확한 SHA tag를 pull하고 revision label과 GHCR `sha256` RepoDigest를 확인한다.
6. 네 image digest 후보를 `/opt/pawcycle/state/<sha>.images`와 비교한다. 기존 기록과 한 글자라도 다르면 기록을 덮어쓰지 않고 중단하며, 최초 SHA만 mode `600` 파일로 원자적으로 기록한다.

계약 차이·digest drift·preflight 실패 시 `docker compose up`을 호출하지 않으므로 기존 정상 release state, container와 named volume은 변하지 않는다.
...
rollback은 현재 SHA와 대상 SHA의 `infra/production/**`가 같음을 먼저 검증하고 두 SHA를 preflight한 뒤 application image만 이전 SHA로 바꾼다. 계약이 다르면 설정 복원 없이 application image만 바꾸는 것이 안전하지 않으므로 container·state·volume 변경 전에 중단한다. 활성화 실패 시에는 계약 검사를 이미 통과한 직전 현재 SHA만 다시 복구한다. MySQL container가 Compose 판단에 따라 재사용 또는 재생성될 수 있어도 동일 named volume을 사용한다.
...
| `infra/production/**` 계약 차이 | application-only rollback 안전성 없음 | 현재·대상 SHA Git diff 확인 | container·state·volume 변경 전 중단, 별도 release bundle 설계 요청 |
...
대상 release `b9cf3cf51c5ffd4b85c6eafc78706ed079e299d6`에서 동일 `current-sha`, 네 container health, proxy만 host HTTP `80` 공개, 내부·외부 두 HTTP smoke, 재부팅 뒤 Docker 자동 시작·동일 MySQL volume·동일 SHA·health·smoke 복구를 사용자/Tech Lead가 확인했다. 실제 이전 SHA rollback, EC2 자원 여유, SSM·IAM 세부 설정과 backup·restore는 별도 증거가 없으므로 통과로 기록하지 않는다.
