...
Production host에서 먼저 현재 배포 상태와 observability를 확인한다. `diagnose-backend-state.sh --scope production` 결과와 그 결과를 입력으로 한 `--scope observability` 결과가 모두 `NORMAL`이어야 한다. `NORMAL`이 아니거나 release가 진행 중이거나 상태가 불확실하면 실행하지 않는다. 이 확인에는 credential, cookie, session, response body, product ID, raw DB data를 local artifact나 repository에 기록하지 않는다.
...
scenario는 redirect를 따르지 않으며 HTTP 200 이외 응답은 expected-status error로 기록한다. expected-status error 또는 dropped iteration이 한 건이라도 발생하면 k6 threshold가 실패하고 shell의 `set -e`가 다음 RPS 단계를 막는다. health 이상, EC2/RDS 지표 악화 또는 load generator 이상도 즉시 k6 process를 종료하고 다음 단계를 실행하지 않는다. 입력 URL에는 path, query, fragment, credential 또는 HTTP scheme을 넣을 수 없다.

각 성공 단계의 stdout aggregate는 `targetRps`, `actualRps`, `droppedIterations`, `p50Ms`, `p95Ms`, `p99Ms`, `maxMs`, `expectedStatusErrorRate`, `allocatedVUs`, `activeVUs`만 포함한다. response body·product ID·cookie·session·credential·raw DB data는 출력하거나 저장하지 않는다. 결과는 외부 desktop/network/load-generator 한계를 포함한 단일 배포 경로의 관측값이며 Application 또는 RDS의 독립 최대 capacity로 해석하지 않는다.
...
마지막 실행 뒤에는 실행 전과 같은 production과 observability diagnostic을 다시 수행해 둘 다 `NORMAL`인지 확인한다. 하나라도 `NORMAL`이 아니면 후속 단계나 재실행을 하지 않고 상태 안정화와 사용자 판단을 기다린다. 이 harness는 GET만 사용하고 Production nginx/Compose/Application/RDS 설정을 변경하지 않는다. 저장소 준비 변경의 복구는 이 PR을 revert하는 방식으로 한다.
