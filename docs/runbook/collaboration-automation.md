...
- 위험 등급에 필요한 작업 보고서와 실제 소비자가 있는 역할 인수인계가 PR 산출물로 남도록 확인한다.
...
작업 등급, 승격, 산출물·QA·검증과 delta-only 명세의 상세 권위 원본은 `docs/runbook/lean-harness.md`다.

필수 검증을 통과하고 작업 등급에 필요한 산출물이 준비되면 Codex는 commit과 push를 수행한다.
...
- 등급 없는 기존 산출물에 하위 호환 일반 규칙과 경고 적용
- 변경 diff의 공백 오류
