...
Discord 알림은 단순 상태 텍스트가 아니라 한 메시지 안의 1~3개 Rich Embed로 작업 맥락, 처리 과정, 검증·리뷰 상태와 다음 작업을 전달한다.
...
`pull_request_target`과 `workflow_run`은 기본 브랜치의 신뢰된 알림 스크립트를 checkout한다. `workflow_dispatch` Preview만 선택한 ref의 스크립트를 사용한다. GitHub API 조회는 read-only 권한으로 제한하고, 조회 실패 값은 `확인 불가`로 표시한다.

## 9. Discord Embed 디자인

첫 번째 요약 Embed의 공통 필드는 다음이다.
...
- 처리·검증·리뷰: 주요 변경, 검증과 QA, job별 결과, 실패 job·step, 리뷰 상태, 미해결 스레드
...
Discord에는 전체 diff, 긴 로그, Stack Trace, Secret 값, Webhook URL, password, token과 개인정보를 넣지 않는다. 외부 입력의 control character를 제거하고 `@everyone`·`@here`를 비활성화하며 필드와 전체 payload 길이를 제한한다.
...
Secret 설정 후 실제 연결 테스트가 필요하면 GitHub Actions에서 `Collaboration Notification` 워크플로를 수동 실행하고 `scenario`를 선택한다.
...
`connection_test`는 실제 제품 이벤트가 아닌 `[PREVIEW]` payload를 만든다. 특정 PR 보고서 미리보기는 `scenario=pr_preview`와 양의 정수 `pr_number`를 입력한다. 성공 로그에는 HTTP 2xx 상태가 표시되어야 하며 Discord 채널에서 하나의 메시지가 실제로 수신되어야 한다.
