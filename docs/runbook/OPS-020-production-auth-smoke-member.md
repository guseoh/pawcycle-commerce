...
6. 실행 중인 Backend에 연결된 non-internal `database-egress` network가 정확히 하나이고 Backend만 연결됐는지 확인한다. 외부 Production MySQL 접속 정보는 보호된 `backend.env`의 Spring datasource 환경 변수로만 주입된다.
...
Wrapper는 root·TTY 검사를 Docker 접근 전에 수행하고 배포·rollback과 같은 `deploy.lock`을 획득한다. 그 뒤 current release, runtime/state mode, OPS-019 source, SHA tag·OCI revision·registry digest, 실행 중인 Backend의 image identity·health, non-root image user, Backend의 유일한 non-internal `database-egress` network와 단독 연결을 읽기 전용으로 검증한다. 모든 preflight가 성공한 뒤에만 TTY prompt가 나타난다.

## Container 안전 계약

One-shot Container는 digest reference, `--rm --interactive`, restart 없음, port publish 없음, volume mount 없음, `--log-driver none`, Backend에 연결된 유일한 non-internal `database-egress` network 하나만 사용한다. Compose용 runtime env는 새 파일로 변환하지 않고 보호된 `backend.env`에서 검증된 datasource 항목만 process-substitution pipe로 Docker에 전달한다. 회원 email·password는 이 env 경로에 포함하지 않는다. Root filesystem은 read-only이고 `/tmp` tmpfs, user `pawcycle`, no-new-privileges, all capability drop, memory·CPU·PID limit을 적용한다. 현재 web Backend·Frontend·Proxy service를 중지·재시작·변경하지 않는다.
...
- Preflight 오류: 어떤 credential도 입력하지 말고 deploy lock, state·runtime·실행 중인 Backend image·database-egress 및 datasource 주입 계약을 비민감 정보로 재확인한다.
...
Immutable image 확인 불가, current SHA 불일치, runtime/state mode 불일치, Backend의 database-egress topology 불명확 또는 외부 datasource 계약 불일치, duplicate 또는 성공 여부 불명확이면 실행을 중단하고 사용자/Tech Lead에게 보고한다. 보고에는 단계와 종료 코드만 포함하고 식별자·credential·원시 오류는 포함하지 않는다.
