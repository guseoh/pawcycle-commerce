...
6. External managed MySQL endpoint의 reachability와 health를 별도 read-only preflight로 확인한다.
7. 실행 중인 Backend container가 `pawcycle-production-database-egress` network에 attached인지 확인한다.
8. `pawcycle-ops020-auth-smoke-member` Container가 남아 있지 않은지 확인한다.
9. 운영 email과 강한 password는 사용자가 그 자리에서 정하며 다른 화면·문서·clipboard에 복제하지 않는다.
