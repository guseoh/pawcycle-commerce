...
4. 실제 cutover 승인에는 ARCH-013에서 승인한 `sslMode=REQUIRED` 계약과 deferred 상태인 VERIFY_CA/VERIFY_IDENTITY hardening의 위험 수용을 명시적으로 포함한다. 현재 위험 수용 또는 별도 hardening 완료가 확인되지 않으면 cutover는 `SKIPPED`로 중단한다.
5. 승인된 별도 실행에서만 write quiesce/activation을 수행하고, 그 직후 Backend health, API smoke, external HTTPS를 독립 확인한다. 실패 시 다음 단계로 진행하지 않는다.
