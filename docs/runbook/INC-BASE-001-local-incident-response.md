...
2. Prometheus Targets 화면에서 Backend target의 `health`·`lastError`를 확인하고, Alerts 화면에서 `PawCycleBackendScrapeUnavailable`·`PawCycleReconciliationFailure` 상태를 확인한다.
3. Alertmanager receiver와 notification failure counter를 확인한 뒤 Discord 채널에서 해당 alert의 firing·resolved 전달을 확인한다. alert는 원인 판정이 아니라 첫 확인 신호이므로 다음 순서로 원인을 구분한다.
4. Backend container 내부 `/actuator/health`를 10초 이내로 확인한다.
5. Backend scrape가 유지되면 Hikari와 reconciliation failure metric, 관련 Backend log를 확인한다.
