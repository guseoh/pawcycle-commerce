...
10. workflow 범위만으로 해결할 수 없는 Backend/Frontend 코드 또는 의존성 문제가 보이면 해당 역할 작업으로 넘기고 SRE 작업에서는 중단한다.
...
1. 새 Backend 구현이 아직 MySQL 환경을 소비하지 않는지 확인한다.
2. `.github/workflows/validate-conventions.yml`의 `services.mysql`, `Verify MySQL service`와 Backend step의 세 datasource 환경 변수를 함께 제거한다.
3. 이 런북의 CI MySQL service·환경 변수·진단 절차를 이전 기준선으로 되돌린다.
4. Java 25 Backend test/build와 Node.js 24 Frontend 검증이 다시 통과하는지 확인한다.
