...
smoke 실행 전 현재 shell의 `PAWCYCLE_LOCAL_QA_BOOTSTRAP_EMAIL`과 `PAWCYCLE_LOCAL_QA_BOOTSTRAP_PASSWORD`를 `.env.local`과 동일한 local-only 값으로 안전하게 설정하며 값을 명령 이력에 직접 입력하거나 출력하지 않는다. 테스트 트래픽은 `./smoke.ps1 -BaseUri $ProxyUrl` 또는 `$ProxyUrl` 기준 공개 상품 API 호출을 사용한다. 트래픽 전후 Prometheus query 결과를 비교해 HTTP request count, JVM heap/GC/thread, process/system CPU와 Hikari active/idle/pending을 확인한다. reconciliation은 기존 조건부 Scheduler 실행 결과만 관측하며 cadence를 변경하거나 새 trigger를 추가하지 않는다. idempotency cleanup은 승인된 runtime trigger와 운영 batch size가 없으므로 Phase A integration test 증거를 사용한다.
