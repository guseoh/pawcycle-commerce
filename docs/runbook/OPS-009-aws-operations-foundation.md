...
| Instance | `t3.small` | 선택 화면과 생성 후 유형 확인 |
...
- [ ] 같은 목적의 Budget, role, Security Group, EC2, EBS, Elastic IP가 이미 존재하지 않음을 확인했다.
...
- [ ] SSM용 아웃바운드 HTTPS와 OS package·공개 GHCR용 인터넷 경로가 있다.
...
2. 아래 월간 Cost budget 두 개를 만든다. Budget action이나 자동 리소스 중지는 구성하지 않는다.
...
3. 각 알림을 운영자가 확인하는 이메일에 연결하고 AWS의 구독 확인 절차가 표시되면 완료한다.
4. Budget 상세 화면에서 기간이 Monthly이고 금액, actual/forecast 알림, Credits·Refunds 포함 여부가 표와 일치하는지 다시 확인한다.
5. 증거에는 `Gross/Net 설정 일치`, `알림 구독 확인`, `크레딧 만료 검토` 여부만 남긴다. 이메일 주소, 잔액, 계정 ID는 저장소에 남기지 않는다.
...
1. IAM에서 EC2 use case의 role을 만든다. 이 role은 EC2 서비스가 assume하며 사람의 로그인이나 장기 access key에 사용하지 않는다.
2. 현재 단계에서는 AWS 관리형 정책 `AmazonSSMManagedInstanceCore` 하나만 연결한다.
3. role에 인라인 `s3:*`, `logs:*`, `cloudwatch:*`, `iam:*` 권한을 추가하지 않는다.
4. 생성 후 Trust relationship의 서비스가 EC2이고 연결 정책이 하나뿐인지 확인한다.
...
성공 기준은 EC2 신뢰 관계, `AmazonSSMManagedInstanceCore` 단일 연결, access key 부재다. 광범위 권한이 섞였으면 instance에 연결하지 말고 role을 삭제한 뒤 원인을 기록한다.

### 3. Security Group 생성

Security Group을 `ap-northeast-2`의 승인된 기존 VPC에 만든다.
...
10. 계정 식별자나 Secret이 없는 `Project=pawcycle-commerce`, `Environment=prod`, `Task=OPS-009` 같은 관리 tag만 사용한다.
11. 비상 SSH용 key pair를 선택하거나 새로 만들되 private key는 사용자가 저장소 밖의 안전한 위치에 보관한다. TCP 22 inbound는 만들지 않는다.

생성 직후 다음을 확인한다.

- instance type, AMI 이름·게시자, Region, VPC/subnet
...
2. 방금 만든 instance에 연결한다.
3. instance networking 화면에서 public IPv4가 연결한 Elastic IP임을 확인한다.
4. 실제 IP는 저장소, PR, 이슈, 스크린샷 파일명에 기록하지 않는다.
...
| EC2·EBS | 승인값, 암호화, DeleteOnTermination false | 설치 금지, 역순 정리 |
...
실패했거나 기반을 사용하지 않기로 결정하면 의존성의 역순으로 정리한다. 실행 전 각 리소스가 OPS-009에서 만든 대상인지 Console tag와 연결 관계로 확인한다.
...
5. `Delete on termination=No`인 root EBS가 Available로 남았는지 확인한다.
6. 복구 필요성이 없다는 사용자 판단 뒤에만 해당 EBS를 명시적으로 삭제한다. Snapshot 생성은 비용과 데이터 보존 결정을 요구하므로 별도 승인 없이 만들지 않는다.
7. Elastic IP를 Disassociate한 뒤 더 이상 필요 없고 DNS 의존성이 없음을 확인하고 Release한다. Release 이후 동일 주소 복구는 보장되지 않는다.
8. 연결 대상이 없는 Security Group을 삭제한다.
9. instance profile/role에서 정책을 분리하고 OPS-009 전용 role만 삭제한다.
10. Budget은 계정을 계속 운영하면 비용 감시를 위해 유지한다. 계정 사용을 완전히 중단하기로 사용자가 결정한 경우에만 두 Budget을 삭제한다.
11. Billing Console에서 남은 EBS, public IPv4와 기타 과금 리소스가 없는지 다시 확인한다.
...
- EC2 role trust와 연결 정책 목록
- Security Group inbound 표와 SSH 제거 확인
- EC2 type·OS·Region, EBS 유형·크기·암호화·보존 설정
