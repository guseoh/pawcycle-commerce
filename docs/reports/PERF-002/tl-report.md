...
- 기준 브랜치: `main`
- 작업 브랜치: `ops/tl`
- 결정 상태: D1-A~D6-A `Approved`

## 작업 목적

PERF-001의 로컬 성능 기준선 추천안 D1-A~D6-A에 대한 사용자 승인을 단일 원본으로 기록하고, 다음 Platform/SRE 측정 작업이 추가 결정을 만들지 않고 동일 조건을 재현할 수 있게 한다.
...
| PERF-001 상태 일치 | 전체 `Resolved by PERF-002`, D1~D6 `Approved by PERF-002`로 갱신 |
| 선택되지 않은 대안 미승인 | `Deferred` 또는 미승인으로 명시 |
| 재현 가능한 다음 측정 | cohort·횟수·setup·reset·통계·실패·보존 조건 고정 |
...
- 다음 Platform/SRE가 사용할 변경 불가 조건과 중단 조건을 인수인계했다.
...
- D1-A~D6-A: `Approved`
- PERF-001 D1~D6: `Approved by PERF-002`
...
현재 PowerShell·Docker 도구로 일회성 로컬 측정을 허용한다. 운영 배포, CI, dashboard, alert와 관측성 stack은 변경하지 않는다.
...
- `HEAD == origin/main == d1162b5`: 확인
- clean `ops/tl`, 열린 `ops/tl` PR 없음: 확인
...
- `git diff --check`: 통과
- PR 본문 UTF-8 validator: PR 생성 전 실행
...
PR 생성 후 CodeRabbit/Codex Review와 CI를 확인하고 유효한 지적만 반영한다.
...
다음 Platform/SRE는 PERF-002 승인 원본과 인수인계를 입력으로 사용해 저장소 변경 없는 일회성 기준선을 측정한다.
...
Platform/SRE가 승인 조건을 변경하지 않고 일회성 로컬 기준선을 측정하고 집계 결과·재현 명령·제한을 보고한다.
...
- PR 생성 전
- 예정 제목: `docs(tl): PERF-002 측정 조건 승인`
- 대상 브랜치: `main`
- 자동 병합하지 않는다.
