...
- 기준 브랜치: `main`
- 작업 브랜치: `feat/be`
- 상태: 구현과 로컬 정적 검토 완료, Java 25·MySQL 검증은 Repository Validation 결과 확인 필요
...
- Java 25/MySQL Backend test와 build: PR의 Repository Validation에서 확인 예정
...
구체적인 로컬 실행 환경과 Runbook은 Platform/SRE가 같은 작업 ID의 인수인계를 입력으로 완성한다. Backend가 제공하는 설정 계약은 다음과 같다.
...
- 로컬 Java 25와 MySQL 검증은 현재 PC 환경 제약으로 미실행이다.
...
- commit·push 결과는 작업 완료 후 갱신한다.

## PR 상태

- `main` 대상 Backend PR 생성 예정
- 자동 병합하지 않는다.
