...
이번 변경은 MySQL healthcheck에 한정되므로 해당 Compose 설정·MySQL 재생성·연결 경로만 검증하고, image와 제품 코드·bootstrap 동작에 관한 다음 검증은 반복하지 않았다.
...
healthcheck 변경과 직접 관련 없는 검증을 다시 실행하면 근거가 늘지 않고 로컬 상태와 검증 비용만 불필요하게 바뀌므로 기존 성공 결과를 사용했다.
