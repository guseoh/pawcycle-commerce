...
- 다음 결과는 PR #45의 검증 당시 이력이며 현재 head와 최신 CI 상태가 아니다: head `4d8eb26`, Repository Validation run `29336962489` 성공
  - Commit and PR conventions 통과
  - Application validation 통과
- forwarded header 리뷰 수정 후 Compose 설정과 Nginx `nginx -t` 통과
- 실행 중 컨테이너 환경을 값 비출력·비저장 방식으로 현재 검증 프로세스에만 전달해 proxy만 재생성
- proxy 재생성 후 healthy, 외부 단일 origin의 Frontend와 `/api/products`, session·CSRF 연결 통과
- MySQL·Backend·Frontend 컨테이너와 MySQL named volume identity 유지, Backend reset `false` 유지
- `mysqladmin ping`은 인증 실패에도 서버 응답만으로 성공할 수 있어, 실제 QA DB 사용자로 TCP 접속해 대상 database에서 `SELECT 1`을 실행하는 healthcheck로 교체
- 올바른 credential의 `SELECT 1`은 exit code 0, 의도적으로 잘못된 password와 존재하지 않는 database는 각각 non-zero 확인
- 실행 중 컨테이너 환경을 실제 값 비출력·비저장 방식으로 현재 PowerShell 프로세스에만 전달해 Compose 설정을 검사하고 MySQL service만 재생성
- 수정 healthcheck로 MySQL healthy 확인, MySQL named volume과 schema table 수, Backend·Frontend·proxy container identity, Backend 단일 인스턴스와 reset `false`, `board-mysql-dev` 상태 보존 확인
- MySQL 재생성 후 동일 origin의 `/products`, `/api/products`, CSRF endpoint 연결 통과
- 재시작과 reset 직후 Compose 명령이 서비스 준비 전에 종료돼 smoke 또는 QA 검증이 먼저 시작될 수 있는 readiness race를 확인하고, Runbook과 SRE→QA 인수인계의 관련 `up` 명령에 `--wait --wait-timeout 120` 추가
- Docker Compose v2.40.0에서 `--wait`와 `--wait-timeout` 지원 확인
- 실제 `down` 뒤 `up --detach --wait --wait-timeout 120` 성공, `mysql`·`backend`·`frontend`·`proxy`가 모두 healthy 상태에서 명령 종료 확인
- reset을 `false`로 유지한 Backend·proxy `--force-recreate`에도 같은 health 대기를 적용해 성공 확인
- 재시작과 재생성 전후 MySQL named volume identity와 공개 상품 API 응답 hash, reset `false`, Backend 단일 인스턴스, 다른 프로젝트 container 상태 보존 확인
- 재생성 후 동일 origin의 `/products`, `/api/products`, `/api/auth/csrf` 연결 통과
- `Preserved`의 상품·SKU fixture 정확히 한 개 검증은 `smoke.ps1`의 시나리오 분기 전 공통 assertion에 이미 있어 추가 변경하지 않음
...
6. healthcheck 수정 검증에서도 `sh -c`를 거친 첫 직접 실행은 SQL 인용부호 손상으로 실패했고, 다음 실행은 현재 프로세스에 `MYSQL_PWD`를 매핑하지 않아 password 미전달로 실패했다. `MYSQL_PASSWORD`를 현재 PowerShell 프로세스의 `MYSQL_PWD`로만 매핑하고 MySQL client를 `sh -c` 없이 직접 실행해 정상·오류 credential 결과를 확인했다. 의도한 인증 실패 stderr가 PowerShell 종료 오류로 승격된 문제는 해당 명령만 비종료 처리해 실제 non-zero exit code를 확인했다.
7. health 대기 검증의 첫 시도는 PowerShell 명령 경계에서 proxy port 판별 정규식 escape가 손상돼 상태 변경 전에 중단됐다. 정규식 대신 Docker port 출력의 마지막 구분자 뒤 값을 정수로 파싱하도록 집중 수정한 뒤 같은 환경에서 검증이 통과했다.

## 반복하지 않은 검증과 이유

이번 추가 변경은 문서의 Compose health 대기 명령에 한정되므로 실제 재시작·대상 서비스 재생성·연결 경로만 검증하고, image와 제품 코드·bootstrap 동작에 관한 다음 검증은 반복하지 않았다.

- 공식 image pull·build
- Full smoke
- Preserved smoke의 구독 보존과 fixture 멱등성 assertion
- reset `true` 뒤 빈 상태
- reset `false` 복원 뒤 빈 상태

health 대기 문서 변경과 직접 관련 없는 전체 smoke를 다시 실행하면 근거가 늘지 않고 구독 상태와 검증 비용만 불필요하게 바뀌므로 기존 성공 결과를 사용했다.
...
- 현재 `.env.local`은 없으며 이번 검증에는 실행 중 컨테이너 값을 현재 PowerShell 프로세스에만 전달했다. 다음 재시작과 QA 수행 전에는 Runbook에 따라 `.env.example`을 `.env.local`로 복사하고 안전한 로컬 값을 준비해야 한다.
...
- PR은 최초 생성 시 Draft였으며 이후 상태는 이 보고서에 복제하지 않는다.
- 현재 Draft/Ready, head SHA, latest CI, mergeable과 review thread 수는 GitHub PR과 checks를 권위 있는 원본으로 확인한다.
- 자동 병합하지 않는다.
