...
| `REQ-SUB-001` | `skus.subscribable`, `subscriptions` 생성 데이터, 회원·SKU 참조, 수량·주기·날짜 제약 후보 |
...
- `REQ-SUB-001` 추적성에 `skus.subscribable`을 추가해 구독 생성 조건의 SKU 구독 가능 여부가 API-001, Backend 구현, QA 흐름에서 보이도록 보강했다.
- API-001 인수인계에 상품 목록 응답 후보의 필수 표시 데이터 검토 항목을 보강했다.
- API-001 인수인계에 상품 상세 응답 후보의 필수 표시 데이터와 배송 주기 표현 검토 항목을 보강했다.
- Backend 구현으로 넘길 결정에서 값 객체 사용 여부 자체를 다시 여는 표현을 피하고, 승인된 수량·배송 주기 불변 조건을 보호하는 값 객체의 구현 형태와 JPA 매핑 방식으로 정리했다.
- API-001 오류 범주에 `sku_id`, `quantity`, `delivery_cycle_weeks` 필수 입력 누락을 추가했다.
- `next_order_date`가 `created_date`보다 이후이며 `created_date + delivery_cycle_weeks`로 계산된다는 날짜 불변 조건을 제약 조건 후보에 추가했다.
- API 요청·응답 JSON, HTTP 상태, URL, 인증 구현, Flyway, JPA Entity, CodeRabbit 설정은 확정하거나 변경하지 않았다.
...
| `C:\Program Files\Git\bin\bash.exe scripts/validate-commit-message.sh --message "docs(data): DATA-001 리뷰 반영"` | 통과 |
| PR 리뷰 반영 후 재검증 | 통과 |
...
- PR #17은 Draft에서 Ready for review로 전환됐고 CodeRabbit 리뷰가 수행됐다. 이번 변경은 사용자가 지정한 리뷰 코멘트만 최소 반영한다.

## CodeRabbit 확인 결과

`.coderabbit.yaml`에서 Draft PR 리뷰가 비활성화되어 있어 최초 Draft PR에서는 리뷰가 건너뛰어졌다.

사용자가 PR #17을 Ready for review로 전환한 뒤 CodeRabbit 리뷰가 수행됐다. CodeRabbit 지적은 전부 반영하지 않고, 사용자가 지정한 항목만 제품·도메인·아키텍처 승인 범위 안에서 반영했다.
...
- 리뷰 반영 커밋 메시지: `docs(data): DATA-001 리뷰 반영`
...
- PR checks:
  ...
  - `CodeRabbit`: Ready for review 전환 후 리뷰 수행됨
- 자동 병합하지 않았다.
