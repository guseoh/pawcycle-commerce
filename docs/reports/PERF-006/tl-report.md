...
- PERF-007은 상태 변경과 래퍼 검증 전에 `git status --porcelain` 빈 출력과 `git rev-parse HEAD`·`git rev-parse origin/main`의 실제 SHA가 고정한 기준 commit과 일치하는지 확인한다.
...
- PERF-007은 래퍼의 경로·이름, 적용 가능한 version·commit과 SHA-256, 검증 명령·비민감 입력 설명을 기록하고 측정 직전 동일 SHA-256을 재확인한다.
- 래퍼 사전검증 직후이자 reset 준비 기동 직전에 최초 Git 검사를 다시 수행하고 작업 트리, `HEAD`, `origin/main`과 고정 기준 commit의 일치를 재확인한다.
- PERF-004 종료 상태는 volume 보존 `down`이며 필수 게이트 뒤 volume을 삭제하지 않는 승인 Compose 기동과 네 service `healthy` 확인을 QA reset 전에 수행한다.
- Reset 준비 기동은 cold start 표본과 PERF-002·003 반복 횟수·통계에서 제외하고, 실패하면 QA 상태 변경과 측정을 시작하지 않는다.
...
| 승인 순서 | Volume 보존 준비 기동·healthy 확인 → QA reset → reset `false` → seed 생성·확인 → cold 3회 → healthy → 30초 안정화 → warm 전체 |
| 준비 기동 경계 | Reset 전 최초 기동은 cold 표본과 PERF-002·003 반복 횟수·통계에서 제외, 기동·healthy 실패 시 QA 상태 변경과 측정 시작 금지 |
| 래퍼 게이트 | 실제 수정 아티팩트 식별자·SHA-256과 검증 명령·비민감 입력 설명 기록, 동일 SHA의 request parameter·container stats parsing 사전검증 필수 |
| 기준 고정 | PERF-007 시작 시 최신 `origin/main` SHA와 환경 fingerprint 기록 |
| Git 상태 | 최초 확인과 래퍼 사전검증 직후·reset 준비 기동 직전 재확인에서 `git status --porcelain` 빈 출력과 `git rev-parse HEAD`·`git rev-parse origin/main`의 실제 SHA·고정 기준 commit 일치 확인 |
| Git 결과 증거 | PERF-007 결과에 최초 검사와 사전검증 이후 재검사의 명령 결과·비교 결과를 모두 기록 |
| 변경 감지 | PERF-004 이후 제품 코드·실행 설정 변경 시 측정 시작 금지와 사용자 결정 요청 |
| 실패 경계 | 준비 기동·healthy 실패 시 상태 변경 전 중단, 상태 변경 이후 임의 reset·재재실행 없이 중단 |
...
- `scripts/validate-commit-message.sh --message 'docs(tl): PERF-006 측정 준비 순서 보완'`: 통과
- `git diff --check`: 통과
- 변경 문서 민감정보 패턴 검사: 통과
...
PR #53의 CodeRabbit과 Codex Review를 확인했다. Clean worktree 게이트 전파, fingerprint 필드, DB 상태 변경 범위, 래퍼 아티팩트 식별·재검증 조건, 사전검증 이후 Git 재검사와 최신 Codex P2의 reset 전 volume 보존 준비 기동·healthy 확인 요구를 승인 범위 안에서 반영했다. 최신 head 재검토 결과는 PR 원격 상태와 완료 보고에서 확인한다.

## AI 리뷰 미반영 항목과 이유

의도적으로 미반영한 항목은 없다. 최신 유효 스레드는 현재 문서 근거를 답변하고 resolve하며 최신 head의 자동 재검토 상태를 확인한다.

## 적용 방법

Platform/SRE는 `docs/performance/PERF-006-local-baseline-rerun-approval.md`와 `docs/handoffs/PERF-006/tl-to-sre.md`를 최신 입력으로 사용해 PERF-007을 준비한다. PERF-007 결과에는 사용한 래퍼 식별자·SHA-256, 상태 변경 없는 검증 명령·비민감 입력 설명·결과와 측정 직전 SHA 재확인 결과, 최초 Git 검사와 사전검증 직후·reset 준비 기동 직전 Git 재검사의 명령 결과·비교 결과, reset 전 준비 기동·healthy 결과와 cold 표본 제외 근거를 기록한다.
...
Platform/SRE가 작업 ID `PERF-007`로 최신 기준 commit·환경 fingerprint와 래퍼 사전검증·Git 재검사 게이트를 확인한 뒤, volume 보존 준비 기동과 네 service healthy 확인을 거쳐 승인된 순서의 cold와 warm 전체 기준선을 한 번 측정한다. 준비 기동은 cold 표본에서 제외한다.

## Git 결과

- 최신 `main` 기준: `54eb9230b20556cfdd01c443b8c2749fe0b119fe`
- 병합된 PR #51의 기존 역할 브랜치를 정리하고 깨끗한 `ops/tl`을 재생성했다.
- 최초 승인 문서 초안 commit: `774dd52 docs(tl): PERF-006 전체 기준선 재실행 승인`. 리뷰 반영 전 PR 브랜치 commit이며 PERF-007 기준 입력 SHA로 사용하지 않는다.
- 원격 `origin/ops/tl` push 완료
- 리뷰가 반영된 최종 승인 문서는 PR #53의 최신 head와 병합 후 `origin/main` 이력에서 확인한다. 현재 review 반영 commit은 자기 SHA를 같은 commit 안에 기록할 수 없으므로 PR 원격 이력과 완료 보고에서 확인한다.
