...
| QA-F006-002 | 상품 목록부터 상세까지 논리적 Tab·focus·Enter | 사용자가 논리적 Tab 순서와 시각적 focus 표시를 확인하고, `상세 보기`를 Enter로 실행해 상세 진입을 완료 | 통과 |
...
| QA-F006-005 | 구독 목록·상세와 로그아웃 keyboard 동작 | 사용자가 링크·버튼의 keyboard 동작, 구독 목록·상세 이동, 로그아웃 후 회원 상태 정리와 보호 화면 이동을 확인 | 통과 |
| QA-F006-006 | 입력 오류 안내와 focus 이동 | 사용자가 오류 안내와 focus 처리를 확인 | 통과 |
| QA-F006-007 | 첫 Tab에 skip link 표시와 본문 이동 | `Ctrl+L`로 새 진입 후 첫 Tab에 `본문으로 건너뛰기`가 표시됐고, Enter 후 URL이 `/products#main-content`로 바뀌며 본문으로 이동함을 사용자가 확인 | 통과 |
...
| QA-F006-017 | 상품 ID 0 차단 | `/products/0`을 `returnTo`로 실제 로그인 후 `http://localhost:8080/products` | 통과 |
| QA-F006-018 | 구독 ID 0 차단 | `/subscriptions/0`을 `returnTo`로 실제 로그인 후 `http://localhost:8080/products` | 통과 |

결과 합계는 통과 15건, 일부 미실행 0건, 전체 미실행 3건, 실패 0건, 차단 0건이다.

## 핵심 검증 결과

- 상품 목록 → 상품 상세 → 로그인 → SKU·수량·배송 주기 입력 → 구독 생성 → 목록·상세 → 로그아웃의 keyboard-only 흐름을 사용자가 same-origin GUI 브라우저에서 완료했다.
- skip link는 새 상품 목록 진입 직후 첫 Tab에 실제 표시됐고, Enter 후 `/products#main-content`와 본문 이동을 확인했다.
- 외부 절대 URL, protocol-relative URL, `/login`, 상품·구독 ID 0, 음수 상품 ID, 음수 구독 ID와 숫자가 아닌 ID는 실제 브라우저 로그인 뒤 모두 same-origin `/products`로 수렴했다.
...
- 정상 화면 keyboard-only 흐름에서 focus 누락, 비논리적 순서, Enter·Space 비활성 또는 입력 오류 focus 결함은 관찰되지 않았다.
- GET 오류 화면의 재시도 keyboard 접근성과 session 만료는 제품 통과가 아니라 전체 미실행이다.
...
- 정상 화면 keyboard-only 흐름과 skip link Enter는 통과했지만, GET endpoint별 오류·재시도 UI의 focus·Enter 동작과 session 만료는 실제 브라우저 미재현 위험으로 남는다.
...
**조건부 통과**다. 정상 화면 keyboard-only 흐름, skip link Enter 이동과 비정상 `returnTo` 경계는 실제 GUI 관찰과 브라우저 URL 증거로 통과했다. 정상 MVP 흐름의 회귀와 P0·P1·계약 위반은 발견되지 않았다. 다만 GET 오류 화면의 재시도 keyboard 접근성과 session 만료 3건은 안전 재현 수단 부재로 전체 미실행이므로 조건부 판정을 유지한다.
...
- branch: `test/qa`
- QA 산출물 commit: `f75f95d test(qa): FOUNDATION-006 MVP 접근성 후속 검증`
- `origin/test/qa` push와 upstream 설정 완료

## PR 결과

- 제목: `test(qa): FOUNDATION-006 MVP 접근성 후속 검증`
- PR: `#48`
- head/base: `test/qa` → `main`
- 상태: Open, Ready for review
- 원격 제목·UTF-8 본문·head/base·Draft 상태 재확인 완료
- CI: commit·PR convention, Application validation, 협업 알림 통과
- CodeRabbit: 계획서 LGTM, 실제 결과 추적성 보강 1건을 유효한 지적으로 반영
- 자동 병합 미수행
