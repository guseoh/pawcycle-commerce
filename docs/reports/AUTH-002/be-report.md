...
- 정규화한 email login, unique·not-null email, non-null `password_hash VARCHAR(100)`
...
PR 생성 후 AI review 결과를 확인한다. 현재 문서는 ARCH-006 review에서 남은 DR1~DR3를 직접 해결하도록 작성했다.
...
- commit·push 결과는 검증 완료 후 이 보고서의 후속 상태에서 추적한다.

## PR 결과

- Draft PR 생성, 원격 CI와 AI review 결과 확인이 남아 있다.
- 자동 병합하지 않는다.
