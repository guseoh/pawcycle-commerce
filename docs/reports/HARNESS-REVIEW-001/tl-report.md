...
- `.github/pull_request_template.md`에 작업 필요성, 해결 방법과 핵심 결정, 검토한 대안, 위험과 제한: 한계와 트레이드오프 섹션 추가
...
CodeRabbit이 지적한 동적 PR 상태 고정 문제와 Codex가 지적한 위험 영역 추출 호환성 문제를 반영했다. 예외 브랜치 관련 지적은 사용자 승인과 작업 범위를 대조해 미반영했다.

## AI 리뷰 미반영 항목과 이유

`ops/tl-harness-review-001`은 정규 역할 브랜치와 Discord 역할 매핑에 포함되지 않는다. 그러나 기존 `ops/tl`에는 서로 분기된 미병합 이력이 있었고, 해당 이력을 삭제·reset·rebase·force push 없이 보존하기 위해 사용자가 이번 작업에 한해 예외 브랜치 사용을 명시적으로 승인했다. 이번 작업에서 `AGENTS.md`, 역할 브랜치 정책 또는 Discord 알림 자동화를 영구 확장하는 것은 승인 범위를 넘으므로 반영하지 않는다.
...
PR #67을 `main` 대상으로 생성했다. PR 상태, head SHA, CI와 review 상태는 GitHub를 권위 원본으로 확인한다. 자동 병합하지 않으며 사용자가 직접 병합 여부를 결정한다.
