...
| Repository Validation | 최신 PR head의 GitHub PR checks | conventions와 application validation 전체 성공 확인 |
| Pre-merge Discord report | GitHub PR checks | 예상된 실패: 기본 브랜치에 collector가 없어 `exit 1`, 전송 단계 미도달 |
...
보완 commit push 후 최신 PR head의 Repository Validation 전체 성공을 GitHub PR checks에서 확인했다. 이어진 collaboration report는 기본 브랜치에 collector가 없는 상태를 `exit 1`로 명시해 실패했고 builder·sender와 Webhook 호출에는 도달하지 않았다. 이는 병합 전 호환 실패를 숨기지 않는 승인된 동작이며, 실제 전송 검증은 병합 후 기본 브랜치에서 수행한다.
...
1. PR #40의 최신 Repository Validation 전체 성공과 모든 유효한 review thread의 resolved 상태를 GitHub에서 확인한다.
...
- 보안·상태 판정 보완 커밋: `ab83d5a` `fix(discord): 알림 보안과 상태 판정 보완`
...
- 최초 상태: Draft
- 자동 병합하지 않는다.
- 최신 PR head의 Repository Validation 전체 성공과 모든 유효한 review thread의 resolved 상태를 GitHub에서 확인한다.
- 현재 head, CI run, Draft·Ready·mergeable 상태와 review thread 수는 GitHub를 기준으로 한다.
