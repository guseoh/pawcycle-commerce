...
- raw request는 변수명뿐 아니라 `HttpServletRequest`/`ServletRequest` 타입도 식별해 request 객체 자체를 차단하고, body/payload 접근도 차단한다. 반면 `request.getMethod()` 같은 안전한 메타데이터 projection은 허용한다. 테스트가 이름을 `req`로 바꾼 fixture까지 포함해 이 positive/negative 경계를 직접 검증한다.
...
- PASS — `git diff --check`
- PASS — correction 이전 구현은 Repository Validation의 Backend/MySQL, Harness, conventions와 Application validation을 통과했다.
- NOT RUN — 로컬 MySQL 기반 subscription 통합 테스트는 JDBC URL이 구성되지 않아 Spring context 시작 전에 실패했다. 동일 범위는 CI의 MySQL 8.4 Backend 전체 suite에서 검증한다.
- 최신 HEAD, Repository Validation, PR Metadata Validation과 review 상태는 동적 값이므로 PR #298의 GitHub 상태를 권위 원본으로 확인한다.
...
- CHANGE — Billing 성공 INFO의 raw `memberId` 제거, logger 직접 민감 인자 회귀 테스트 추가, raw request 객체/body의 타입 기반 검출과 안전한 request metadata projection 경계 보정
