...
사용자가 production에서 직접 검증한 EC2 `StatusCheckFailed` SNS email 알림 결과를 비민감 증거로 기록하고, 실행 중 확인된 AWS CLI NoRegion 결함을 재발 방지한다.

## 입력 문서

OPS-015 사용자 실행 결과, OPS-015 Runbook, production 알림 script 계약과 production 운영 아키텍처를 입력으로 사용했다.

## 승인 입력

저장소 준비는 일반 작업으로 수행됐고, 실제 production 알림 검증은 사용자가 별도로 승인하고 직접 실행한 고위험 작업이었다. 이번 보완에서는 추가 AWS 실행을 승인하지 않았다.
...
STS 호출의 승인 region 명시, fake AWS 회귀 test, OPS-015 보고서·인수인계와 현재 운영 상태 문서를 갱신한다. Runbook에는 전역 AWS CLI region 설정에 의존하지 않는 실행 계약을 명시한다.

## 변경하지 않은 범위

이번 보완에서 AWS CLI create·verify·set-alarm-state·cleanup과 AWS 리소스 생성·변경·삭제를 실행하지 않았다. CPU·메모리·디스크·application 알림, Agent, Lambda, Discord, 자동 복구, IAM, application·DB 동작도 변경하지 않았다.
...
알림 공통 script와 fake AWS test, OPS-015 보고서·인수인계·Runbook, production 운영 아키텍처를 변경한다.
...
STS 대상 확인도 `PAWCYCLE_ALERT_REGION`에서 검증된 서울 region을 명시적으로 사용하므로 전역 AWS CLI region 설정 부재에 의한 NoRegion 재발을 차단한다. 알림 리소스의 계약과 동작은 변경하지 않는다.
...
구현자가 아닌 사용자/Tech Lead가 production에서 ALARM·OK email 수신과 verify 결과를 직접 확인했다. 저장소 보완에서는 fake AWS가 STS 호출의 서울 region 전달을 독립된 호출 marker로 검증한다.

## 실행하지 못한 검증과 이유

추가 AWS 실행은 명시적 제외 범위이므로 create·verify·set-alarm-state·cleanup을 다시 실행하지 않았다. 실제 EC2 장애를 유발하지 않았고 실제 `StatusCheckFailed` 발생 시 탐지 시간이나 복구 시간도 측정하지 않았다. Application 전체 test는 application 동작을 변경하지 않아 반복하지 않았다.
...
PR의 유효한 리뷰 지적은 GitHub review thread를 권위 원본으로 확인한다.

## AI 리뷰 미반영 항목과 이유

현재 저장소 문서에 동적 review 상태를 고정하지 않는다.
...
Cleanup 절차는 실행 검증하지 않았다. 단일 host 장애 도메인과 email 전달 채널 의존성이 남고, actual production DB restore 등 OPS-015 밖의 기존 미완료 운영 항목은 그대로다.
...
최신 `main`에서 새 `ops/sre`를 준비했다. commit·push 결과는 Git과 PR을 권위 원본으로 확인한다.

## PR 결과

`main` 대상 PR을 생성하고 자동 병합하지 않는다. 현재 head와 Repository Validation 상태는 GitHub를 권위 원본으로 확인한다.
