...
- PERF-004 기준 commit과 cold start 3회 결과
...
- `[string]$Body = $null`: `System.String`, `IsNull=False`, `Length=0`, null 비교 사용 시 body 첨부 경로
- 두 factor 값: `System.Object[]`, count 2, `$number * $factor`에서 PERF-004와 같은 `op_Multiply` RuntimeException
- 수정 방향 확인: null 또는 empty body 미첨부, non-empty JSON body만 첨부
- 수정 방향 확인: exact unit lookup에서 scalar factor count 1과 byte 계산

이 검증은 PowerShell object만 사용했으며 HTTP, Docker, DB와 실제 ID를 사용하지 않았다. 래퍼 원본을 repository script로 추가하지 않았다.

## 주요 결과

- PERF-004 cold start 3회와 환경 fingerprint를 부분 결과로 유지했다.
- Warm HTTP·container 결과는 `미완료·사용 불가`로 유지했다.
- QA seed와 첫 route warm-up으로 최초 state가 소비된 경계를 명시했다.
- 선택지 A·B·C와 권고안 A의 환경 동일성·초기화·사전검증·단일 재실행 조건을 작성했다.
...
- 외부 요청 없는 수정 방향의 body 조건·scalar factor 검증: 통과
...
- 실제 endpoint·container stats를 사용한 수정 래퍼 검증: 이번 작업의 상태 변경·실행 제외 범위
...
성능 측정을 실행하거나 제품·실행 설정을 변경하지 않았다. Cold 부분 결과의 의미와 warm 미완료 경계만 보존했다.

## 적용 방법

사용자/Tech Lead가 `docs/performance/PERF-005-local-baseline-rerun-decision-request.md`의 A·B·C 중 하나를 승인한다. 승인 전에는 어떤 재실행도 시작하지 않는다.

## 남은 위험

- 원본 임시 래퍼 전체가 없어 수정본과 원본의 완전한 line-by-line 비교는 불가능하다.
- A의 환경 동일성이 충족되지 않으면 PERF-004 cold와 새 warm을 하나의 기준선으로 사용할 수 없다.
- 재실행 중 상태 변경 이후 실패하면 다시 중단해야 한다.
- C 선택 시 warm 기준선은 계속 없다.

## 다음 작업

Tech Lead가 A·B·C 중 하나와 추가 조건을 결정한다. A 또는 B를 승인하면 별도 작업 ID에서 상태 변경 없는 래퍼 검증을 먼저 수행하고 승인된 초기화 경계로 한 번만 실행한다.
