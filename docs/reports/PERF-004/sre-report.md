...
- Cold start 3회 전체 경과는 25,917 ms, 25,155 ms, 25,293 ms였다. 다만 seed가 cold 뒤에 생성돼 승인된 seed-before-cold 순서를 따르지 않은 부분 관측이다.
...
PERF-005의 순수 PowerShell 객체 최소 재현에서 두 type·연산 특성을 확인하고 body 미첨부 조건과 scalar factor라는 수정 방향을 확인했다. 실제 재실행용 수정 래퍼 아티팩트는 검증하지 않았으며 endpoint, Docker, DB와 QA state는 사용하지 않았다.
...
DB schema와 migration 변경은 없다. 승인된 QA subscription reset 뒤 cold 3회를 먼저 수행했고 그 뒤 seed 하나를 준비했다. 이 순서는 승인된 seed-before-cold 순서와 달랐다. Warm 측정 중단 후 추가 reset이나 상태 변경을 수행하지 않았고 QA 회원 외 데이터와 volume은 삭제하지 않았다.
...
제품이나 실행 설정을 변경하지 않았으므로 성능 변경은 없다. Cold start 3회는 순서 이탈이 있는 부분 관측일 뿐 승인된 cold 기준선이 아니며 warm HTTP·container 기준선은 미완료다.

## 적용 방법

완료된 결과가 아니므로 새 warm 결과와 결합하거나 regression 비교·최적화 근거로 적용하지 않는다. Tech Lead는 `docs/performance/PERF-004-local-baseline-results.md`의 cold 부분 관측과 순서 이탈, 실패 경계를 검토하고 A·B·C 중 하나를 결정한다.
...
- Seed-before-cold 순서 이탈 때문에 cold 3회는 승인된 하나의 완전한 기준선 구성요소가 아니다.
