...
PERF-004에서 사실 기반으로 얻은 cold start 부분 관측과 warm 측정 중단 근거를 전달하고, 별도 재실행 승인 여부를 Tech Lead가 판단할 수 있게 한다.
...
- Cold start 3회 전체 경과 부분 관측: 25,917 ms, 25,155 ms, 25,293 ms
- 세 회차의 mysql, backend, frontend와 proxy health convergence
- Compose·QA fixture·reset·smoke 통과 근거
- Warm 측정이 첫 유효 표본 전에 중단된 실패 경계

이 결과는 cold start 부분 관측으로만 보존한다. Seed가 cold 뒤에 생성돼 승인된 seed-before-cold 순서를 따르지 않았으므로 완료된 PERF-004 기준선이나 새 warm 결과와 결합 가능한 승인 cold cohort가 아니다.
...
- PERF-004 측정 수행 당시 commit·push·PR이 수행되지 않았는지
...
- Cold start 값은 seed-before-cold 승인 순서를 따르지 않아 새 warm 결과와 결합하거나 SLO·threshold·regression 비교에 사용할 수 없다.
