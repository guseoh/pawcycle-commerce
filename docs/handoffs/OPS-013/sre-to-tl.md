...
- S3 bucket 계약과 EC2 instance role 최소 권한: Verified
...
- application 중지 후 `MemAvailable` 970 MiB, 복구 후 640 MiB 확인: Verified
- 유지보수 종료 후 새 Session Manager 세션의 읽기 재검증: Verified

실제 production DB에는 restore하지 않았다. 위 결과는 운영 backup과 별도 임시 DB의 복원 가능성 검증이며 actual production restore 완료를 의미하지 않는다. 실제 identifier·credential·row·table count 값은 기록하지 않았다.
...
2. OPS-013 전용 private bucket의 승인 계약과 지정 prefix 14일 lifecycle을 확인했다.
...
2026-07-24 실행에서는 temporary restore container·volume·work directory와 temporary source가 남지 않았고 production MySQL container·volume이 유지됐다. S3 object는 14일 lifecycle 대상으로 두며 instance role에 `DeleteObject` 등 자동 삭제 권한을 추가하지 않았다.
...
자동 schedule·실패 알림, 실제 production restore, 기존 shared bucket 재사용, cross-region·versioning·Glacier·KMS, 별도 restore EC2, 장기 보존과 RPO/RTO 확정은 별도 승인이 필요하다. 현재 검증 결과를 actual production restore 완료나 자동화 완료로 확장하지 않는다.
