...
2. bucket Public Access Block 4/4, SSE-S3와 지정 prefix 14일 lifecycle을 적용한다.
3. instance role에 bucket 계약 read와 지정 prefix `PutObject`·`GetObject`만 허용한다.
4. DDL·migration이 없는 저부하 시점에 `backup`을 실행한다.
5. completion marker까지 성공한 backup ID로 `restore-verify`를 실행한다.
6. schema·Flyway·핵심 table count 일치와 temporary resource cleanup을 확인한다.
7. production release·HTTPS state·MySQL container·volume이 바뀌지 않았음을 비민감 상태로 기록한다.
...
- compressed dump가 승인된 5 GiB single-request upload 한도 초과
...
script는 종료 trap으로 temporary container·volume·work file을 제거한다. 강제 종료 뒤에는 정확한 backup ID로 `cleanup`을 실행한다. cleanup은 OPS-013 label, `none` network와 production volume 미사용을 확인한 resource만 제거한다. production MySQL·volume·release·HTTPS state와 S3 object는 삭제하지 않는다.
...
- bucket과 EC2가 같은 승인 region이며 PAB 4/4·SSE-S3·versioning 비활성·14일 prefix lifecycle인가?
...
실제 bucket·IAM·lifecycle 적용, 운영 backup·isolated restore와 증거 확인은 사용자 작업이다. 자동 schedule·알림, 실제 production restore, cross-region·versioning·Glacier·KMS와 장기 보존은 별도 승인이 필요하다.

## 남은 위험

- 같은 EC2·EBS 장애는 backup source와 restore 검증 환경에 함께 영향을 줄 수 있다.
- compressed dump 5 GiB 초과는 multipart 권한·cleanup 설계가 필요한 별도 승인 항목이다.
