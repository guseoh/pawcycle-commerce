...
- `Write-Output '<후속 작업 ID>' | py -3 scripts/validate-task-artifacts.py --from-stdin`
- `& "C:\Program Files\Git\bin\bash.exe" scripts/validate-commit-message.sh --message "<커밋 메시지>"`

후속 Backend 작업 ID는 현재 검증 하네스가 허용하는 접두사로 정한다. `BE-001` 같은 새 접두사를 사용하려면 먼저 사용자 승인과 검증 하네스 변경이 필요하다.
...
사용자가 DATA-001, API-001, AUTH-001, ARCH-001, FOUNDATION-000을 구현 입력으로 승인한 뒤 실제 최소 구현을 진행하는 경우:

```text
feat(backend): 첫 수직 MVP 구독 API 최소 구현
```

위 제목은 커밋/PR 제목 후보이며, 실제 작업 ID는 하네스가 허용하는 접두사로 별도 확정한다.
