...
- 제어 문자, local-part 또는 domain 내부 공백, 여러 `@`, 빈 local-part, 빈 domain과 전체 길이 254자 초과 입력은 `VALIDATION_FAILED`로 거부
...
첫 인증 구현에서는 device/session 목록 endpoint, DTO, service, repository, session 관리 화면과 다른 session 또는 device 강제 로그아웃 기능을 만들지 않는다.

email 변경 endpoint, 탈퇴 endpoint, 탈퇴 email tombstone 또는 재사용 정책, 탈퇴 후 재가입 처리와 SMTPUTF8·IDN 변환 로직을 만들지 않는다.
...
- ASCII email 허용, 앞뒤 ASCII space·tab 제거, local-part case 보존, domain 소문자화
- 제어 문자, trim 후 빈 값, 내부 공백, 여러 `@`, 빈 local-part, 빈 domain, 254자 초과와 ASCII 범위 밖 문자의 `VALIDATION_FAILED`
- unique·not-null·정규화 충돌 실패
...
- PR #31의 최초 Codex review 3건과 CodeRabbit review 1건을 모두 반영했다.

| Review thread ID | 검토 주체 | 반영 내용 | 상태 |
| --- | --- | --- | --- |
| `PRRT_kwDOTLGt186P4376` | Codex | malformed email 거부 조건과 `VALIDATION_FAILED` 검증 복원 | 반영 완료 |
| `PRRT_kwDOTLGt186P4379` | Codex | device/session 목록과 강제 로그아웃 Deferred 복원 | 반영 완료 |
| `PRRT_kwDOTLGt186P438A` | Codex | SMTPUTF8, email 변경·탈퇴·재가입 정책 Deferred 복원 | 반영 완료 |
| `PRRT_kwDOTLGt186P45gT` | CodeRabbit | 작업 보고서의 개인 로컬 경로와 인증 계정 정보 제거 | 반영 완료 |
| `PRRT_kwDOTLGt186P5Jtt` | CodeRabbit 후속 검토 | 최초 review 4건의 thread ID와 수정 내용 추적성 추가 | 반영 완료 |

- Codex review 3건과 CodeRabbit review 총 2건에 미반영 항목은 없다.
