...
- EC2의 실제 memory·disk·CPU 여유와 재부팅 복구
...
실제 SSM IAM, 공개 GHCR anonymous pull, EIP HTTP, reboot와 자원 사용량은 사용자 검증 전 통과로 표시하지 않는다. memory 부족이나 migration 차이를 발견하면 배포를 중단한다.

## 다음 권장 작업

사용자가 PR diff와 검증을 확인해 병합한 뒤 OPS-009 gate부터 실제 적용한다. 단일 release와 rollback이 확인된 뒤 TLS·DNS와 backup·restore를 우선 결정하고, 자원 여유가 검증된 뒤 Blue·Green을 검토한다.
