...
- 사용자/Tech Lead가 release `b9cf3cf51c5ffd4b85c6eafc78706ed079e299d6`의 활성화, 동일 `current-sha`, 네 container health와 proxy의 host HTTP `80` 단독 공개를 확인했다.
- EC2 내부 loopback `127.0.0.1`의 두 endpoint와 별도로 외부 사용자 PC 브라우저에서 EC2 외부 HTTP `80`의 `/products`·`/api/products`를 확인했다.
- `/pawcycle-commerce/prod` 아래 네 SecureString과 EC2 role의 해당 prefix `ssm:GetParameter` 값 비출력 조회, runtime materialize 성공을 확인했다.
- 배포 전 단일 시점에 disk 38G 중 3.0G 사용·35G 여유·8%, available memory 약 1.4 GiB와 swap 0B를 확인했다.
- 재부팅 뒤 Docker 자동 시작, 동일 MySQL volume, 동일 SHA, health와 EC2 내부 loopback smoke 복구를 확인했다.

## 미확정 결정

- IAM 정책 전체의 최소 권한 독립 검토
- GHCR package Public 설정 결과와 실제 image digest
- 지속 CPU, 부하 중 memory, OOM과 장기 성능
- 실제 이전 SHA rollback
...
8. 미충족 후속 게이트로 이전 SHA rollback 뒤 동일 MySQL volume과 application health를 확인한다. 이 단계는 현재 인수인계 완료 조건이 아니다.
...
- 네 service가 healthy이고 EC2 내부 loopback 두 HTTP smoke가 성공하는가?
- 외부 사용자 PC에서 EC2 외부 HTTP `80`의 두 endpoint가 성공하는가?
- runtime file 세 개가 mode `600`이고 Secret이 출력되지 않았는가?
- stop/start와 재부팅 뒤 volume 이름과 데이터가 유지되는가?
- 후속 rollback을 실행했다면 volume 이름·data와 service가 유지되는가?
...
- 실제 `t3.small`의 배포 전 단일 시점 disk·available memory·swap은 확인했지만 지속 CPU, 부하 중 memory, OOM과 장기 성능은 미확정이다.
...
SSM prefix 조회 성공은 확인했지만 IAM 정책 전체의 최소 권한, 공개 GHCR anonymous pull, 실제 이전 SHA rollback과 지속 자원 사용량은 별도 증거가 없어 통과로 표시하지 않는다. memory 부족이나 migration 차이를 발견하면 배포를 중단한다.

## 다음 권장 작업

단일 release 활성화, SSM materialize, EC2 내부 loopback·외부 사용자 PC HTTP와 재부팅 복구는 확인됐다. 실제 이전 SHA rollback은 완료 조건이 아닌 미충족 후속 게이트로 남기고, TLS·DNS와 backup·restore는 별도 승인하며 지속 자원 여유가 검증된 뒤 Blue·Green을 검토한다.

## 완료 조건

- 실제 두 image가 동일 병합 SHA와 digest로 확인됨
- 모든 health, EC2 내부 loopback과 외부 사용자 PC HTTP smoke, reboot가 통과함
- runtime file mode와 Secret 비노출이 확인됨
- 실제 실행·미실행·실패 기록이 비민감 형식으로 남음

## 미충족 후속 게이트

- 실제 이전 SHA rollback은 미실행이다. 실행 전 별도 사용자 승인 작업으로 분리하고, 실행한 경우 동일 MySQL volume·data와 application health를 확인한다.
