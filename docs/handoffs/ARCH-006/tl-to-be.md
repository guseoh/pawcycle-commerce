...
- 최소 인증 API·CSRF 전달·cookie 세부 정책·credential 계약 누락을 구현 전 중단 조건으로 식별했다.
...
- CSRF 활성화
- 상태 변경 요청의 유효한 CSRF token 요구
- HttpOnly session cookie
- session id 응답 body·로그 비노출
...
4. CSRF token 획득·노출 방식, token/header/cookie 이름, 갱신 시점, 로그인 전후·로그아웃 후 처리와 Frontend API client 전송 규칙
...
- 승인된 전달 계약에 따른 유효한 CSRF token 요청 허용
- 로그인 성공·실패
...
- HttpOnly 적용과 session id 응답 body·로그 비노출
...
- ARCH-006 PR #29의 CodeRabbit 상세 review thread 2건을 확인했다.
- cookie 세부 정책과 CSRF 전달 계약의 승인 상태 지적 2건을 모두 반영했으며 미반영 항목은 없다.
...
- HttpOnly만 Approved cookie 요구사항이다. SameSite=Lax와 운영 Secure는 기본 후보이며, SameSite·Secure·cross-origin·domain·path·수명은 Frontend origin과 로컬·운영 배포 구성을 확인한 뒤 제안·검증해야 한다.
- CSRF 보호 활성화만 Approved다. token 획득·노출·이름·갱신과 로그인/로그아웃 전후 Frontend 전송 계약은 Decision Required다.
...
- 최소 인증 API·CSRF 전달·credential 계약이 해결되지 않은 상태에서 해당 구현을 확정해야 함
- SameSite·Secure·cross-origin·domain·path·cookie 수명이 정해지지 않은 상태에서 cookie 세부 구조를 확정해야 함
