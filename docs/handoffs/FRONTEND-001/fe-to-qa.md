...
- CSRF token은 메모리에만 두고 로그인 전과 로그인 성공 직후 획득한다.
- `CSRF_INVALID` 뒤 token은 다시 받지만 실패한 POST는 자동 재실행하지 않는다.
...
7. `CSRF_INVALID`를 유도하면 token 갱신 안내가 나오며 로그인·구독 생성·로그아웃 POST가 자동 재실행되지 않는다.
8. 목록의 `subscriptions: []`는 빈 상태이고, 값이 있으면 서버가 준 내림차순을 그대로 표시한다.
9. 미존재·타인 소유·비숫자 구독 ID는 같은 조회 불가 화면이며 소유권 정보를 노출하지 않는다.
10. 일시적 상품·목록·상세 오류는 명시적 재시도 후 회복하고, 오류 요약은 키보드 포커스를 받는다.
11. ISO 날짜가 로컬 timezone 이동 없이 `YYYY. M. D.`이고 가격은 서버 반환값과 일치한다.
12. 좁은 화면, keyboard-only, label/fieldset, focus-visible과 skip link를 확인한다.
