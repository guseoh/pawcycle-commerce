...
첫 수정 head의 Repository Validation run `29151260379`에서는 동일 transaction 안에서 중복 email 위반 뒤 실행한 음수 price assertion이 실패했다. 두 제약 검증을 각각 독립된 rollback transaction으로 분리했으나, 재검증 run `29151342478`에서도 동일 테스트 179행이 실패했다. 이후 CHECK metadata와 root `SQLException`을 식별하는 진단 구조로 원인을 확정해 현재 head에서 해결했다.

## 현재 head 검증 결과

- 검증 head: `4ba57ba2b32282d49e12e77e7c5cf2449a538ede`
- Repository Validation run: `29151829482`
- Java 25 Backend test/build 통과
- OPS-006 MySQL 8.4 service 초기화와 version 검증 통과
- `chk_skus_price_nonnegative`의 `CHECK`, `ENFORCED=YES` metadata 확인 통과
- 음수 가격 insert가 root `SQLException`의 constraint명, SQLState `HY000`, vendor code `3819`로 해당 CHECK 위반임을 확인
- Frontend install/lint/build 회귀 통과

기존 실패는 MySQL CHECK 위반이 발생했지만 Spring 예외를 `DataIntegrityViolationException` 구체 타입으로 고정해 검증한 데서 발생했다. 현재 테스트는 `DataAccessException` 계층과 CHECK 위반 root cause를 함께 검증한다.
...
- CHECK metadata와 root `SQLException`의 constraint 식별 정보
