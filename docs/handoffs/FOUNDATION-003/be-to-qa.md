...
첫 수정 head의 Repository Validation run `29151260379`에서는 동일 transaction 안에서 중복 email 위반 뒤 실행한 음수 price assertion이 실패했다. 두 제약 검증을 각각 독립된 rollback transaction으로 분리했으나, 재검증 run `29151342478`에서도 동일 테스트 179행이 실패했다. 같은 검증에 대한 집중 수정·재검증 후에도 실패가 유지돼 추가 반복을 중단했으며, QA 인수 전 Backend 원인 분석이 더 필요하다.
