...
- email 정규화·unique/not-null, `password_hash VARCHAR(100) NOT NULL`
...
| DR3 | email 정규화/unique/not-null, `password_hash VARCHAR(100) NOT NULL`, seed 금지·fixture 분리 | V1 migration·Entity·test data 작성 전 |
...
- 응답·로그·URL에 password/hash/session id/token 부재
