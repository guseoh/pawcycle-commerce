...
공통 시각 토큰, URL·overlay·중복 제출 규칙은 [정식 재설계 기준](./MVP4-UX-004-customer-commerce-redesign.md)을 따른다.
...
| 레퍼런스 | Kurly의 2단 헤더·중앙 검색·카테고리 메뉴는 `CONFIRMED/ADAPT`; Olive Young의 넓은 검색과 GNB는 `CONFIRMED/ADAPT`, 과밀한 링크 수는 `REJECT`. Kurly에서 검색 버튼 클릭으로 `/search?sword=…` 전환을 확인했으며 Enter는 해당 관찰에서 이동하지 않아 PawCycle 기준으로 복제하지 않는다. |
| 최종 IA | 1행: 로고, 배송/지원 같은 저빈도 유틸리티, 로그인/마이/장바구니. 2행: 카테고리 trigger, 중앙 검색, 핵심 내비게이션 `상품/정기배송/주문`. 모바일: 로고·검색·장바구니·메뉴만 남기며 검색은 별도 전체 폭 행으로 확장한다. |
...
| hover/focus/navigation | 링크 hover는 underline 또는 색+밑줄, 버튼 hover는 surface 변화를 사용한다. `Tab`: skip link→logo→nav→search→account→cart 순서. `/login?returnTo=`에는 현재 안전한 GET 경로만 전달한다. |
...
| success | 검색 제출 후 `/products` 결과 heading에 focus 또는 programmatic announcement를 제공한다. 로그인 성공 후 원래 안전한 GET 위치로 복귀한다. |
| responsive | ≥1024 두 행, 600–1023 검색이 둘째 행 전체 폭, <600 로고/검색/장바구니/메뉴. drawer 너비 `min(360px, 100vw)`; 320px에서도 가로 스크롤 금지. |
| accessibility | search landmark 1개, `type=search`, visible label 또는 동등한 접근성 이름, drawer focus trap·Escape·trigger 복귀, 현재 경로 `aria-current=page`. |
...
| 레퍼런스 | Kurly의 프로모션→상품 rail 구조와 Olive Young의 제어 가능한 carousel은 `CONFIRMED/ADAPT`. 자동 재생 중심 hero와 반복 추천은 `REJECT`. |
...
| visual hierarchy | hero는 최대 360px 높이, copy 7열·보조 시각 5열. 상품 rail은 4–5개가 일부 잘리지 않고 보이며 제목/근거/전체보기 순서다. 배경색 alternation은 최대 3종이다. |
...
| interaction | hero CTA는 `/products`; 펫 유형은 `/products?petType=`; rail 버튼은 한 카드 폭 단위 이동한다. drag만 요구하지 않는다. 추천 CTA는 로그인 또는 `/pets`로 안전하게 이동한다. |
...
| empty | 추천 없음은 `아직 맞춤 추천을 만들 정보가 부족해요`와 `/pets` 또는 전체 상품 CTA. 발견 rail이 비면 해당 섹션만 숨기되 신뢰 strip은 유지한다. |
...
| responsive | ≥1200 12열, rail 5개; 768–1199 3개; <768 2개; 320px 1개. hero는 <768 단일 열이며 시각 요소는 copy 뒤에 둔다. |
| accessibility | carousel은 자동 재생하지 않거나 명시적 pause를 제공한다. 섹션 제목 구조 `h1` 하나, `h2` 순차. 추천 근거는 이미지 alt가 아닌 본문으로 제공한다. |
| gap·impact | 홈 section 순서와 중복 제거, 독립 async boundary, 추천 fallback, rail 제어를 조정한다. 기존 API 안에서 가능하다. |
...
| 레퍼런스 | Kurly Pet PLP의 좌측 필터·결과 수·정렬·URL 파라미터·밀도는 `CONFIRMED/ADAPT`. 고정 4열은 `REJECT`; PawCycle의 더 넓은 탐색 셸에 맞춰 5/4열을 쓴다. Olive Young PLP는 접근 차단으로 `UNVERIFIED`. |
...
| responsive | 1440+: 228px rail+5열; 1200–1439: 216px+4열; 900–1199: drawer+3열; 360–899: drawer+2열; 320–359/200%: 1열. toolbar는 모바일에서 filter/sort 2분할 sticky row가 될 수 있다. |
...
| 레퍼런스 | Kurly/Olive Young의 가격 계층, 할인·배송·best 배지는 `CONFIRMED/ADAPT`; 의미가 중복된 다수 배지는 `REJECT`. |
...
| responsive | 5/4열에서는 actions를 한 줄, 2열에서는 비교를 보조 행, 1열에서는 이미지 112px+정보 행 카드로 전환할 수 있다. CTA 높이는 44px 유지. |
...
| 최종 IA | breadcrumb→gallery/purchase summary→anchor nav→상품 정보→review→Q&A→관련/보완 추천→배송·반품 신뢰. 정기배송은 주문 후 또는 `/subscriptions/new`의 승인된 경로로만 안내한다. |
...
| responsive | ≥1024 7/5열, summary sticky; 600–1023 6/6 또는 단일 열; <600 단일 열, 하단 `가격+담기` sticky bar. sticky bar는 keyboard와 safe area를 침범하지 않는다. |
...
| 레퍼런스 | 외부 비교 화면은 직접 검증하지 못했으므로 `UNVERIFIED`; 일반 관행을 근거처럼 쓰지 않고 PawCycle API 계약으로 설계한다. |
...
| interaction | 최대 3개. 제거 후 1개가 되면 표를 숨기고 추가 선택 CTA. `차이만 보기`는 URL 또는 현재 페이지 상태로 유지하되 상품 ID 외 개인 데이터는 저장하지 않는다. |
...
| responsive | ≥900 semantic table. <900 상품 선택 tabs + 속성별 2열 비교; 3번째는 tab 전환. 가로 drag만 요구하지 않는다. 200% 확대에서도 핵심 label 고정. |
...
| 레퍼런스 | 외부 위시리스트 계정 화면은 직접 확인하지 않아 `UNVERIFIED`; Product Card의 검증된 action 계층만 재사용한다. |
...
| responsive | desktop 4–5열, tablet 3열, mobile 2열, 320px 1열 행. mobile action은 항상 보인다. |
