...
| 레퍼런스 | Petco·PetSmart·Pet Valu의 다음 날짜·skip·주기·수량·결제·취소 관리는 공식 설명으로 `INDIRECT/ADAPT`; 실제 계정 UI는 `UNVERIFIED`다. |
...
| navigation | detail route. legacy 별칭으로 들어오면 기능·focus가 동일해야 하며 current route 전환은 history를 불필요하게 두 번 쌓지 않는다. |
...
| responsive | D8 SSOT: 1024px 이상 main 8+summary 4, 1023px 이하 single. mobile 날짜·주기 편집은 480px 이하 중앙 dialog, 주소·결제처럼 field가 많은 편집은 full-screen dialog다. sticky CTA는 열린 편집에만 하나다. |
...
| gap·impact | 현재 통합 변경 UI를 날짜/주기 command별로 분리하고 ETag conflict·pending·issue 표현을 제안 계약에 명시한다. API 변경 없음. |
...
| 최종 IA | source 상품/주문 확인→구독 상품·수량→2/4/8주 선택→첫/다음 배송일→배송지→결제수단→최종 summary/생성의 한 페이지 progressive section이다. 완료 section은 read-only summary로 접고 현재 section 하나만 편집하며 URL에 미완료 민감 데이터를 저장하지 않는다. |
...
| navigation | Recommended Default 주 진입점은 주문 상세의 `정기배송으로 다시 받기`다. cancel은 source order로 복귀하고 direct `/subscriptions/new` 진입은 list로 복귀한다. success는 detail이다. PO 승인 전에는 구현 기준이 아니다. |
...
| responsive | D8 SSOT: 1024px 이상 8열 form+4열 summary, 1023px 이하 단일 열 progressive section이다. mobile은 accordion 대신 완료 summary+현재 section을 유지하고 CTA는 52px다. |
...
| responsive | D8 SSOT: 1024px 이상 8열 next-delivery+4열 recent-order, 1023px 이하 single list. 관리 link는 전 범위 44px 행이다. |
...
| interaction | 행 링크가 있을 때만 목적지로 이동한다. 읽음 처리는 서버 성공 후 반영하며 optimistic 처리는 사용하지 않는다. 전체 읽음은 API가 있을 때만 노출한다. |
...
| 최종 IA | heading+추가→기본 주소→기타 주소 행. desktop 편집은 760px modal, 767px 이하 편집은 full-screen dialog만 사용하며 inline form은 사용하지 않는다. |
...
| accessibility | 주소는 `<address>`로 묶고 dialog focus, 기본 상태 text, autocomplete 속성을 제공한다. |
...
| navigation | checkout/subscription returnTo는 GET만 전달한다. provider 취소는 저장된 sanitized 호출 GET 화면으로 복귀하고 호출 화면이 없으면 `/billing-methods`로 이동한다. |
