...
D1-A~D6-A의 추천 범위는 `Approved by PERF-002`다. Container sampling 방식과 warm-up 5회의 적용 단위는 `Decision Required`이며, 이 때문에 실제 기준선 측정은 `Blocked Pending Detail Approval`이다. 측정 결과, 성능 목표, 병목 가설, 최적화와 확장 도구도 승인되지 않았다.
