...
이 문서가 D1-A~D6-A 승인 상태 원본이다. PERF-001에 없던 container sampling의 구체 방식과 warm-up 적용 단위는 PERF-003에서 추가 승인됐으며 세부 승인 원본은 `docs/performance/PERF-003-local-baseline-detail-approval.md`다. 실제 측정은 PERF-003 인수인계를 입력으로 사용하는 별도 Platform/SRE 작업에서만 수행한다. SLO, 목표값, 회귀 threshold, 고부하, 신규 도구 또는 제품·실행 설정 변경은 승인하지 않는다.
...
Container 자원 SLI candidate와 event-based sampling의 구체 시점·횟수·지표별 집계·optional 규칙은 `Approved by PERF-003`다. 각 warm 측정 단위의 `before`, `mid`, `after` 3회와 상세 집계 규칙은 `docs/performance/PERF-003-local-baseline-detail-approval.md`를 따른다.
...
| warm-up | `Approved by PERF-003`: 각 승인된 읽기 route의 측정 cohort 직전 순차 5회, 모든 측정 집계에서 제외 |
...
- container sampling: event-based, 각 warm 측정 단위의 `before`·`mid`·`after` 3회와 지표별 집계·optional/null 규칙 승인
- warm-up 5회: 승인된 각 읽기 route별 측정 cohort 직전 순차 5회 승인

두 항목은 `Approved by PERF-003`다. 실제 기준선 측정의 세부 결정 차단은 해소됐으며 Platform/SRE는 PERF-003 인수인계를 입력으로 별도 작업에서 일회성 측정을 시작할 수 있다.

## 승인된 범위의 변경 불가 조건

D1-A~D6-A의 승인된 cohort, 측정 반복 횟수, concurrency, reset·seed, 통계 계산, 실패 처리와 보존 정책을 임의로 변경하지 않는다. PERF-003에서 승인된 container sampling 방식과 warm-up 적용 단위도 변경 불가 조건이다.
...
- PERF-003에서 승인된 container sampling 방식 또는 warm-up 적용 단위를 변경해야 함
- 결과 개선을 위해 표본을 제거·대체하거나 조건을 변경해야 함
- SLO·threshold·병목 또는 최적화를 함께 결정해야 함

## 승인 결과

D1-A~D6-A의 추천 범위는 `Approved by PERF-002`이고 container sampling 방식과 warm-up 5회의 적용 단위는 `Approved by PERF-003`다. 실제 기준선 측정은 `Open for One-time Local Baseline Measurement`다. 측정 결과, 성능 목표, 병목 가설, 최적화와 확장 도구는 승인되지 않았다.
