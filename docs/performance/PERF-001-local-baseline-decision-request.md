...
PERF-002에서 D1-A~D6-A의 추천 범위를 사용자 승인 입력으로 확정했고, PERF-003에서 container sampling 방식과 warm-up 5회의 적용 단위를 추가 승인했다. 승인 상태 원본은 `docs/performance/PERF-002-local-baseline-approved-inputs.md`, 세부 승인 원본은 `docs/performance/PERF-003-local-baseline-detail-approval.md`다. 실제 측정은 PERF-003 인수인계를 입력으로 사용하는 별도 Platform/SRE 작업에서만 수행한다.
...
- D1-A 승인
- QA 구독 reset·반복 생성 경계 승인
- browser rendering 제외 승인
- D1-B/C/D 미승인
...
- Frontend·proxy 경계, 공개·인증된 읽기와 인증 lifecycle의 client elapsed milliseconds: median(p50), p95, maximum
- 표본 10개의 구독 상태 변경 client elapsed milliseconds: median(p50), maximum
- method·정규화 route·status별 요청 수
- 전체·cohort별 endpoint 기대 status 불일치(`unexpected_status`) 수와 오류 비율
...
HTTP redirect가 승인 흐름에 포함되면 endpoint별 기대 status와 비교한다. 기대한 3xx는 오류에서 제외하고, 기대하지 않은 3xx를 포함한 모든 기대 status 불일치를 `unexpected_status` 오류로 집계한다. p50은 정렬된 표본의 중앙값을 사용하며 짝수 표본은 가운데 두 값의 산술평균으로 계산한다. 따라서 30개는 15·16번째 값, 10개는 5·6번째 값의 평균이다. p95는 nearest-rank `ceil(0.95 × n)`을 사용하고 모든 통계에 표본 수를 함께 기록한다. 경계·읽기·lifecycle 30회는 p50·p95·max를 기록하지만 상태 변경 10회의 nearest-rank p95는 max와 같으므로 독립적인 p95로 보고하지 않는다.
...
- D2-A SLI candidate·percentile·expected status mapping 승인
- container sampling의 구체 시점·횟수·간격·지표별 집계 방식은 `Approved by PERF-003`
- SLO·성능 목표는 미결정 유지
- D2-B/C/D 미승인
...
- Frontend·proxy 경계 cohort: `GET /products`를 순차 30회 실행하고 HTML 응답의 client elapsed만 별도 집계
...
Frontend·proxy 경계와 읽기·lifecycle 30회는 p50·p95·max, 상태 변경 10회는 p50·max를 기록한다. p50은 짝수 표본의 가운데 두 값 평균, p95는 nearest-rank로 고정한다. 상태 변경 p95가 필요하면 표본 수 증가를 별도 승인받는다. cold start와 warm request latency를 한 분포에 합치지 않는다. reset은 QA 회원의 구독만 대상으로 run 사이에서만 수행하고 즉시 `false`로 복원하며, iteration마다 Backend를 재기동하지 않는다.
...
- D3-A의 cold·warm-up·반복·동시성·reset·환경 통제 승인
- warm-up은 PERF-003에서 승인된 각 읽기 route별 측정 cohort 직전 순차 5회로 `Approved by PERF-003`
- 선택되지 않은 D3 대안 미승인
...
- HTTP 응답이 없는 `transport_error`와 `timeout` record의 `actual_status`는 명시적인 `null`로 두고 outcome으로 구분한다. 빈 문자열이나 `0`으로 대체하지 않는다.
- 실패 record도 동일 schema로 집계하고 루프를 중단하거나 성공 표본으로 대체하지 않는다.
- PERF-002 승인 원본은 위 필드명을 canonical schema로 유지한다. `actual_status`는 transport error·timeout에서, `response_bytes_if_available`은 확인 불가 시, `subscription_count_before_if_state_change`는 상태 변경 외 cohort에서 각각 `null`이며 raw record는 직렬화하지 않는다.
...
- D4-A allowlist·route 정규화·client elapsed·실패 처리 승인
- 선택되지 않은 D4 대안 미승인
...
- 경계·읽기·인증 lifecycle의 표본 수와 p50·p95·max, 상태 변경의 표본 수와 p50·max, status·오류 집계
- 짝수 표본 p50의 가운데 두 값 평균과 p95 nearest-rank 계산 규칙
...
- D5-A 집계 Markdown·memory-only·조건부 OS temp 정책 승인
- artifact·외부 저장소와 선택되지 않은 D5 대안 미승인
...
- D6-A 저장소 변경 없는 일회성 최소안 승인
- D6-B/C/D와 재사용 script는 미승인

## 결정 요약

| 결정 | 추천 | 대안 | 현재 상태 | 사용자 승인 필요 |
| --- | --- | --- | --- | --- |
| D1 범위 | D1-A 경계·공개 읽기·인증된 읽기·인증 lifecycle·구독 상태 변경 | 공개만, browser 포함, Full 총경과 | Approved by PERF-002 | 아니오 |
| D2 SLI 후보 | D2-A 경계·읽기·lifecycle p50·p95·max, 쓰기 p50·max, status·오류·자원·환경 | 평균만, metric 확장, 즉시 SLO | Approved by PERF-002, sampling detail by PERF-003 | 아니오 |
| D3 조건 | D3-A cold 3, warm-up 5, boundary·read·lifecycle 30, write 10, concurrency 1 | 축소안, 쓰기 표본 증가, concurrency 3, 장시간 부하 | Approved by PERF-002, warm-up detail by PERF-003 | 아니오 |
| D4 record | D4-A allowlist·정규화·client elapsed | Nginx timing, Backend log, raw 보존 | Approved by PERF-002 | 아니오 |
| D5 증거 | D5-A 집계 Markdown·raw 미커밋 | raw CSV, Actions artifact, 외부 저장 | Approved by PERF-002 | 아니오 |
| D6 도구 | D6-A 현재 PowerShell·Docker | repository script, k6, metric stack | Approved by PERF-002 | 아니오 |

## 승인 범위를 반영한 실험 절차 초안

아래 절차의 실행 세부는 PERF-003에서 승인됐다. 실제 측정은 PERF-003 인수인계를 입력으로 사용하는 별도 Platform/SRE 작업에서만 실행한다.
...
2. `GET /products`를 순차 warm-up 5회 실행하고 집계에서 제외한 직후, Frontend·proxy 경계 cohort를 concurrency 1로 30회 측정해 공개 읽기와 별도 집계한다.
3. `GET /api/products`를 순차 warm-up 5회 실행하고 집계에서 제외한 직후, 해당 공개 읽기 route를 concurrency 1로 30회 측정한다.
4. `GET /api/products/{productId}`를 순차 warm-up 5회 실행하고 집계에서 제외한 직후, 해당 공개 읽기 route를 concurrency 1로 30회 측정한다.
5. `GET /api/subscriptions`의 각 warm-up·측정 iteration에 승인된 session·CSRF·로그인 setup과 로그아웃 cleanup 경계를 적용한다. 순차 warm-up 5회와 setup·cleanup 시간을 집계에서 제외한 직후, 해당 인증된 읽기 route만 30회 측정한다.
6. `GET /api/subscriptions/{subscriptionId}`의 각 warm-up·측정 iteration에도 같은 인증 setup·cleanup 경계를 적용한다. 순차 warm-up 5회와 setup·cleanup 시간을 집계에서 제외한 직후, 해당 인증된 읽기 route만 30회 측정한다.
7. 인증 lifecycle은 별도 warm-up 없이 매 iteration 새 session → 로그인 전 CSRF → 로그인 → 로그인 후 CSRF → 인증 확인 → 로그아웃 → session 폐기 순서로 30회 실행하고 읽기 결과와 분리한다.
8. 구독 상태 변경은 별도 warm-up 없이 결과 표에서 10회 순차 실행한다. 매 iteration의 생성 전 `subscription_count_before`를 기록하고, 생성 후 목록·상세 GET은 상태 변경 후속 확인으로 분류한다.
9. 요청 직전에 Stopwatch를 시작하고 PowerShell 5.1 호환 `try/catch/finally`로 성공, HTTP error, transport error와 timeout을 동일 record schema에 남긴다.
10. 각 warm 측정 단위에서 첫 iteration 직전 `before`, 30회 중 15번째 iteration 직후 `mid`, 마지막 iteration 직후 `after`의 event-based container 표본을 수집한다. 별도 warm-up이 없는 10회 구독 상태 변경 cohort에도 첫 iteration 직전 `before`, 5번째 iteration 직후 `mid`, 마지막 iteration 직후 `after`를 동일하게 수집하고 PERF-003의 지표별 집계·nullable 규칙을 적용한다.
11. 실패 요청을 삭제하거나 재실행해 대체하지 않는다.

### 5. 집계와 안전 확인

1. 경계·읽기·인증 lifecycle 30회는 endpoint·cohort별 p50·p95·max를, 상태 변경 10회는 p50·max를 기대 status 불일치 수·오류 비율과 함께 계산한다.
2. p50은 정렬된 짝수 표본의 가운데 두 값 평균, p95는 nearest-rank `ceil(0.95 × n)`으로 계산하고 표본 수를 함께 기록한다.
3. 상태 변경은 iteration과 `subscription_count_before`를 함께 보고하고 증가하는 목록 결과를 고정 cardinality 읽기 기준선으로 표현하지 않는다.
4. container 자원은 PERF-003의 event-based sampling, 누적 counter 차이, PIDs nullable과 restart count 규칙에 따라 집계하고 실제 사용한 조건을 기록한다.
5. 집계 전에 process memory의 seed `subscriptionId`를 폐기하고, 보존할 record·log·문서에 credential, header, body, cookie, session ID, CSRF token, 원시 log와 실제 데이터 ID가 없는지 확인한다.
6. 단일 snapshot이나 smoke 총경과를 endpoint latency로 오표기하지 않는다.
7. raw record를 보존하지 않고 집계와 재현 명령만 성능 실험 문서에 기록한다. 승인된 OS temp 예외를 사용했다면 민감정보 점검과 삭제 완료도 확인한다.
...
사용자는 PERF-002에서 D1-A~D6-A의 추천 범위를 승인했고 PERF-003에서 event-based container sampling 방식과 각 읽기 route별 순차 warm-up 5회를 추가 승인했다. 실제 기준선 측정의 실행 게이트는 해소됐으며 다음 Platform/SRE는 PERF-003 인수인계를 입력으로 일회성 로컬 기준선을 측정할 수 있다. 선택되지 않은 모든 대안은 `Deferred` 또는 미승인 상태이며, SLO·threshold·고부하·신규 도구·측정 script 구현·최적화에는 별도 승인이 필요하다.
