...
- 작업 ID: `PERF-001`
- 역할: Platform/SRE
- 상태: `Partially Resolved by PERF-002`
...
PERF-002에서 D1-A~D6-A의 추천 범위를 사용자 승인 입력으로 확정했다. 승인 상태 원본은 `docs/performance/PERF-002-local-baseline-approved-inputs.md`다. PERF-001에 없던 container sampling의 구체 방식과 warm-up 5회의 endpoint·cohort별 적용 단위는 승인되지 않았으므로 실제 측정은 두 세부 결정 전까지 시작하지 않는다.
...
`Approved Scope / Container Sampling Decision Required by PERF-002`
...
- container sampling의 구체 시점·횟수·간격·지표별 집계 방식은 미승인
- SLO·성능 목표는 미결정 유지
- D2-B/C/D 미승인
...
`Approved Scope / Warm-up Unit Decision Required by PERF-002`
...
- warm-up 5회를 전체 5회, endpoint별 5회 또는 cohort별 5회 중 어디에 적용할지는 미승인
- 선택되지 않은 D3 대안 미승인
...
| D2 SLI 후보 | D2-A 경계·읽기·lifecycle p50·p95·max, 쓰기 p50·max, status·오류·자원·환경 | 평균만, metric 확장, 즉시 SLO | Approved Scope / Container Sampling Decision Required | 예 |
| D3 조건 | D3-A cold 3, warm-up 5, boundary·read·lifecycle 30, write 10, concurrency 1 | 축소안, 쓰기 표본 증가, concurrency 3, 장시간 부하 | Approved Scope / Warm-up Unit Decision Required | 예 |
...
## 승인 범위와 미결정 세부를 반영한 실험 절차 초안

아래 절차는 container sampling 방식과 warm-up 적용 단위가 승인되기 전에는 실행할 수 없다.
...
2. warm-up은 측정 제외 5회라는 범위만 승인됐다. 전체·endpoint별·cohort별 적용 단위가 결정될 때까지 실행하지 않는다.
...
9. container stats의 구체 sampling 시점·횟수·간격과 지표별 집계 방식이 결정될 때까지 자원 기준선 측정을 실행하지 않는다.
...
4. container 자원은 사용자 승인 후 확정된 sampling 방식·지표별 집계 규칙과 실제 사용한 조건을 기록한다. 승인 전에는 구체 sampling 결과를 만들지 않는다.
...
사용자는 PERF-002에서 D1-A~D6-A의 추천 범위를 승인했다. 다만 container sampling의 구체 방식과 warm-up 5회의 적용 단위는 승인 범위에 포함되지 않았다. 두 항목을 사용자/Tech Lead가 추가 승인하기 전에는 다음 Platform/SRE가 실제 기준선을 측정할 수 없다. 선택되지 않은 모든 대안은 `Deferred` 또는 미승인 상태이며, 실제 부하 실행·측정 script 구현·최적화에도 별도 승인이 필요하다.
