...
PERF-002에서 D1-A~D6-A를 사용자 승인 입력으로 확정했다. 승인된 단일 원본은 `docs/performance/PERF-002-local-baseline-approved-inputs.md`이며, 이 문서의 선택되지 않은 대안과 SLO·성능 목표·확장 도구는 승인되지 않았다.
...
| D2 SLI 후보 | D2-A 경계·읽기·lifecycle p50·p95·max, 쓰기 p50·max, status·오류·자원·환경 | 평균만, metric 확장, 즉시 SLO | Approved by PERF-002 | 아니오 |
| D3 조건 | D3-A cold 3, warm-up 5, boundary·read·lifecycle 30, write 10, concurrency 1 | 축소안, 쓰기 표본 증가, concurrency 3, 장시간 부하 | Approved by PERF-002 | 아니오 |
...
2. 승인된 warm-up 횟수만큼 Frontend·proxy 경계와 공개·인증된 읽기를 실행하되 결과에 포함하지 않는다.
...
9. container stats는 각 warm endpoint 또는 lifecycle cohort의 첫 요청 전·중간 iteration 직후·마지막 iteration 직후에 event-based로 3회 수집하고 관측점·`timestamp_utc`와 집계값만 남긴다.
...
4. container 세 표본은 지표별로 집계한다. CPU percentage와 memory usage bytes는 평균·최대, network RX/TX와 block read/write는 누적 byte 값의 평균·최대, PIDs는 제공된 정수 표본의 평균·최대와 가용 표본 수를 기록한다. Windows에서 PIDs가 제공되지 않으면 `null`로 유지하고 `0`으로 대체하지 않는다. 누적 network·block I/O 값은 처리량으로 해석하지 않으며 시작 전·중간·종료 event-based sampling schedule을 함께 기록한다.
...
사용자는 PERF-002에서 D1-A~D6-A를 모두 승인했다. 다음 Platform/SRE 작업은 `docs/performance/PERF-002-local-baseline-approved-inputs.md`의 고정 조건으로 저장소 변경 없는 일회성 기준선만 측정한다. 선택되지 않은 모든 대안은 `Deferred` 또는 미승인 상태이며, 실제 부하 실행·측정 script 구현·최적화에는 별도 승인이 필요하다.
