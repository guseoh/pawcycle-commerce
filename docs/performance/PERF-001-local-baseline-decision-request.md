...
2. `GET /products`, `GET /api/products`, `GET /api/products/{productId}`, `GET /api/subscriptions`, `GET /api/subscriptions/{subscriptionId}`의 각 route 측정 cohort 직전에 순차 warm-up 5회를 수행하고 모든 측정 집계에서 제외한다. 인증 lifecycle과 구독 상태 변경에는 별도 warm-up을 적용하지 않는다.
3. Frontend·proxy 경계 `GET /products`를 concurrency 1로 30회 실행하고 공개 읽기와 별도 집계한다.
4. 공개 읽기를 concurrency 1로 30회 실행한다.
5. 인증된 읽기는 iteration마다 별도 session·CSRF·로그인 setup을 완료한 뒤 seed ID의 목록·상세 GET만 읽기 latency로 30회 측정하고, 로그아웃 cleanup과 setup 시간은 별도 기록한다.
6. 인증 lifecycle은 매 iteration 새 session → 로그인 전 CSRF → 로그인 → 로그인 후 CSRF → 인증 확인 → 로그아웃 → session 폐기 순서로 30회 실행하고 읽기 결과와 분리한다.
7. 구독 상태 변경은 별도 결과 표에서 10회 순차 실행한다. 매 iteration의 생성 전 `subscription_count_before`를 기록하고, 생성 후 목록·상세 GET은 상태 변경 후속 확인으로 분류한다.
8. 요청 직전에 Stopwatch를 시작하고 PowerShell 5.1 호환 `try/catch/finally`로 성공, HTTP error, transport error와 timeout을 동일 record schema에 남긴다.
9. 각 warm 측정 단위에서 첫 iteration 직전 `before`, 30회 중 15번째 또는 10회 중 5번째 iteration 직후 `mid`, 마지막 iteration 직후 `after`의 event-based container 표본을 수집하고 PERF-003의 지표별 집계·nullable 규칙을 적용한다.
10. 실패 요청을 삭제하거나 재실행해 대체하지 않는다.
