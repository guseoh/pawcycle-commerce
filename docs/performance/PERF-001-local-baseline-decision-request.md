...
1. 경계·읽기·인증 lifecycle 30회는 endpoint·cohort별 p50·p95·max를, 상태 변경 10회는 p50·max를 status·오류 비율과 함께 계산한다.
...
5. credential, header, body, cookie, session ID, CSRF token, 원시 log와 실제 데이터 ID가 없는지 확인한다.
