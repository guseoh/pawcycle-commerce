...
7. 사전검증 직후이자 첫 QA reset 직전에 아티팩트 SHA-256이 검증한 값과 같은지 다시 확인한다.
...
10. 식별자·SHA-256 불일치, 래퍼 검증 실패, 아티팩트 재현 실패, dirty worktree, `HEAD`·`origin/main` 변경 또는 고정 기준 commit 불일치가 있으면 endpoint 호출, QA reset·seed, cold start와 warm 측정을 시작하지 않는다.
...
- 래퍼 사전검증 직후 Git 재검사에서 dirty worktree, `HEAD` 변경, `origin/main` 변경 또는 고정 기준 commit 불일치가 확인되면 endpoint 호출과 첫 QA reset 전에 중단한다.
