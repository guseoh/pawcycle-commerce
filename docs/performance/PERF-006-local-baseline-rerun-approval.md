...
사용자는 `B. Cold와 warm 전체 재실행`을 승인한다. PERF-007은 필수 게이트 뒤 reset 준비 기동을 거쳐 QA reset부터 cold와 warm 전체를 하나의 새 run으로 수행한다. 선택지 A의 부분 결과 연속안과 선택지 C의 측정 종료안은 선택하지 않는다.
...
5. 실제 재실행용 수정 래퍼 아티팩트를 재현하고 로컬 경로 또는 이름, 적용 가능한 version·commit과 SHA-256을 기록한다.
6. 같은 식별자의 아티팩트로 request parameter 구성과 container stats parsing을 상태 변경 없는 로컬 입력으로 검증하고 검증 명령·비민감 입력 설명과 결과를 기록한다.
7. 사전검증 직후이자 reset 준비 기동 직전에 아티팩트 SHA-256이 검증한 값과 같은지 다시 확인한다.
8. 같은 시점에 `git status --porcelain` 출력이 비어 있는지 다시 확인하고, `git rev-parse HEAD`와 `git rev-parse origin/main`의 실제 SHA가 모두 1단계에서 고정한 기준 commit과 같은지 다시 비교한다.
9. 최초 Git 검사와 사전검증 이후 Git 재검사 결과를 모두 PERF-007 결과에 기록한다.
10. 식별자·SHA-256 불일치, 래퍼 검증 실패, 아티팩트 재현 실패, dirty worktree, `HEAD`·`origin/main` 변경 또는 고정 기준 commit 불일치가 있으면 Docker 준비 기동, endpoint 호출, QA reset·seed, cold start와 warm 측정을 시작하지 않는다.
...
사전검증 이후 Git 재검사를 포함한 필수 게이트 통과 뒤 PERF-007은 다음 순서를 한 번만 실행한다.

1. PERF-004 종료 상태인 volume 보존 `down`을 확인한다.
2. Volume을 삭제하지 않고 승인된 Compose `up --detach --wait --wait-timeout 180`을 실행한다.
3. 네 service가 모두 `healthy`인지 확인한다.
4. 이 최초 기동을 reset 준비로만 기록하고 cold start 측정 표본과 PERF-002·003의 반복 횟수·통계에서 제외한다.
5. QA 회원 구독을 run 시작 상태로 한 번 reset한다.
6. reset 설정을 즉시 `false`로 복원한다.
7. 측정 집계에서 제외되는 seed 구독 1건을 생성하고 목록·상세로 확인한다.
8. Volume을 삭제하지 않는 `down`과 승인된 `up --detach --wait --wait-timeout 180`으로 cold start를 3회 수행한다.
9. 세 번째 cold start 뒤 네 service의 `healthy`를 확인한다.
10. Warm 측정 직전 측정 집계에서 제외되는 30초 안정화를 수행한다.
11. 다섯 읽기 route 각각에 cohort 직전 warm-up 5회를 적용한 뒤 승인된 읽기 측정을 수행한다.
12. 별도 warm-up 없이 인증 lifecycle 측정을 수행한다.
13. 별도 warm-up 없이 구독 상태 변경 측정을 수행한다.
...
- 래퍼 사전검증 직후 Git 재검사에서 dirty worktree, `HEAD` 변경, `origin/main` 변경 또는 고정 기준 commit 불일치가 확인되면 Docker 준비 기동 전에 중단한다.
- Reset 준비 기동 또는 네 service의 `healthy` 확인이 실패하면 QA reset·seed, cold start와 warm 측정을 시작하지 않는다.
...
- 래퍼 아티팩트와 비민감 검증 입력 원문은 저장소에 추가하지 않고 식별자·명령·입력 설명·검증 결과만 PERF-007 집계 문서에 기록한다.
...
- Volume 보존 reset 준비 기동 또는 네 service의 `healthy` 확인에 실패함
