  ...
  return <section>
    ...
    <div className="cart-layout">
      ...
      <aside className="section-card cart-summary" aria-labelledby="cart-summary-title">
        ...
        {items?.length && unavailableItems.length === 0 ? <Link className="button button-primary" href="/checkout">장바구니 전체 주문하기</Link> : <Link className="button button-secondary" href="/products">상품 둘러보기</Link>}
      </aside>
    </div>
    {items?.length && unavailableItems.length === 0 && pricing ? <div className="mobile-transaction-bar"><strong>총 {items.reduce((total, item) => total + item.quantity, 0)}개 · {formatPrice(pricing.paymentAmount)}</strong><Link className="button button-primary" href="/checkout">전체 주문하기</Link></div> : null}
  </section>;
}
