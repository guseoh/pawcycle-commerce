    ...
    <div className="cart-layout">
      <section className="section-card cart-items-panel" aria-labelledby="cart-items-title">
        ...
        {items?.length ? <ul className="cart-item-list">{items.map((item) => <li className="cart-item" key={item.skuId}>
          ...
          <div className="cart-item-main"><Link href={`/products/${item.productId}`}><strong>{item.productName}</strong></Link><span className="cart-sku">{item.skuName} · {item.skuCode}</span><strong className="cart-price">{formatPrice(item.price)}</strong></div>
