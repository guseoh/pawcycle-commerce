...
export default function Home() {
  ...
  const [items, setItems] = useState<RecommendationItem[] | null>(null); const [error, setError] = useState<string | null>(null); const [petError, setPetError] = useState<string | null>(null); const [petRetry, setPetRetry] = useState(0);
  useEffect(() => { if (auth.status === "authenticated") void v2Api.pets.list().then(({ body }) => { setPets(body.items); setPetError(null); }).catch(() => setPetError("반려동물 목록을 불러오지 못했습니다.")); }, [auth.status, petRetry]);
