...
function PersonalizedRecommendations() {
  ...
  return <section className="section-card"><h2>반려동물 상품 추천</h2>{petError ? <ErrorState title="반려동물 목록을 불러오지 못했습니다." message={petError} onRetry={() => { setPets(null); setPetError(null); setPetRetry((value) => value + 1); }} /> : pets === null ? <LoadingState>반려동물 목록을 불러오고 있습니다.</LoadingState> : pets.length === 0 ? <p>등록된 반려동물이 없습니다.</p> : <><label className="form-field">반려동물 선택<select className="input" value={petId} onChange={(e) => { recommendationRequest.current += 1; setPetId(e.target.value); setItems(null); setError(null); }}><option value="">선택하세요</option>{pets.map((pet) => <option value={pet.petId} key={pet.petId}>{pet.name} · {pet.petType}</option>)}</select></label><button className="button button-primary" type="button" disabled={!petId} onClick={() => void load()}>추천 보기</button></>}{error ? <ErrorState title="추천을 불러오지 못했습니다." message={error} onRetry={() => void load()} /> : null}{items?.length === 0 ? <p>현재 추천 가능한 상품이 없습니다.</p> : items ? <ul className="history-list">{items.map((item) => <li key={item.productId}><Link href={`/products/${item.productId}`}><strong>{item.name}</strong></Link><span>{item.category.name} · {item.reason}</span></li>)}</ul> : null}</section>;
}
