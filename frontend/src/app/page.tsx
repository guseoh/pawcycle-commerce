...
import { ApiError, ProductSummary, productApi, recommendationApi, type RecommendationItem, type ProductSort } from "@/lib/api";
...
export default function Home() {
  ...
  return (
    <div className="home-stack shopping-home">
      ...
      <HomeProductPreview id="popular" title="많이 이야기하는 상품" description="다른 반려가족의 리뷰가 쌓인 상품을 만나보세요." sort="REVIEW_COUNT" />
      <SubscriptionValue />
  ...
  return (
    <section className="recommendation-section" aria-labelledby="recommendation-title">
      ...
      {petError ? null : pets === null ? <LoadingState>반려동물 목록을 불러오고 있습니다.</LoadingState> : pets.length === 0 ? <div className="empty-callout"><strong>등록된 반려동물이 없습니다.</strong><Link href="/subscriptions/new">반려동물 등록하고 정기배송 시작하기</Link></div> : <>
        ...
        {error ? <ErrorState headingLevel={3} title="추천을 불러오지 못했습니다." message={error} onRetry={() => void load()} /> : null}
