...
function AddressesForMember({ returnTo }: { returnTo: string | null }) {
  ...
  const load = useCallback(() => {
    ...
    void addressApi.list().then((result) => {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await auth.executeWithCsrf(async (csrf) => {
        if (editId === null) await addressApi.create(form, csrf);
        else await addressApi.update(editId, form, csrf);
      });
