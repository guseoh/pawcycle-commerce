...
export default function CheckoutPage() {
  ...
  const load = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      try {
        ...
        setCoupons([]);
        setCouponId(null);
        setCouponError("사용 가능한 쿠폰을 확인하지 못했습니다. 잠시 후 다시 시도해 주세요.");
      }
      return true;
    } catch (reason) {
      setCart(null);
      ...
      setError("주문 정보를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
      return false;
    }
  }, [auth]);
  ...
  async function checkout() {
    ...
    try {
      ...
      if (reason instanceof ApiError && reason.code === "CART_CHANGED") {
        ...
        setError("주문 결과를 확인하지 못했습니다. 같은 요청으로 다시 시도해 주세요.");
  ...
  if (result) {
    ...
    return <section className="checkout-success section-card"><p className="eyebrow">결제 단계</p><h1>주문 준비가 완료되었습니다.</h1><p>{result.orderName} · 주문 번호 {result.orderNumber}</p><p className="field-help">아래에서 결제를 진행하거나 주문 상세에서 현재 상태를 확인할 수 있어요.</p><PriceSummary pricing={confirmedPricing} couponSelected={Boolean(couponId)} /><TossPaymentWidget checkout={result} /><Link className="button button-secondary" href={`/orders/${result.orderId}`}>주문 상세 확인</Link></section>;
  }

  const unavailable = cart.filter((item) => !item.purchasable);
  return <section>
    <header className="page-heading"><p className="eyebrow">Checkout</p><h1>주문 확인</h1><p>상품, 배송지, 쿠폰을 확인하면 주문을 준비하고 결제 단계로 이어집니다.</p></header>
