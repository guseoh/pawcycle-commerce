...
function CommerceSnapshotPanel({ snapshot, error, onRetry }: { snapshot: CommerceSnapshot | null; error: string | null; onRetry: () => void }) {
  ...
  const nextSubscription = activeSubscriptions.find((item) => item.nextScheduledDate);
...
export default function MyPage() {
  ...
  const loadSnapshot = useCallback(async () => {
    ...
    try {
      const [orders, subscriptions, cart, notifications] = await Promise.all([commerceFinalApi.orders(), v2Api.subscriptions.list(0, 3), commerceFinalApi.cart(), commerceFinalApi.notifications()]);
      setSnapshot({ orders, subscriptions: subscriptions.body.items, cartQuantity: cart.items.reduce((total, item) => total + item.quantity, 0), unreadNotifications: notifications.filter((item) => !item.readAt).length });
