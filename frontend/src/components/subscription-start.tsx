...
import { newIdempotencyKey, subscriptionApi, type Pet, type PlanVersion } from "@/lib/subscription-api";
...
export function SubscriptionStart({ basePath = "/subscriptions" }: { basePath?: string }) {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      void subscriptionApi.plans.list(selectedPetId).then(({ body }) => { if (active) { setPlans(body.items); setPlanState("idle"); } }).catch((error: unknown) => {
  ...
  const selectPlan = useCallback(async (plan: PlanVersion, preferredCycle: number | null = null) => {
    ...
    try {
      const response = await subscriptionApi.plans.detail(plan.planVersionId, selectedPetId);
  ...
  async function registerPet(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await auth.executeWithCsrf((csrf) => subscriptionApi.pets.create({ name, petType }, csrf));
  ...
  async function createSubscription(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await auth.executeWithCsrf((csrf) => subscriptionApi.subscriptions.create({ petId: selectedPetId, planVersionId: selectedPlan.planVersionId, deliveryCycleWeeks: cycle }, csrf, idempotencyKey));
