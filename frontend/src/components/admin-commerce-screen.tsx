...
import { adminCommerceApi, type AdminAuditLog, type AdminCoupon, type AdminInventory, type AdminMembershipGrade, type AdminOrder, type AdminCouponInput, type AdminMembershipGradeInput } from "@/lib/admin-commerce-api";
...
function PromotionsPanel({ coupons, grades, mutation, onRefresh }: { coupons: ReturnType<typeof useAdminResource<AdminCoupon[]>>; grades: ReturnType<typeof useAdminResource<AdminMembershipGrade[]>>; mutation: ReturnType<typeof useAdminMutation>; onRefresh: () => Promise<void> }) {
  return <div className="admin-commerce-columns"><section className="admin-resource"><div className="admin-section-heading"><h2>쿠폰</h2><span>{coupons.data ? `${coupons.data.length}개` : "—"}</span></div><ResourceState loading={coupons.loading} error={coupons.error} onRetry={() => void coupons.reload().catch(() => undefined)} />{coupons.data && !coupons.error ? <><CouponCreateForm pending={mutation.pending} onCreate={(input) => void mutation.run((csrf) => adminCommerceApi.createCoupon(input, csrf), onRefresh)} />{coupons.data.length ? <ul className="admin-commerce-list">{coupons.data.map((coupon) => <CouponRow key={coupon.couponId} coupon={coupon} pending={mutation.pending} onIssue={(memberId) => void mutation.run((csrf) => adminCommerceApi.issueCoupon(coupon.couponId, memberId, csrf), onRefresh)} />)}</ul> : <div className="empty-callout">등록된 쿠폰이 없습니다.</div>}</> : null}</section><section className="admin-resource"><div className="admin-section-heading"><h2>멤버십 등급</h2><span>{grades.data ? `${grades.data.length}개` : "—"}</span></div><ResourceState loading={grades.loading} error={grades.error} onRetry={() => void grades.reload().catch(() => undefined)} />{grades.data && !grades.error ? <><GradeCreateForm pending={mutation.pending} onCreate={(input) => void mutation.run((csrf) => adminCommerceApi.createMembershipGrade(input, csrf), onRefresh)} />{grades.data.length ? <ul className="admin-commerce-list">{grades.data.map((grade) => <li className="admin-commerce-row" key={grade.gradeId}><div><strong>{grade.name} · {grade.code}</strong><span>기준 {formatPrice(grade.minimumPurchaseAmount)} · 순서 {grade.displayOrder} · {grade.active ? "활성" : "비활성"}</span><span>혜택 쿠폰 {grade.benefitCouponId ?? "없음"}</span></div><EvaluateMembership pending={mutation.pending} onEvaluate={(memberId) => void mutation.run((csrf) => adminCommerceApi.evaluateMembership(memberId, csrf), onRefresh)} /></li>)}</ul> : <div className="empty-callout">등록된 멤버십 등급이 없습니다.</div>}</> : null}</section></div>;
}

function CouponCreateForm({ pending, onCreate }: { pending: boolean; onCreate: (input: Record<string, unknown>) => void }) {
  const [form, setForm] = useState(EMPTY_COUPON);
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!form.name.trim() || !form.discountValue || !form.validFrom || !form.validUntil) return; onCreate({ name: form.name.trim(), discountType: form.discountType, discountValue: Number(form.discountValue), minimumOrderAmount: Number(form.minimumOrderAmount || 0), maximumDiscountAmount: form.maximumDiscountAmount ? Number(form.maximumDiscountAmount) : null, validFrom: form.validFrom.replace("T", " "), validUntil: form.validUntil.replace("T", " "), active: form.active }); setForm(EMPTY_COUPON); };
  const change = (key: keyof AdminCouponInput, value: string | boolean) => setForm((current) => ({ ...current, [key]: value }));
  return <form className="admin-form" onSubmit={submit}><h3>쿠폰 생성</h3><div className="admin-fields"><label className="form-field">이름<input className="input" required value={form.name} onChange={(event) => change("name", event.target.value)} disabled={pending} /></label><label className="form-field">할인 방식<select className="input" value={form.discountType} onChange={(event) => change("discountType", event.target.value as AdminCouponInput["discountType"])} disabled={pending}><option value="FIXED_AMOUNT">정액</option><option value="PERCENTAGE">정률</option></select></label><label className="form-field">할인 값<input className="input" required type="number" min="0" step="0.01" value={form.discountValue} onChange={(event) => change("discountValue", event.target.value)} disabled={pending} /></label><label className="form-field">최소 주문 금액<input className="input" type="number" min="0" step="0.01" value={form.minimumOrderAmount} onChange={(event) => change("minimumOrderAmount", event.target.value)} disabled={pending} /></label><label className="form-field">최대 할인 금액<input className="input" type="number" min="0" step="0.01" value={form.maximumDiscountAmount} onChange={(event) => change("maximumDiscountAmount", event.target.value)} disabled={pending} placeholder="선택" /></label><label className="form-field">시작 일시<input className="input" required type="datetime-local" value={form.validFrom} onChange={(event) => change("validFrom", event.target.value)} disabled={pending} /></label><label className="form-field">종료 일시<input className="input" required type="datetime-local" value={form.validUntil} onChange={(event) => change("validUntil", event.target.value)} disabled={pending} /></label><label className="form-field"><input className="admin-checkbox" type="checkbox" checked={form.active} onChange={(event) => change("active", event.target.checked)} disabled={pending} /> 활성</label></div><button className="button button-primary" type="submit" disabled={pending}>쿠폰 생성</button></form>;
}

function CouponRow({ coupon, pending, onIssue }: { coupon: AdminCoupon; pending: boolean; onIssue: (memberId: number) => void }) {
  ...
  return <li className="admin-commerce-row"><div><strong>{coupon.name}</strong><span>#{coupon.couponId} · {coupon.discountType === "PERCENTAGE" ? `${coupon.discountValue}%` : formatPrice(coupon.discountValue)} · {coupon.active ? "활성" : "비활성"}</span><span>{coupon.validFrom} ~ {coupon.validUntil}</span></div><form className="admin-inline-form" onSubmit={issue}><label className="form-field" htmlFor={`coupon-member-${coupon.couponId}`}>회원 ID<input id={`coupon-member-${coupon.couponId}`} className="input" type="number" min="1" value={memberId} onChange={(event) => setMemberId(event.target.value)} disabled={pending} placeholder="회원 ID" /></label><button className="button button-secondary" type="submit" disabled={pending || !memberId}>쿠폰 발급</button></form></li>;
}
