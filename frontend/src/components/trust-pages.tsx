...
const trustSections = {
  shipping: { eyebrow: "배송 정책", title: "배송 안내", intro: "주문을 안심하고 기다릴 수 있도록 배송 기준을 한곳에 정리했어요.", sections: [["배송 시작", "결제와 주문 검증이 완료되면 배송 준비를 시작합니다. 배송 상태는 주문 상세와 알림에서 확인할 수 있어요."], ["배송지 확인", "주문 직전 선택한 배송지를 서버가 다시 확인합니다. 주문 생성 후에는 주문 상세의 배송지 snapshot이 기준입니다."], ["배송 지연", "재고나 배송 상태에 변동이 생기면 주문 상세와 알림에서 확인할 수 있습니다."]] },
  returns: { eyebrow: "교환·반품 정책", title: "교환·반품 안내", intro: "상품 상태와 주문 상태에 따라 가능한 절차가 달라질 수 있어요.", sections: [["반품 요청", "배송 완료 후 주문 상세에서 요청 가능 여부가 표시될 때 반품 사유를 입력해 요청할 수 있습니다."], ["주문 취소", "결제가 완료되고 배송 준비 중인 주문만 주문 상세의 취소 action을 사용할 수 있습니다."], ["처리 상태", "요청·승인·수령·환불 상태는 서버가 결정하며 주문 상세에서 최신 상태를 확인할 수 있습니다."]] },
} as const;
...
export function FaqPage() {
  const questions = [["주문 금액은 어디서 확인하나요?", "Checkout과 주문 상세에서 서버가 확정한 상품 금액, 할인, 배송비, 최종 결제 금액을 확인할 수 있습니다."], ["장바구니 가격과 재고가 달라질 수 있나요?", "장바구니는 현재 Catalog projection을 보여주며 주문 직전에 가격·주소·재고를 다시 확인합니다. 충돌이 있으면 주문을 만들지 않고 원인을 안내합니다."], ["정기배송 배송일을 바꿀 수 있나요?", "서버가 허용한 상태와 action이 표시되는 경우에만 다음 배송일을 변경할 수 있습니다."], ["반품 요청은 어디서 하나요?", "주문 상세에서 요청 가능할 때 반품 요청을 선택하고 사유를 입력해 주세요."]];
...
export function NoticePage() {
  return <section className="trust-page"><header className="page-heading"><p className="eyebrow">Notice</p><h1>공지사항</h1><p>서비스 이용에 필요한 최신 안내를 확인하세요.</p></header><section className="section-card notice-list"><article><span className="tag">서비스 안내</span><h2>주문·정기배송 상태는 서버 기준으로 표시됩니다.</h2><p>가격, 재고, 배송일과 주문 상태는 화면에서 임의로 계산하지 않고 서버가 제공한 결과를 표시합니다.</p></article><article><span className="tag">결제 안내</span><h2>실제 Toss 결제 연동은 준비 중입니다.</h2><p>이번 범위에서는 주문 준비와 상태 확인을 제공하며, Provider 브라우저 연동은 별도 승인 범위에서 진행됩니다.</p></article></section></section>;
}
