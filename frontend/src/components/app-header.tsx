...
export function AppHeader() {
  ...
  useEffect(() => {
    ...
    const resize = () => { setCategoryOpen(false); setMenuOpen(false); };
  ...
  return <header className="site-header shopping-header">
    <div className="header-primary">
      ...
      <form className="header-search" role="search" onSubmit={(event) => { event.preventDefault(); submitSearch(); }}>
        ...
        {searchDraft ? <button className="search-clear" type="button" aria-label="검색어 지우기" onClick={() => { setSearchDraft(""); document.getElementById("header-search")?.focus(); }}>×</button> : null}
