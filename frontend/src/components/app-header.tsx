...
export function AppHeader() {
  ...
  const subscriptionDiscoveryActive = pathname === "/products" && searchParams.get("subscribable") === "true";
  ...
  return <header className="site-header shopping-header">
    <div className="header-primary">
      ...
      <nav className="header-catalog" aria-label="상품 탐색"><Link href="/products" aria-current={pathname === "/products" && !subscriptionDiscoveryActive ? "page" : undefined}>상품</Link><button ref={categoryButton} type="button" aria-expanded={categoryOpen} aria-controls="category-navigation" onClick={() => { setMenuOpen(false); setSearchOpen(false); setCategoryOpen(open => !open); }}>카테고리</button><Link href="/products?subscribable=true" aria-current={subscriptionDiscoveryActive ? "page" : undefined}>정기배송 상품</Link></nav>
      <div className="header-search-shell" ref={searchShell}>
        ...
        {searchOpen ? <div id="header-search-discovery" className="search-discovery-panel" aria-label="상품 검색 탐색">
          ...
          <section aria-labelledby="search-category-title"><h2 id="search-category-title">카테고리로 찾기</h2>{discovery.state.status === "loading" ? <p className="search-discovery-status" role="status">카테고리를 불러오는 중입니다.</p> : null}{discovery.state.status === "error" ? <p className="search-discovery-status">탐색 정보를 불러오지 못했지만 검색은 계속 사용할 수 있어요.</p> : null}<div className="search-discovery-links search-discovery-category-links">{categories.slice(0, 8).map((category) => <Link key={category.categoryId} href={`/products?category=${encodeURIComponent(category.slug)}`} onClick={() => setSearchOpen(false)}>{category.name}</Link>)}</div></section>
          {brands.length ? <section aria-labelledby="search-brand-title"><h2 id="search-brand-title">브랜드로 찾기</h2><div className="search-discovery-links search-discovery-brand-links">{brands.slice(0, 8).map((brand) => <Link key={brand.brandId} href={`/products?brand=${encodeURIComponent(brand.slug)}`} onClick={() => setSearchOpen(false)}>{brand.name}</Link>)}</div></section> : null}
        </div> : null}
      </div>
