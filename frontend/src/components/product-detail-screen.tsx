...
import { buildLoginHref, formatPetType, formatPrice, isInternalDemoLabel, notifyCommerceChanged, rememberRecentProduct, type RecentProduct } from "@/lib/frontend-utils";
...
export function ProductDetailScreen({ productId }: { productId: string }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      void productApi.detail(productId).then((product) => {
        if (!active) return;
        if (isInternalDemoLabel(product.name) || isInternalDemoLabel(product.category.name)) { setState({ status: "not-found" }); return; }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      void productApi.list({ category: relatedCategorySlug, petType: relatedPetType, size: 5, sort: "NEWEST" }).then((response) => {
        if (!active) return;
        setRelatedProducts((response.items ?? response.products ?? []).filter((item) => item.productId !== relatedProductId && !isInternalDemoLabel(item.name) && !isInternalDemoLabel(item.category.name)).slice(0, 4));
        setRelatedError(null);
      }).catch((error: unknown) => {
        if (!active) return;
  ...
  return <>
    ...
    <section className="product-gallery">{product!.thumbnailUrl ? <img className="product-hero-image" src={product!.thumbnailUrl} alt={product!.name} /> : <span className="image-placeholder">PawCycle</span>}</section>
    ...
    {recentProducts.length ? <section className="section-card product-context-section" aria-labelledby="recent-products-title"><div className="section-title"><h2 id="recent-products-title">최근 본 상품</h2><Link className="text-link" href="/products">상품 더 보기</Link></div><div className="mini-product-grid">{recentProducts.map((item) => <Link className="mini-product-card" href={`/products/${item.productId}`} key={item.productId}>{item.thumbnailUrl ? <img src={item.thumbnailUrl} alt={item.name} loading="lazy" /> : <span className="image-placeholder" aria-hidden="true">PawCycle</span>}<strong>{item.name}</strong>{item.price !== null ? <span>{formatPrice(item.price)}</span> : null}</Link>)}</div></section> : null}
    {relatedLoading ? <section className="section-card"><LoadingState>관련 상품을 불러오고 있습니다.</LoadingState></section> : relatedError ? <section className="section-card"><ErrorState title="관련 상품을 불러오지 못했습니다." message={relatedError} onRetry={() => setRelatedRetry((value) => value + 1)} /></section> : relatedProducts?.length ? <section className="section-card product-context-section" aria-labelledby="related-products-title"><div className="section-title"><h2 id="related-products-title">함께 살펴보기</h2><span className="field-help">같은 카테고리의 공개 상품</span></div><div className="mini-product-grid">{relatedProducts.map((item) => <Link className="mini-product-card" href={`/products/${item.productId}`} key={item.productId}>{item.thumbnailUrl ? <img src={item.thumbnailUrl} alt={item.name} loading="lazy" /> : <span className="image-placeholder" aria-hidden="true">PawCycle</span>}<strong>{item.name}</strong><span>{item.representativePrice === null ? "가격 준비 중" : formatPrice(item.representativePrice)}</span></Link>)}</div></section> : relatedProducts ? <section className="section-card"><div className="empty-callout">같은 카테고리의 다른 상품이 아직 없습니다.</div></section> : null}
  </>;
}
