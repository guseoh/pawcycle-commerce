...
export function ProductDetailScreen({ productId }:{productId:string}) {
  ...
  async function addWishlist(){if(!product||busy)return;if(auth.status==="anonymous"){location.assign(buildLoginHref(`/products/${product.productId}`));return;}setBusy(true);setMessage(null);try{await auth.executeWithCsrf(csrf=>commerceFinalApi.addWishlist(product.productId,csrf));setMessage("위시리스트에 담았습니다.");}catch(error){setMessage(error instanceof ApiError?error.message:"위시리스트에 담지 못했습니다.");}finally{setBusy(false);}}
  async function addCart(){if(!product||!cartSkuId||busy){setMessage("일반 구매 옵션을 선택해 주세요.");return;}if(auth.status==="anonymous"){location.assign(buildLoginHref(`/products/${product.productId}`));return;}const parsed=Number(quantity);if(!Number.isInteger(parsed)||parsed<1){setMessage("수량을 확인해 주세요.");return;}setBusy(true);setMessage(null);try{await auth.executeWithCsrf(csrf=>commerceFinalApi.addCart(cartSkuId,parsed,csrf));setMessage("장바구니에 담았습니다.");}catch(error){setMessage(error instanceof ApiError?error.message:"장바구니에 담지 못했습니다.");}finally{setBusy(false);}}
