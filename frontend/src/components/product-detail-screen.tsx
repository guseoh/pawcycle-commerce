...
export function ProductDetailScreen({ productId }: { productId: string }) {
  ...
  return <>
    ...
    <section className="product-purchase-zone" aria-label="상품 구매 영역">
      <div className="product-detail-layout">
        ...
        <div className="product-purchase-stack">
          <section className="product-summary"><div className="card-meta"><span className="tag">{formatPetType(product!.petType)}</span><span className="tag">{categoryName}</span></div><h1>{productName}</h1><p className="description product-summary-description">{description}</p><p className="purchase-price">{displayPrice === null ? "가격 준비 중" : formatPrice(displayPrice)}</p><p className={`product-availability ${product!.purchasable ? "is-available" : "is-unavailable"}`}>{product!.purchasable ? "구매 가능" : "현재 품절"}</p></section>
    ...
    <nav className="product-info-nav" aria-label="상품 정보 바로가기"><a href="#product-intro">상품 소개</a><a href="#product-details">상세 정보</a><a href="#product-shipping">배송 안내</a><a href="#product-returns">교환·반품</a></nav>
