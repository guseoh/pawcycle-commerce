  ...
  return <>
    ...
    <div className="product-detail-layout">
      ...
      <div className="product-purchase-stack">
        <section className="product-summary"><div className="card-meta"><span className="tag">{formatPetType(product!.petType)}</span><span className="tag">{categoryName}</span></div><h1>{productName}</h1><p className="description">{description}</p><p className={`tag ${product!.purchasable ? "tag-positive" : "tag-muted"}`}>{product!.purchasable ? "구매 가능한 상품" : "현재 품절"}</p></section>
