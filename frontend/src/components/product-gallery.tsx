...
export function ProductGallery({ product }: { product: ProductDetail }) {
  ...
  return <section className="catalog-gallery" aria-label="상품 이미지">
    ...
    {lightboxOpen ? <dialog ref={dialogRef} className="gallery-lightbox" aria-label={`${product.name} 이미지 크게 보기`} onPointerDown={(event) => { if (event.target === event.currentTarget) { const box = event.currentTarget.getBoundingClientRect(); if (event.clientX < box.left || event.clientX > box.right || event.clientY < box.top || event.clientY > box.bottom) setLightboxOpen(false); } }} onCancel={(event) => { event.preventDefault(); setLightboxOpen(false); }}>
