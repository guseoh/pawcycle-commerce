...
export function CommerceOrderDetail({ orderId }: { orderId: string }) {
  ...
  return <section className="section-card"><h1>주문 {order.orderNumber}</h1><p>주문 상태: {order.status} · 결제: {order.paymentAmount.toLocaleString()}원</p><p>배송 상태: {order.delivery?.status ?? "결제 대기"}</p>{order.delivery?.trackingNumber ? <p>운송장: {order.delivery.trackingNumber}</p> : null}<p>환불: {order.refunds.length ? order.refunds.map((refund) => `${refund.status} (${refund.attemptNo}차)`).join(", ") : "없음"}</p>{message ? <p role="alert">{message}</p> : null}<div className="button-row">{order.availableActions.includes("REQUEST_CANCELLATION") ? <button className="button button-danger" disabled={pending} onClick={() => void submit("cancel")}>주문 취소</button> : null}{order.availableActions.includes("REQUEST_RETURN") ? <button className="button button-secondary" disabled={pending} onClick={() => void submit("return")}>반품 요청</button> : null}</div>{order.refunds.some((refund) => refund.status === "UNKNOWN") ? <p>환불 상태를 확인 중입니다. 중복 환불 요청은 할 수 없습니다.</p> : null}</section>;
}
