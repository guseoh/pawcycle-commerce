...
export function Mvp2SubscriptionDetail({ subscriptionId, created, replayed, basePath = "/mvp2/subscriptions" }: { subscriptionId: string; created: boolean; replayed: boolean; basePath?: string }) {
  ...
  return <div className="detail-stack">
    ...
    {subscription.issue ? <section className="section-card issue-card"><p className="eyebrow">조치 필요</p><h2>{subscriptionIssueCopy(subscription.issue.code)}</h2><p>{subscription.issue.message}</p>{subscription.issue.code === "SHIPPING_ADDRESS_REQUIRED" && hasAction("UPDATE_SHIPPING_ADDRESS") ? <AddressForm address={address} setAddress={setAddress} saving={addressSaving} onSubmit={updateShippingAddress} /> : null}{subscription.issue.code === "BILLING_METHOD_REQUIRED" && hasAction("REGISTER_BILLING_METHOD") ? <Link className="button button-primary" href="/billing-methods">결제수단 등록 상태 확인</Link> : null}{subscription.issue.code === "PAYMENT_SUPPORT_REQUIRED" || subscription.issue.code === "STOCK_UNAVAILABLE" ? <p className="field-help">이 문제는 고객지원 또는 상품 상태 확인이 필요합니다.</p> : null}</section> : null}
...
function AddressForm({ address, setAddress, saving, onSubmit }: { address: AddressRequest; setAddress: (address: AddressRequest) => void; saving: boolean; onSubmit: () => Promise<void> }) {
  ...
  useEffect(() => { let active = true; void commerceFinalApi.addresses().then((items) => { if (active) setSavedAddresses(items); }).catch(() => { if (active) setSavedAddresses([]); }); return () => { active = false; }; }, []);
