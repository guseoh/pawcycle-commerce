...
import { buildLoginHref, formatIsoLocalDate, formatPrice, formatScheduleStatus, formatSubscriptionStatus, subscriptionIssueCopy, userFacingCatalogLabel } from "@/lib/frontend-utils";
...
export function Mvp2SubscriptionDetail({ subscriptionId, created, replayed, basePath = "/mvp2/subscriptions" }: { subscriptionId: string; created: boolean; replayed: boolean; basePath?: string }) {
  ...
  useEffect(() => { if (!canLoadPlans || planPetId === null) return; let active = true; void v2Api.plans.list(planPetId).then(({ body }) => { if (!active) return; setPlans(body.items); setPlansError(null); }).catch(() => { if (!active) return; setPlans(null); setPlansError("배송 선택지를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요."); }); return () => { active = false; }; }, [canLoadPlans, planPetId, plansRetryKey]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      void v2Api.subscriptions.cycleSuggestion(loadedSubscriptionId).then(({ body }) => { if (active) setCycleSuggestion(body); }).catch((error: unknown) => {
        ...
        else { setCycleSuggestion(null); setCycleSuggestionError("배송 주기 제안을 불러오지 못했습니다."); }
  ...
  if (!subscription) return <ErrorState title="구독 상세를 불러오지 못했습니다." message={message ?? "잠시 후 다시 시도해 주세요."} onRetry={reload}><Link className="button button-secondary" href={basePath}>목록으로</Link></ErrorState>;
  return <div className="detail-stack subscription-detail-layout">
    ...
    {subscription.issue ? <section className="section-card issue-card" aria-labelledby="subscription-issue-title"><p className="eyebrow">조치 필요</p><h2 id="subscription-issue-title">정기배송을 계속하려면 확인해 주세요</h2><p>{subscriptionIssueCopy(subscription.issue.code)}</p>{subscription.issue.code === "SHIPPING_ADDRESS_REQUIRED" && hasAction("UPDATE_SHIPPING_ADDRESS") ? <AddressForm address={address} setAddress={setAddress} saving={addressSaving} onSubmit={updateShippingAddress} /> : null}{subscription.issue.code === "BILLING_METHOD_REQUIRED" && hasAction("REGISTER_BILLING_METHOD") ? <Link className="button button-primary" href="/billing-methods">결제수단 등록으로 이동</Link> : null}{subscription.issue.code === "PAYMENT_SUPPORT_REQUIRED" || subscription.issue.code === "STOCK_UNAVAILABLE" ? <p className="field-help">현재 가능한 작업을 확인한 뒤 필요한 조치를 선택해 주세요.</p> : null}</section> : null}
    ...
    {subscription.pendingChange ? <section className="section-card subscription-pending-change"><h2>적용 예정 변경</h2><dl className="detail-list"><dt>적용일</dt><dd>{formatIsoLocalDate(subscription.pendingChange.appliesOn)}</dd><dt>변경 금액</dt><dd>{formatPrice(subscription.pendingChange.packagePriceKrw)}</dd><dt>변경 주기</dt><dd>{subscription.pendingChange.deliveryCycleWeeks}주마다</dd></dl><ItemList items={subscription.pendingChange.items} /></section> : null}
    <section className="section-card subscription-actions"><h2>구독 관리</h2><p>현재 가능한 작업만 표시합니다.</p><div className="button-row">{(["skip-next", "pause", "resume", "cancel"] as Command[]).filter((command) => command !== "cancel" && hasAction(command.replaceAll("-", "_").toUpperCase())).map((command) => <button key={command} className="button button-secondary" type="button" disabled={Boolean(pending)} onClick={() => void runCommand(command)}>{pending === command ? "처리 중" : LABEL[command]}</button>)}</div>{hasAction("CANCEL") ? <div className="subscription-danger-zone"><h3>정기배송 해지</h3><p>해지하면 다음 배송과 이후 정기배송 일정이 중단됩니다.</p><button className="button button-danger" type="button" disabled={Boolean(pending)} onClick={() => void runCommand("cancel")}>{pending === "cancel" ? "해지 중" : "정기배송 해지"}</button></div> : null}
