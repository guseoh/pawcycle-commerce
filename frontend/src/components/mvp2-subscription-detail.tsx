...
function ItemList({ items }: { items: { productId?: number; productName: string; skuName: string; thumbnailUrl?: string | null; quantity: number }[] }) { return <ul className="history-list subscription-item-list">{items.map((item, index) => <li key={`${item.productName}-${item.skuName}-${index}`}>{item.thumbnailUrl ? <img src={item.thumbnailUrl} alt="" /> : <span className="image-placeholder" aria-hidden="true">PawCycle</span>}<div>{item.productId ? <Link href={`/products/${item.productId}`}><strong>{item.productName}</strong></Link> : <strong>{item.productName}</strong>}<span>{item.skuName} · {item.quantity}개</span></div></li>)}</ul>; }
