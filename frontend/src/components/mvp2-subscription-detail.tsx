...
export function Mvp2SubscriptionDetail({ subscriptionId, created, replayed, basePath = "/mvp2/subscriptions" }: { subscriptionId: string; created: boolean; replayed: boolean; basePath?: string }) {
  ...
  const allowedCycles = [...new Set(plans?.flatMap((plan) => plan.allowedDeliveryCycleWeeks) ?? [])];
  ...
  return <div className="detail-stack">
    ...
    <section className="section-card"><h2>구독 관리</h2><p>표시되는 작업은 서버가 현재 허용한 작업입니다.</p><div className="button-row">{(["skip-next", "pause", "resume", "cancel"] as Command[]).filter((command) => hasAction(command.replaceAll("-", "_").toUpperCase())).map((command) => <button key={command} className={`button ${command === "cancel" ? "button-danger" : "button-secondary"}`} type="button" disabled={Boolean(pending)} onClick={() => void runCommand(command)}>{pending === command ? "처리 중" : LABEL[command]}</button>)}</div>
      ...
      {hasAction("CHANGE_DELIVERY_CYCLE") ? <form className="command-panel" onSubmit={(event) => { event.preventDefault(); const weeks = Number(deliveryCycleWeeks); if (allowedCycles.includes(weeks)) void runCommand("change-delivery-cycle", { deliveryCycleWeeks: weeks }); }}><h3>배송 주기 변경</h3><label className="form-field">배송 주기<select className="input" value={deliveryCycleWeeks} onChange={(event) => setDeliveryCycleWeeks(event.target.value)} disabled={!plans || Boolean(pending)}>{allowedCycles.map((weeks) => <option key={weeks} value={weeks}>{weeks}주마다</option>)}</select></label><button className="button button-secondary" type="submit" disabled={!allowedCycles.includes(Number(deliveryCycleWeeks)) || Boolean(pending)}>{pending === "change-delivery-cycle" ? "변경 중" : "배송 주기 변경"}</button></form> : null}
      {hasAction("CHANGE_PLAN") ? <form className="command-panel" onSubmit={(event) => { event.preventDefault(); const id = Number(planVersionId); if (Number.isSafeInteger(id) && id > 0) void runCommand("change-plan", { planVersionId: id }); }}><h3>플랜 변경</h3><label className="form-field">플랜<select className="input" value={planVersionId} onChange={(event) => setPlanVersionId(event.target.value)} disabled={!plans || Boolean(pending)}><option value="">플랜을 선택하세요</option>{plans?.map((plan) => <option key={plan.planVersionId} value={plan.planVersionId}>{plan.planName} · {formatPrice(plan.packagePriceKrw)}</option>)}</select></label><button className="button button-secondary" type="submit" disabled={!planVersionId || Boolean(pending)}>{pending === "change-plan" ? "변경 중" : "플랜 변경"}</button></form> : null}
    </section>
    {subscription.issue ? <section className="section-card"><h2>처리가 필요합니다</h2><p>{subscription.issue.message}</p>{subscription.issue.code === "SHIPPING_ADDRESS_REQUIRED" && hasAction("UPDATE_SHIPPING_ADDRESS") ? <AddressForm address={address} setAddress={setAddress} saving={addressSaving} onSubmit={updateShippingAddress} /> : null}{subscription.issue.code === "BILLING_METHOD_REQUIRED" && hasAction("REGISTER_BILLING_METHOD") ? <Link className="button button-primary" href="/billing-methods">결제수단 등록으로 이동</Link> : null}{subscription.issue.code === "PAYMENT_SUPPORT_REQUIRED" || subscription.issue.code === "STOCK_UNAVAILABLE" ? <p className="field-help">이 문제는 고객센터 또는 상품 상태 확인이 필요합니다.</p> : null}</section> : null}
  </div>;
}
