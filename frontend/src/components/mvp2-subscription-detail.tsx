...
export function Mvp2SubscriptionDetail({ subscriptionId, created, replayed, basePath = "/mvp2/subscriptions" }: { subscriptionId: string; created: boolean; replayed: boolean; basePath?: string }) {
  ...
  return <div className="detail-stack subscription-detail">
    ...
    <header className="subscription-overview page-heading"><p className="eyebrow">정기배송 · Daily Orbit</p><h1>내 정기배송</h1><p>{subscription.pet?.name ?? "반려동물"} · <span className="status-badge">{formatSubscriptionStatus(subscription.status)}</span></p></header>
