...
import { newIdempotencyKey, v2Api, type Pet, type PlanVersion, type V2SubscriptionDetail } from "@/lib/v2-api";
...
export function Mvp2SubscriptionDetail({ subscriptionId, created, replayed }: { subscriptionId: string; created: boolean; replayed: boolean }) {
  ...
  const [ownedPets, setOwnedPets] = useState<Pet[] | null>(null);
  const [planVersionId, setPlanVersionId] = useState<number | null>(null);
  const [legacyPetId, setLegacyPetId] = useState<number | null>(null);
  const commandKeys = useRef<Partial<Record<Command, string>>>({});
  const errorRef = useRef<HTMLDivElement>(null);

  const load = useCallback(() => { setSubscription(null); setMessage(null); setMessageKind(null); setRequestKey((key) => key + 1); }, []);
  ...
  useEffect(() => {
    ...
    let active = true;
    void v2Api.subscriptions.detail(subscriptionId, { schedulePage, commandPage }).then((response) => {
      if (!active) return;
    ...
    return () => { active = false; };
  }, [auth, commandPage, requestKey, router, schedulePage, subscriptionId]);

  const changePetId = subscription?.pet?.petId ?? legacyPetId;
  const currentCycle = subscription?.currentSnapshot.deliveryCycleWeeks ?? null;

  useEffect(() => {
    if (subscription?.pet || subscription?.status !== "ACTIVE") return;
    let active = true;
    void v2Api.pets.list().then(({ body }) => { if (active) setOwnedPets(body.items); }).catch(() => { if (active) setOwnedPets([]); });
    return () => { active = false; };
  }, [subscription?.pet, subscription?.status]);

  useEffect(() => {
    if (subscription?.status !== "ACTIVE" || changePetId === null || currentCycle === null) return;
    let active = true;
    queueMicrotask(() => { if (active) { setPlans(null); setPlanLoadError(null); } });
    void v2Api.plans.list(changePetId).then(({ body }) => {
      if (active) setPlans(body.items.filter((plan) => plan.allowedDeliveryCycleWeeks.includes(currentCycle)));
    }).catch((error: unknown) => { if (active) setPlanLoadError(error instanceof ApiError ? error.message : "호환 플랜을 불러오지 못했습니다."); });
    return () => { active = false; };
  }, [changePetId, currentCycle, planRetryKey, subscription?.status]);
  ...
  async function runCommand(command: Command, body: Record<string, unknown> = {}) {
    ...
    setMessage(null); setMessageKind(null); setPendingCommand(command);
    ...
    try {
      ...
      setMessage(response.replayed ? "이전 성공 결과를 다시 표시했습니다." : `${COMMAND_LABEL[command]} 요청이 반영되었습니다.`); setMessageKind("success");
      ...
      else if (error instanceof ApiError && error.code === "SUBSCRIPTION_VERSION_MISMATCH") { setMessage("다른 변경이 먼저 반영되었습니다. 최신 상태를 다시 불러온 뒤 선택을 확인해 주세요."); setRequestKey((key) => key + 1); }
  ...
  const canChange = subscription?.status === "ACTIVE";
  ...
  return <div className="detail-stack">
    ...
    <section className="section-card" aria-labelledby="commands-title"><h2 id="commands-title">구독 관리</h2><p>허용되는 작업은 현재 서버 상태와 최신 버전에 따라 결정됩니다.</p><div className="button-row">{subscription.status === "ACTIVE" ? <><button className="button button-secondary" type="button" disabled={Boolean(pendingCommand)} onClick={() => void runCommand("skip-next")}>{pendingCommand === "skip-next" ? "처리 중" : "다음 회차 건너뛰기"}</button><button className="button button-secondary" type="button" disabled={Boolean(pendingCommand)} onClick={() => void runCommand("pause")}>{pendingCommand === "pause" ? "처리 중" : "일시정지"}</button></> : null}{subscription.status === "PAUSED" ? <button className="button button-primary" type="button" disabled={Boolean(pendingCommand)} onClick={() => void runCommand("resume")}>{pendingCommand === "resume" ? "처리 중" : "재개"}</button> : null}{subscription.status !== "CANCELED" ? <button className="button button-danger" type="button" disabled={Boolean(pendingCommand)} onClick={() => void runCommand("cancel")}>{pendingCommand === "cancel" ? "처리 중" : "해지"}</button> : null}</div>{canChange ? <div className="command-panel"><h3>플랜 변경</h3>{subscription.pet === null ? <>{ownedPets === null ? <LoadingState>변경에 사용할 반려동물을 불러오고 있습니다.</LoadingState> : <label className="form-field">반려동물<select className="input" value={legacyPetId ?? ""} onChange={(event) => { const id = Number(event.target.value); setLegacyPetId(Number.isSafeInteger(id) && id > 0 ? id : null); setPlanVersionId(null); commandKeys.current["change-plan"] = undefined; }}><option value="">반려동물을 선택하세요</option>{ownedPets.map((pet) => <option key={pet.petId} value={pet.petId}>{pet.name}</option>)}</select></label>}</> : null}{changePetId === null ? <p>변경에 사용할 본인 반려동물을 선택해 주세요.</p> : planLoadError ? <ErrorState title="호환 플랜을 불러오지 못했습니다." message={planLoadError} onRetry={() => setPlanRetryKey((key) => key + 1)} /> : plans === null ? <LoadingState>호환 플랜을 불러오고 있습니다.</LoadingState> : plans.length === 0 ? <p>현재 배송 주기를 유지할 수 있는 호환 플랜이 없습니다.</p> : <><label className="form-field">플랜<select className="input" value={planVersionId ?? ""} onChange={(event) => { const id = Number(event.target.value); const plan = plans.find((item) => item.planVersionId === id) ?? null; setPlanVersionId(plan?.planVersionId ?? null); commandKeys.current["change-plan"] = undefined; }}><option value="">플랜을 선택하세요</option>{plans.map((plan) => <option key={plan.planVersionId} value={plan.planVersionId}>{plan.planName} · {formatPrice(plan.packagePriceKrw)}</option>)}</select></label><p className="field-help">배송 주기는 {currentCycle}주로 유지됩니다.</p><button className="button button-primary" type="button" disabled={!selectedPlan || Boolean(pendingCommand)} onClick={() => void runCommand("change-plan", { planVersionId, ...(subscription.pet === null ? { petId: changePetId } : {}) })}>{pendingCommand === "change-plan" ? "변경 중" : "다음 회차부터 플랜 변경"}</button></>}</div> : null}</section>
    <section className="section-card" aria-labelledby="schedule-title"><h2 id="schedule-title">예정 회차</h2>{subscription.schedules.items.length ? <ol className="history-list">{subscription.schedules.items.map((item) => <li key={item.scheduleId}><strong>{formatIsoLocalDate(item.scheduledDate)}</strong><span>{item.status}</span></li>)}</ol> : <p>표시할 예정 회차가 없습니다.</p>}<PageControls page={subscription.schedules.page} size={subscription.schedules.size} total={subscription.schedules.totalElements} onChange={setSchedulePage} /></section>
    <section className="section-card" aria-labelledby="history-title"><h2 id="history-title">명령 이력</h2>{subscription.commandHistory.items.length ? <ol className="history-list">{subscription.commandHistory.items.map((item, index) => <li key={`${item.occurredAt}-${index}`}><strong>{item.commandType}</strong><span>{item.result} · {item.occurredAt}</span></li>)}</ol> : <p>표시할 명령 이력이 없습니다.</p>}<PageControls page={subscription.commandHistory.page} size={subscription.commandHistory.size} total={subscription.commandHistory.totalElements} onChange={setCommandPage} /></section>
  </div>;
}

function PageControls({ page, size, total, onChange }: { page: number; size: number; total: number; onChange: (page: number) => void }) {
  const lastPage = Math.max(0, Math.ceil(total / size) - 1);
  return <div className="button-row"><button className="button button-secondary" type="button" disabled={page === 0} onClick={() => onChange(page - 1)}>이전</button><span className="field-help">총 {total}건 · 현재 {page + 1}페이지</span><button className="button button-secondary" type="button" disabled={page >= lastPage} onClick={() => onChange(page + 1)}>다음</button></div>;
}
