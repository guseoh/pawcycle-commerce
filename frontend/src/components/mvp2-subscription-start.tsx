...
export function Mvp2SubscriptionStart({ basePath = "/mvp2/subscriptions" }: { basePath?: string }) {
  ...
  const loadPets = useCallback(() => {
    ...
    void v2Api.pets.list().then(({ body }) => {
      ...
      setState("error");
      setMessage("반려동물 목록을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
    });
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      void v2Api.plans.list(selectedPetId).then(({ body }) => { if (active) { setPlans(body.items); setPlanState("idle"); } }).catch(() => {
        if (active) { setPlanState("error"); setMessage("호환 플랜을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요."); }
      });
    });
    return () => { active = false; };
  ...
  const selectPlan = useCallback(async (plan: PlanVersion, preferredCycle: number | null = null) => {
    ...
    try {
      ...
      setMessage("플랜 상세를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
  ...
  async function registerPet(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage("반려동물을 등록하지 못했습니다. 입력을 유지한 채 다시 시도할 수 있습니다.");
      focusError();
  ...
  async function createSubscription(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      else if (error instanceof ApiError && FORM_ERROR_CODES.has(error.code)) setMessage("선택한 반려동물과 플랜 정보를 확인해 주세요.");
      else setMessage("구독을 만들지 못했습니다. 같은 요청으로 다시 시도할 수 있습니다.");
      focusError();
