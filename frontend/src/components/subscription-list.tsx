...
import { subscriptionApi, type Page, type SubscriptionSummary } from "@/lib/subscription-api";

export function SubscriptionList({ basePath = "/subscriptions" }: { basePath?: string }) {
  const auth = useAuth();
  const router = useRouter();
  const [result, setResult] = useState<Page<SubscriptionSummary> | null>(null);
  ...
  useEffect(() => {
    ...
    let active = true;
    void subscriptionApi.subscriptions.list(page).then(({ body }) => { if (active) setResult(body); }).catch((error: unknown) => {
      if (!active) return;
