...
const v2ApiSource = readFileSync(new URL("../lib/v2-api.ts", import.meta.url), "utf8");
...
test("홈은 Commerce 진입 흐름과 인증별 상태를 유지한다", () => {
  ...
  assert.match(homeSource, /finalProductApi\.reorderTiming\(\)/);
  assert.match(homeSource, /loadAllSubscriptions\(\)/);
  assert.match(homeSource, /reorderTimingItems\(response\)\.slice\(0, 3\)/);
  assert.match(homeSource, /status === "ACTIVE" && subscription\.nextScheduledDate/);
  assert.match(homeSource, /href=\{`\/subscriptions\/\$\{next\.subscriptionId\}`\}/);
  assert.match(homeSource, /function HomeRoutineEntry/);
...
test("카탈로그 카드는 실제 상품 링크와 일관된 이미지·페이지 상태를 제공한다", () => {
  ...
  assert.match(productsSource, /document\.activeElement === last/);
  assert.match(productsSource, /tabIndex=\{-1\}/);
