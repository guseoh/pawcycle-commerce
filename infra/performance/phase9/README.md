...
HTTP `/api/products` counter, Hikari counters, digest counters에는 Prometheus scrape, backend healthcheck 및 기타 local traffic이 섞일 수 있다. 따라서 request당 query/borrow 값은 오염 가능성을 포함한 diagnostic estimate이며 exact per-request claim으로 사용하지 않는다. metric이 없으면 null/미확인으로 남길 수 있지만 Prometheus query, Docker 필수 조회, MySQL collector가 실패하면 diagnostic은 fail-close한다.
