...
function Get-MySqlAggregate {
    $sql = 'MYSQL_PWD="$MYSQL_ROOT_PASSWORD" mysql -N -uroot "$MYSQL_DATABASE" -e "SELECT CONCAT(CHAR(100,105,103,101,115,116,95,99,111,117,110,116,61), COALESCE(SUM(COUNT_STAR),0)), CONCAT(CHAR(100,105,103,101,115,116,95,119,97,105,116,95,112,115,61), COALESCE(SUM(SUM_TIMER_WAIT),0)), CONCAT(CHAR(100,105,103,101,115,116,95,114,111,119,115,95,101,120,97,109,105,110,101,100,61), COALESCE(SUM(SUM_ROWS_EXAMINED),0)) FROM performance_schema.events_statements_summary_by_digest WHERE SCHEMA_NAME=DATABASE() AND (LOCATE(CHAR(112,114,111,100,117,99,116,115), DIGEST_TEXT)>0 OR LOCATE(CHAR(115,107,117,115), DIGEST_TEXT)>0); SELECT CONCAT(CHAR(116,104,114,101,97,100,115,95,99,111,110,110,101,99,116,101,100,61), COALESCE(MAX(CASE WHEN VARIABLE_NAME=CHAR(84,104,114,101,97,100,115,95,99,111,110,110,101,99,116,101,100) THEN VARIABLE_VALUE ELSE 0 END),0)) FROM performance_schema.global_status;"'
    try {
        $raw = (docker exec $MysqlContainer sh -lc $sql) -join "`n"
        $values = [ordered]@{ digestCount = $null; digestWaitPs = $null; digestRowsExamined = $null; threadsConnected = $null }
        foreach ($part in ($raw -split "`t|`r?`n")) {
        ...
        return [ordered]@{ digestCount = $null; digestWaitPs = $null; digestRowsExamined = $null; threadsConnected = $null }
    }
}

function Get-ContainerEvidence {
    $stats = @(docker stats --no-stream --format '{{.Name}}|{{.CPUPerc}}|{{.MemUsage}}|{{.MemPerc}}|{{.PIDs}}' | Where-Object { $_ -match 'pawcycle-local-integration-(backend|mysql|frontend|proxy)-1' })
    $health = @(docker inspect --format '{{.Name}}|health={{if .State.Health}}{{.State.Health.Status}}{{else}}no-health{{end}}|restarts={{.RestartCount}}|oom={{.State.OOMKilled}}' pawcycle-local-integration-backend-1 pawcycle-local-integration-mysql-1 pawcycle-local-integration-frontend-1 pawcycle-local-integration-proxy-1)
...
function Get-Snapshot([string]$Label) {
    return [ordered]@{
        ...
        mysql = Get-MySqlAggregate
        containers = Get-ContainerEvidence
    }
}
...
$preflight = Get-Snapshot 'preflight'
...
$measurementStart = Get-Snapshot 'measurement-start'
$samples = [System.Collections.Generic.List[object]]::new()
$deadline = (Get-Date).AddSeconds($MeasurementSeconds)
while ((Get-Date) -lt $deadline -and -not $process.HasExited) {
    $samples.Add((Get-Snapshot 'measurement-sample'))
    Start-Sleep -Seconds $SampleSeconds
}
$measurementEnd = Get-Snapshot 'measurement-end'
$process.WaitForExit()
