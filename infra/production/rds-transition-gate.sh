...
CAP_DATABASE= CAP_USER= CAP_PASSWORD= CAP_ROOT_PASSWORD= CAP_HOST= CAP_PORT= CAP_MODE= CAP_URL= CAP_IDENTITY=
capture_runtime(){
  local root=$1 role=$2
  ...
  read_runtime_setting "$root/current/mysql.env" MYSQL_USER CAP_USER
  read_runtime_setting "$root/current/mysql.env" MYSQL_PASSWORD CAP_PASSWORD
  read_runtime_setting "$root/current/mysql.env" MYSQL_ROOT_PASSWORD CAP_ROOT_PASSWORD
  ...
  CAP_IDENTITY="$(printf '%s\0%s\0%s\0%s' "$CAP_DATABASE" "$CAP_USER" "$CAP_PASSWORD" "$CAP_ROOT_PASSWORD" | sha256sum | awk '{print $1}')"
}
validate_state(){
