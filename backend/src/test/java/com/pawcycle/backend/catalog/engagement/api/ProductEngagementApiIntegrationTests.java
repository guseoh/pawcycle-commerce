...
class ProductEngagementApiIntegrationTests {
  ...
  @Autowired
  ProductEngagementApiIntegrationTests(
  ...
  void setUp() {
    ...
    jdbc.update(
        "INSERT IGNORE INTO members(id,email,password_hash,role) VALUES (1,?,?, 'ADMIN'),(2,?,?,"
            + " 'USER'),(3,?,?, 'USER')",
        "mvp4-admin-" + UUID.randomUUID() + "@example.test",
        "fixture",
        "mvp4-user-" + UUID.randomUUID() + "@example.test",
        "fixture",
        "mvp4-other-" + UUID.randomUUID() + "@example.test",
        "fixture");
    Category category =
        categories.saveAndFlush(
            new Category("MVP4 " + UUID.randomUUID(), "mvp4-" + UUID.randomUUID(), 0, true));
    Product product =
        products.saveAndFlush(
            new Product(category, "MVP4 product", "short", "description", "DOG", null, "PUBLIC"));
    Sku sku =
        skus.saveAndFlush(
            new Sku(
                product,
                "MVP4-" + UUID.randomUUID(),
                "2kg",
    ...
    jdbc.update(
        ...
        sku.getPrice());
    jdbc.update(
        "INSERT INTO deliveries(order_id,status,shipped_at) VALUES (?,'SHIPPED',?)",
        orderId,
        Timestamp.from(Instant.now()));
  }
  ...
  void reviewRequiresDeliveredPurchaseAndHiddenReviewLeavesTrustAggregateEmpty() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(status().isOk());
    mockMvc
        .perform(get("/api/products/{productId}/reviews", productId))
        .andExpect(status().isOk())
  ...
  void reviewAndQuestionMutationsEnforceOwnershipAndPreserveModeration() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/reviews/{reviewId}", reviewId)
                ...
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"rating\":4,\"content\":\"owner update\"}"))
        .andExpect(status().isOk());
    ...
    mockMvc
        .perform(
            post("/api/products/{productId}/questions", productId)
                .with(user())
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void answeredQuestionCannotBeChangedAndAnswerNotificationIsNotDuplicated() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/admin/product-questions/{questionId}/answer", questionId)
                .with(admin())
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"answer\":\"답변\"}"))
    ...
    mockMvc
        .perform(
            patch("/api/product-questions/{questionId}", questionId)
                ...
                .content("{\"content\":\"수정\"}"))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.code").value("PRODUCT_QUESTION_LOCKED"));
    mockMvc
        .perform(
            delete("/api/product-questions/{questionId}", questionId).with(user()).with(csrf()))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.code").value("PRODUCT_QUESTION_LOCKED"));
    mockMvc
        .perform(
            put("/api/admin/product-questions/{questionId}/answer", questionId)
                ...
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"answer\":\"수정 답변\"}"))
        .andExpect(status().isOk());
    org.assertj.core.api.Assertions.assertThat(
            jdbc.queryForObject(
                "SELECT COUNT(*) FROM notifications WHERE type='PRODUCT_QUESTION_ANSWERED' AND"
                    + " reference_id=?",
                Integer.class,
                questionId))
        .isEqualTo(1);
  ...
  void adminEndpointsKeepExistingUserBoundaryAndRejectOverflowPagination() throws Exception {
    mockMvc
        .perform(get("/api/products/{productId}/reviews/me", productId))
        .andExpect(status().isUnauthorized());
    mockMvc.perform(get("/api/admin/product-reviews")).andExpect(status().isUnauthorized());
    mockMvc
        .perform(get("/api/admin/product-reviews").with(user()))
        .andExpect(status().isForbidden());
  ...
  void detailSectionsExposeOnlyVisibleRowsInStableOrder() throws Exception {
    mockMvc
        .perform(
            post("/api/admin/products/{productId}/detail-sections", productId)
                .with(admin())
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"title\":\"숨김\",\"body\":\"hidden\",\"displayOrder\":0,\"visible\":false}"))
    ...
    mockMvc
        .perform(
            post("/api/admin/products/{productId}/detail-sections", productId)
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"title\":\"노출\",\"body\":\"plain\",\"displayOrder\":1,\"visible\":true}"))
        .andExpect(status().isCreated());
    mockMvc
        .perform(get("/api/products/{productId}", productId))
        .andExpect(status().isOk())
  ...
  private void deliverOrder() {
    jdbc.update(
        "UPDATE deliveries SET status='DELIVERED',delivered_at=? WHERE order_id=(SELECT order_id"
            + " FROM order_items WHERE sku_id=? LIMIT 1)",
        Timestamp.from(Instant.now()),
        skuId);
  }

  private RequestPostProcessor user() {
    return role(MemberRole.USER, 2L);
  }

  private RequestPostProcessor otherUser() {
    return role(MemberRole.USER, 3L);
  }

  private RequestPostProcessor admin() {
    return role(MemberRole.ADMIN, 1L);
  }

  private RequestPostProcessor role(MemberRole role, long id) {
    return authentication(
        new UsernamePasswordAuthenticationToken(
            new AuthenticatedMemberPrincipal(id, role),
            null,
