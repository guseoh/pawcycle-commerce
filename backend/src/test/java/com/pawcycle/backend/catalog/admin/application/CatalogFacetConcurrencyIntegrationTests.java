...
class CatalogFacetConcurrencyIntegrationTests {
	...
	void tearDown() {
		...
		if (sourceCategoryId != null || targetCategoryId != null) {
			jdbc.update("DELETE FROM category_facets WHERE category_id IN (?,?)",
		...
		if (sourceCategoryId != null) jdbc.update("DELETE FROM categories WHERE id=?", sourceCategoryId);
		if (targetCategoryId != null) jdbc.update("DELETE FROM categories WHERE id=?", targetCategoryId);
	}
	...
	void concurrentProductFacetSetAndCategoryFacetRemovalPreserveInvariant() throws Exception {
		...
		ConcurrentResult results = runTogether(
				() -> expansionService.setProductFacetValues(productId, new ProductFacetValues(List.of(facetOptionId))),
	...
	private void seedCatalog(boolean assignTargetFacet) {
		...
		jdbc.update("INSERT INTO categories(name,slug,display_order,active) VALUES (?,?,0,true)",
				"facet-source-" + suffix, "facet-source-" + suffix);
		sourceCategoryId = lastInsertId();
		jdbc.update("INSERT INTO categories(name,slug,display_order,active) VALUES (?,?,1,true)",
				"facet-target-" + suffix, "facet-target-" + suffix);
		...
		jdbc.update("""
				...
				""", "facet-product-" + suffix, sourceCategoryId, "facet-product-" + suffix);
		productId = lastInsertId();
		jdbc.update("INSERT INTO facet_definitions(`key`,name) VALUES (?,?)",
				"facet-" + suffix, "facet-" + suffix);
		facetDefinitionId = lastInsertId();
		jdbc.update("INSERT INTO facet_options(facet_definition_id,value,display_order) VALUES (?,? ,0)",
				facetDefinitionId, "value-" + suffix);
		facetOptionId = lastInsertId();
		jdbc.update("INSERT INTO category_facets(category_id,facet_definition_id,display_order) VALUES (?,?,0)",
				sourceCategoryId, facetDefinitionId);
		if (assignTargetFacet) {
			jdbc.update("INSERT INTO category_facets(category_id,facet_definition_id,display_order) VALUES (?,?,0)",
					targetCategoryId, facetDefinitionId);
		}
	}

	private ConcurrentResult runTogether(ThrowingOperation first, ThrowingOperation second) throws Exception {
	...
	private Outcome executeTogether(ThrowingOperation operation, CountDownLatch ready, CountDownLatch start) throws Exception {
		ready.countDown();
		if (!start.await(10, TimeUnit.SECONDS)) throw new IllegalStateException("concurrency start timeout");
		try {
			operation.run();
