...
class ProductComparisonServiceTests {
	...
	void duplicateNullAndFourProductIdsAreRejected() {
		for (List<Long> ids : List.of(List.of(1L, 1L), java.util.Arrays.asList(1L, null), List.of(1L, 2L, 3L, 4L))) {
			JdbcTemplate jdbc = mock(JdbcTemplate.class);
			ProductComparisonAiClient ai = mock(ProductComparisonAiClient.class);
			org.assertj.core.api.Assertions.assertThatThrownBy(() -> new ProductComparisonService(jdbc, ai).compare(ids))
					.isInstanceOf(ProductComparisonException.class)
					.hasFieldOrPropertyWithValue("code", "VALIDATION_FAILED");
			verifyNoInteractions(jdbc, ai);
		}
	...
	void threeProductComparisonKeepsCanonicalFactsWhenAiSummaryIsValid() {
		...
		ProductComparisonService.ComparisonResponse response = new ProductComparisonService(jdbc, ai).compare(List.of(1L, 2L, 3L));
	...
	private void stubFacts(JdbcTemplate jdbc, long id) {
		Map<String, Object> row = Map.ofEntries(
				Map.entry("id", id), Map.entry("name", "상품" + id), Map.entry("thumbnail_url", "thumb"),
				Map.entry("brand_name", "브랜드"), Map.entry("category_name", "사료"),
				Map.entry("price", new BigDecimal("1000.00")), Map.entry("compare_at_price", new BigDecimal("1200.00")),
				Map.entry("average_rating", new BigDecimal("4.50")), Map.entry("review_count", 3L),
