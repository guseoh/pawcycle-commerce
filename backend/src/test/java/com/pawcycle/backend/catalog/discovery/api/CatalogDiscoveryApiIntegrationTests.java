...
class CatalogDiscoveryApiIntegrationTests {
	...
	void publicDiscoveryExposesActiveHierarchyBrandsAndCategoryFacets() throws Exception {
		...
		JsonNode root = objectMapper.readTree(mockMvc.perform(get("/api/catalog/discovery"))
		...
		List<String> brandSlugs = testSlugs(root.path("brands"), fixture.brandOneSlug(), fixture.brandTwoSlug());
		...
		JsonNode childFacets = findByField(root.path("categoryFacets"), "categorySlug", fixture.childSecondSlug());
		assertThat(childFacets).isNotNull();
		assertThat(facetKeys(childFacets.path("facets"))).containsExactly(fixture.textureKey(), fixture.proteinKey());
	...
	void existingCategoryAndProductContractsRemainUnchanged() throws Exception {
		...
		JsonNode products = objectMapper.readTree(mockMvc.perform(get("/api/products").param("category", fixture.topSlug()))
				.andExpect(status().isOk())
				.andReturn().getResponse().getContentAsString());
		assertThat(findByField(products.path("items"), "productId", product.getId().toString())).isNotNull();
		...
		mockMvc.perform(get("/api/products/{productId}", product.getId()))
				...
				.andExpect(result -> assertThat(objectMapper.readTree(result.getResponse().getContentAsString())
						.path("category").path("slug").asText()).isEqualTo(fixture.topSlug()));
	}
	...
	private Fixture seedFixture() {
		...
		return new Fixture(topId, topSlug, childFirstSlug, childSecondSlug, grandchildSlug, otherTopSlug,
				...
				otherFacetKey, proteinEarlyValue, proteinLateValue);
	}

	private long insertCategory(String name, String slug, int displayOrder, boolean active, Long parentId) {
		jdbcTemplate.update("INSERT INTO categories(name,slug,display_order,active,parent_id) VALUES (?,?,?,?,?)",
				name, slug, displayOrder, active, parentId);
	...
	private void insertBrand(String name, String slug, boolean active, int displayOrder) {
		jdbcTemplate.update("INSERT INTO brands(name,slug,logo_url,active,display_order) VALUES (?,?,NULL,?,?)",
				name, slug, active, displayOrder);
	}
	...
	private long insertFacet(String key, String name) {
		...
		return jdbcTemplate.queryForObject("SELECT id FROM facet_definitions WHERE `key`=?", Long.class, key);
	}

	private void insertOption(long facetId, String value, int displayOrder) {
		jdbcTemplate.update("INSERT INTO facet_options(facet_definition_id,value,display_order) VALUES (?,?,?)",
				facetId, value, displayOrder);
	}

	private void assignFacet(long categoryId, long facetId, int displayOrder) {
		jdbcTemplate.update("INSERT INTO category_facets(category_id,facet_definition_id,display_order) VALUES (?,?,?)",
				categoryId, facetId, displayOrder);
	}
	...
	private List<String> testFieldValues(JsonNode array, String field, String... expected) {
		...
		for (JsonNode node : array) if (candidates.contains(node.path(field).asText())) result.add(node.path(field).asText());
		return result;
	}
