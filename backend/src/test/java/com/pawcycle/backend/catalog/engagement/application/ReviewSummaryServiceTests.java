...
class ReviewSummaryServiceTests {
	...
	void cacheHitSkipsAiAndValidAiResultIsPersisted() {
		...
		ProductRepository products = mock(ProductRepository.class);
		ReviewSummaryAiClient ai = mock(ReviewSummaryAiClient.class);
		ReviewSummaryService service = new ReviewSummaryService(jdbc, products, ai, Clock.fixed(Instant.parse("2026-08-28T00:00:00Z"), ZoneOffset.UTC));
		List<ReviewSummaryService.ReviewRow> reviews = reviews(3);
		stubSummary(jdbc, products, reviews, reviews, List.of(Map.of("source_fingerprint", service.fingerprint(reviews), "summary", "좋은 품질의 상품입니다.")));
	...
	void validAiResultIsStoredAndInvalidOrExceptionIsUnavailable() {
		for (String aiOutput : List.of("상품의 만족도가 높습니다.", "<script>위험</script>")) {
			...
			if (aiOutput.startsWith("<")) {
				...
				assertThat(response.status()).isEqualTo("AVAILABLE");
				verify(jdbc).update(anyString(), eq(1L), org.mockito.ArgumentMatchers.anyString(), org.mockito.ArgumentMatchers.anyString(), org.mockito.ArgumentMatchers.any(Timestamp.class));
			}
		}
		...
		List<ReviewSummaryService.ReviewRow> reviews = reviews(3);
		stubSummary(jdbc, products, reviews, reviews, List.of());
		when(ai.summarize(org.mockito.ArgumentMatchers.anyList())).thenThrow(new IllegalStateException("provider unavailable"));
		assertThat(new ReviewSummaryService(jdbc, products, ai, Clock.systemUTC()).summary(1L).status()).isEqualTo("UNAVAILABLE");
	}
	...
	void visibleReviewOutsideLatestAiWindowInvalidatesSummaryFingerprint() {
		...
		changedFull.add(new ReviewSummaryService.ReviewRow(31L, 5, "새로운 visible review", timestamp(31)));
		stubSummary(jdbc, products, latest, changedFull, List.of(Map.of("source_fingerprint", service.fingerprint(latest), "summary", "오래된 요약")));
	...
	private void stubSummary(JdbcTemplate jdbc, ProductRepository products, List<ReviewSummaryService.ReviewRow> latest, List<ReviewSummaryService.ReviewRow> all, List<Map<String,Object>> cache) {
		when(products.findPublicById(1L)).thenReturn(Optional.of(mock(Product.class)));
		when(jdbc.queryForObject(org.mockito.ArgumentMatchers.contains("JOIN brands"), eq(Integer.class), eq(1L))).thenReturn(1);
		when(jdbc.query(anyString(), org.mockito.ArgumentMatchers.<RowMapper<ReviewSummaryService.ReviewRow>>any(), eq(1L))).thenReturn(latest, all);
		when(jdbc.queryForObject(org.mockito.ArgumentMatchers.contains("COUNT(*)"), eq(Long.class), eq(1L))).thenReturn((long) all.size());
		when(jdbc.queryForObject(org.mockito.ArgumentMatchers.contains("AVG(rating)"), eq(BigDecimal.class), eq(1L))).thenReturn(new BigDecimal("4.50"));
		when(jdbc.queryForList(org.mockito.ArgumentMatchers.contains("product_review_summaries"), eq(1L))).thenReturn(cache);
	}

	private List<ReviewSummaryService.ReviewRow> reviews(int count) {
		List<ReviewSummaryService.ReviewRow> result = new ArrayList<>();
		for (int index = 1; index <= count; index++) result.add(new ReviewSummaryService.ReviewRow(index, 5, "좋은 상품 " + index, timestamp(index)));
		return result;
	}

	private Timestamp timestamp(int day) { return Timestamp.valueOf("2026-08-" + "%02d".formatted(Math.min(day, 28)) + " 00:00:00"); }
}
