...
class V2IdempotencyCleanupConcurrencyIntegrationTests {
  ...
  void replayHoldingRowLockReturnsStoredResultBeforeCleanupDeletesExpiredRow() throws Exception {
    ...
    try {
      ...
      Future<V2IdempotencyCleanupService.CleanupResult> cleanupResult =
          executor.submit(
              () ->
      ...
      V2IdempotencyCleanupService.CleanupResult deleted =
          cleanupResult.get(AWAIT_SECONDS, TimeUnit.SECONDS);

      assertThat(observation.result().replay()).isTrue();
      assertThat(((Number) observation.result().body().get("subscriptionId")).longValue())
  ...
  void cleanupCommittedFirstMakesSameKeyANewRequestWithNewCompletion() throws Exception {
    ...
    try {
      ...
      assertThat(((Number) created.body().get("subscriptionId")).longValue())
  ...
  private Fixture createFixture(String prefix) {
    ...
    long petId =
        ((Number)
                service
                    .createPet(member.getId(), Map.of("name", "동시성 반려동물", "petType", "DOG"))
                    .get("petId"))
            .longValue();
    ...
    return new Fixture(
        member.getId(), key, request, ((Number) created.body().get("subscriptionId")).longValue());
  }
  ...
  private record Fixture(
      long memberId, String key, Map<String, Object> request, long subscriptionId) {}

  private record ReplayObservation(
      V2SubscriptionService.V2Result result, LocalDateTime completedAt) {}
}
