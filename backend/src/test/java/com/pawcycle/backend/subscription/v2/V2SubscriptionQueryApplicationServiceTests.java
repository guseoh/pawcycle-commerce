...
class V2SubscriptionQueryApplicationServiceTests {
	...
	void nextDeliveryPreservesDecimalAddonAmounts() {
		...
		assertThat(availableActions).containsExactly("CHANGE_PLAN", "CHANGE_DELIVERY_CYCLE", "RESCHEDULE_NEXT", "SKIP_NEXT", "PAUSE", "SET_NEXT_DELIVERY_ADDON", "REMOVE_NEXT_DELIVERY_ADDON", "CANCEL", "UPDATE_SHIPPING_ADDRESS");
	}
	...
	void scheduledWithoutAddonExposesSetAddonButNotRemoveAddon() {
		...
		@SuppressWarnings("unchecked")
		List<String> availableActions = (List<String>) body.get("availableActions");
		assertThat(availableActions).containsExactly("CHANGE_PLAN", "CHANGE_DELIVERY_CYCLE", "RESCHEDULE_NEXT", "SKIP_NEXT", "PAUSE", "SET_NEXT_DELIVERY_ADDON", "CANCEL", "UPDATE_SHIPPING_ADDRESS");
	}
	...
	void recoverableStockHeldOnlyExposesRemoveAddonAndCancelWhenAddonExists() {
		...
		V2SubscriptionQueryApplicationService service = new V2SubscriptionQueryApplicationService(
				store, new ObjectMapper(), Clock.fixed(Instant.parse("2026-08-28T00:00:00Z"), ZoneOffset.UTC));
		V2SubscriptionData.Subscription subscription = new V2SubscriptionData.Subscription(1L, 10L, "ACTIVE", 1L, null, 2, 5L);
		V2SubscriptionData.Snapshot snapshot = new V2SubscriptionData.Snapshot(5L, 2L, 20000L, 2, List.of());
		V2SubscriptionData.NextDeliverySchedule schedule = new V2SubscriptionData.NextDeliverySchedule(
