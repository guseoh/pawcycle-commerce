...
class V2PetPlanApplicationServiceTests {
	...
	void nameOnlyPatchDoesNotRewriteOmittedFieldsFromStaleRead() {
		...
		V2SubscriptionData.Pet current = new V2SubscriptionData.Pet(7L, "기존 이름", "DOG", "기존 품종", new BigDecimal("12.00"));
		when(store.findOwnedPet(10L, 7L)).thenReturn(current);
		V2PetPlanApplicationService service = new V2PetPlanApplicationService(
