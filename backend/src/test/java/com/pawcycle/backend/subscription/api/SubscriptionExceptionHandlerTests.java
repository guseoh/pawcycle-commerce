...
class SubscriptionExceptionHandlerTests {
  ...
  void expectedDomainErrorsUseApprovedStatusesCodesAndEmptyFieldErrors() {
    assertError(
        handler.handleSkuNotFound(new SkuNotFoundException()),
        HttpStatus.NOT_FOUND,
        "SKU_NOT_FOUND");
    assertError(
        handler.handleSkuNotSubscribable(new SkuNotSubscribableException()),
        HttpStatus.CONFLICT,
        "SKU_NOT_SUBSCRIBABLE");
    assertError(
        handler.handleSubscriptionNotFound(new SubscriptionNotFoundException()),
        HttpStatus.NOT_FOUND,
  ...
  void unexpectedErrorsUseEndpointSpecificSafeResponses() {
    assertSafeError(
        handler.handleCreateFailed(
            new SubscriptionCreateFailedException(
                new IllegalStateException("subscriptions table create"))),
        "SUBSCRIPTION_CREATE_FAILED",
        "구독을 생성하지 못했습니다.");
    assertSafeError(
        handler.handleListUnavailable(
            new SubscriptionListUnavailableException(
                new IllegalStateException("subscriptions table list"))),
        "SUBSCRIPTION_LIST_UNAVAILABLE",
        "구독 목록을 불러오지 못했습니다.");
    assertSafeError(
        handler.handleDetailUnavailable(
            new SubscriptionDetailUnavailableException(
  ...
  void jsonTypeErrorsUseKnownFieldAndMalformedJsonUsesRequestFallback() {
    MismatchedInputException typeMismatch =
        MismatchedInputException.from((JsonParser) null, Long.class, "wrong type");
    typeMismatch.prependPath(SubscriptionCreateRequest.class, "skuId");

    ResponseEntity<ApiErrorResponse> typeMismatchResponse =
        handler.handleMalformedJson(
            new HttpMessageNotReadableException(
                "wrong type", typeMismatch, mock(HttpInputMessage.class)));
    ResponseEntity<ApiErrorResponse> malformedResponse =
        handler.handleMalformedJson(
            new HttpMessageNotReadableException(
                "malformed",
    ...
    assertThat(typeMismatchResponse.getBody().fieldErrors())
        .extracting("field")
        .containsExactly("skuId");
    assertThat(malformedResponse.getBody()).isNotNull();
    assertThat(malformedResponse.getBody().fieldErrors())
        .extracting("field")
  ...
  private void assertSafeError(
      ResponseEntity<ApiErrorResponse> response, String code, String message) {
