    ...
    try {
      ...
      jdbc.update(
          "INSERT INTO skus(product_id,sku_code,name,price,subscribable,display_order,status)"
              + " VALUES (?,'LEGACY-1','legacy',1000,true,1,'ACTIVE')",
          productId);
      ...
      assertThat(
              jdbc.queryForObject(
                  "SELECT slug FROM categories WHERE id=(SELECT category_id FROM products WHERE"
                      + " id=?)",
                  String.class,
                  productId))
          .isEqualTo("__pawcycle_uncategorized__");
      assertThat(
              jdbc.queryForObject(
                  "SELECT active FROM categories WHERE slug='__pawcycle_uncategorized__'",
                  Boolean.class))
          .isFalse();
      assertThat(
              jdbc.queryForObject(
      ...
      assertThat(
              jdbc.queryForObject(
                  "SELECT is_nullable FROM information_schema.columns WHERE table_schema=DATABASE()"
                      + " AND table_name='products' AND column_name='category_id'",
                  String.class))
          .isEqualTo("NO");
