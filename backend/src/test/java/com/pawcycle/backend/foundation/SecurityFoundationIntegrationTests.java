...
class SecurityFoundationIntegrationTests {
  ...
  void anonymousProductRecommendationsArePublicAndKeepResponseContract() throws Exception {
    ...
    jdbc.update(
        ...
        categorySlug);
    Long categoryId =
        jdbc.queryForObject("SELECT id FROM categories WHERE slug=?", Long.class, categorySlug);
    ...
    mockMvc
        .perform(get("/api/products/{productId}/related", sourceId))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(get("/api/products/{productId}/complementary", sourceId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.requestId").isString())
        .andExpect(jsonPath("$.products").isArray());

    mockMvc
        .perform(get("/api/products/{productId}/related", Long.MAX_VALUE))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code").value("PRODUCT_NOT_FOUND"));
  }
  ...
  private long insertProduct(Long categoryId, String catalogKey, String name) {
    ...
    return jdbc.queryForObject(
        "SELECT id FROM products WHERE catalog_key=?", Long.class, catalogKey);
  }
  ...
  void recommendationApiRequiresAuthenticationAndHidesNonOwnedPet() throws Exception {
    mockMvc
        .perform(get("/api/recommendations/products").param("petId", "42"))
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.code").value("AUTH_REQUIRED"));
