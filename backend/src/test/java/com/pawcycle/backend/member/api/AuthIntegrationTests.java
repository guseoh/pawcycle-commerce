...
class AuthIntegrationTests {
  ...
  void loginChangesSessionIdPersistsSafePrincipalAndRotatesCsrfToken() throws Exception {
    ...
    MvcResult login =
        login(csrfSession, loginEmail, password)
            .andExpect(status().isOk())
    ...
    assertThat(login.getResponse().getContentAsString())
        .doesNotContain(csrfSession.session().getId());
    ...
    assertThat(context.getAuthentication().getPrincipal())
        .isInstanceOf(AuthenticatedMemberPrincipal.class);
  ...
  void adminLoginAndMeExposeAdditiveAdminRole() throws Exception {
    ...
    mockMvc
        .perform(get("/api/auth/me").session(csrfSession.session()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.memberId").value(admin.getId()))
  ...
  @Test
  void unknownEmailAndWrongPasswordReturnSameGenericFailureWithoutAuthentication()
      throws Exception {
    CsrfSession unknownSession = csrfSession();
    MvcResult unknown =
        login(unknownSession, "unknown@example.com", password)
    ...
    MvcResult wrongPassword =
        login(wrongPasswordSession, member.getEmail(), "wrong-" + password)
            .andExpect(status().isUnauthorized())
            .andExpect(jsonPath("$.code").value("INVALID_CREDENTIALS"))
            .andExpect(jsonPath("$.fieldErrors").isEmpty())
            .andReturn();

    assertThat(json(unknown).get("message").asText())
        .isEqualTo(json(wrongPassword).get("message").asText());
    assertThat(
            unknownSession
                .session()
                .getAttribute(HttpSessionSecurityContextRepository.SPRING_SECURITY_CONTEXT_KEY))
            ...
            post("/api/auth/login")
                ...
                .content(
                    objectMapper.writeValueAsString(
                        Map.of("email", "local@@example.com", "password", ""))))
  ...
  void logoutInvalidatesSessionClearsCookieAndRejectsOldSessionAndToken() throws Exception {
    ...
    mockMvc
        .perform(get("/api/auth/me").cookie(new Cookie("JSESSIONID", authenticatedSessionId)))
        .andExpect(status().isUnauthorized())
  ...
  void defaultLogoutPathsDoNotTerminateAuthenticatedSession() throws Exception {
    ...
    mockMvc
        .perform(get("/logout").session(csrfSession.session()))
        .andExpect(status().isForbidden())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
  ...
  private String csrfToken(MockHttpSession session) throws Exception {
    MvcResult result =
        mockMvc
            .perform(get("/api/auth/csrf").session(session))
            .andExpect(status().isOk())
            .andReturn();
    return token(result);
  }
  ...
  private org.springframework.test.web.servlet.ResultActions login(
      CsrfSession csrfSession, String email, String rawPassword) throws Exception {
