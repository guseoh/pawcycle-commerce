...
class ProductRecommendationServiceTests {
	...
	void trendingUsesOneBatchScoreLookupAndToleratesCandidatesWithoutActivity() {
		...
		RecommendationService.RecommendationResponse response = new ProductRecommendationService(repository, mock(JdbcTemplate.class)).trending("DOG", 10);

		assertThat(response.products()).extracting(RecommendationService.RecommendationItem::productId).containsExactly(1L);
		verify(repository).trendScores(eq(List.of(1L, 2L)), any(LocalDate.class));
	}

	private RecommendationCandidate candidate(long id) {
		return new RecommendationCandidate(id, "상품" + id, "설명", null, "DOG",
				new RecommendationCandidate.Category(id, "food", "사료"));
	}
}
