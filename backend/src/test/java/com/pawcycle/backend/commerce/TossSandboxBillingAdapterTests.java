...
class TossSandboxBillingAdapterTests {
  ...
  void neverUsesNetworkAndMapsBillingChargeOutcomes() {
    assertThat(adapter.issueBillingKey("customer", "auth").billingKey())
        .startsWith("sandbox-billing-");
    assertThat(adapter.charge("billing", "order", BigDecimal.ONE).status()).isEqualTo("SUCCEEDED");
    assertThat(adapter.charge("billing", "order-fail", BigDecimal.ONE).status())
        .isEqualTo("FAILED");
    assertThat(adapter.charge("billing", "order-unknown", BigDecimal.ONE).status())
        .isEqualTo("UNKNOWN");
