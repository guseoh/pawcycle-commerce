...
class CommerceAdminAuthorizationIntegrationTests {
  ...
  void operationsReadKeepsAnonymousUserAndMemberBoundaries() throws Exception {
    mockMvc
        .perform(get("/api/admin/operations"))
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.code").value("AUTH_REQUIRED"));
    mockMvc
        .perform(get("/api/admin/operations").with(role(MemberRole.USER)))
        .andExpect(status().isForbidden())
        .andExpect(jsonPath("$.code").value("ACCESS_DENIED"));
    mockMvc
        .perform(get("/api/admin/operations").with(role(MemberRole.ADMIN)))
  ...
  void requestLengthsUseBeanValidationBeforeAdminMutation() throws Exception {
    mockMvc
        .perform(
            post("/api/admin/deliveries/1/fail")
    ...
    mockMvc
        .perform(
            post("/api/admin/deliveries/1/ship")
                .with(role(MemberRole.ADMIN))
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"carrierCode\":\"" + "x".repeat(51) + "\",\"trackingNumber\":\"ok\"}"))
        .andExpect(status().isBadRequest());
    mockMvc
        .perform(
            post("/api/admin/deliveries/1/ship")
                ...
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"carrierCode\":\"ok\",\"trackingNumber\":\"" + "x".repeat(101) + "\"}"))
        .andExpect(status().isBadRequest());
  }
  ...
  void couponAndMembershipDomainValuesUseBeanValidationBeforeAdminMutation() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/admin/coupons")
                ...
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(negativeCoupon))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code").value("VALIDATION_FAILED"));
    mockMvc
        .perform(
            patch("/api/admin/coupons/999999")
                .with(role(MemberRole.ADMIN))
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(negativeCoupon))
    ...
    mockMvc
        .perform(
            post("/api/admin/membership-grades")
                .with(role(MemberRole.ADMIN))
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(negativeGrade))
        .andExpect(status().isBadRequest());
  }
  ...
  void missingInventoryAdjustmentKeepsNotFoundContract() throws Exception {
    mockMvc
        .perform(
            post("/api/admin/inventories/999999/adjustments")
                .with(role(MemberRole.ADMIN))
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"delta\":1}"))
        ...
        .andExpect(
            result -> {
              ...
              if (status == 401 || status == 403)
                throw new AssertionError("admin mutation was rejected by security: " + path);
            });
  }
