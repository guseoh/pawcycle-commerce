...
public interface FacetOptionRepository extends JpaRepository<FacetOptionEntity, Long> {
  ...
  @Query(
      "select option from FacetOptionEntity option join fetch option.facetDefinition "
          + "where option.facetDefinition.id in :definitionIds "
          + "order by option.facetDefinition.id, option.displayOrder, option.id")
  List<FacetOptionEntity> findAllByDefinitionIdsOrdered(
      @Param("definitionIds") List<Long> definitionIds);
