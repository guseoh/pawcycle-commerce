...
public final class ProductionDemoCatalogImportCommand {
	...
	private static final List<String> ENFORCED_KEYS = ENFORCED_ARGUMENTS.stream()
			.map(ProductionDemoCatalogImportCommand::optionKey)
			.toList();
	...
	static int runIfRequested(
		...
		ResolvedProperty webApplicationType = resolve(args, systemProperties, environment, WEB_APPLICATION_TYPE);
		if (!webApplicationType.present() || webApplicationType.ambiguous()
	...
	private static int runApplication(String[] args) {
		...
		try (PrintStream suppressedOutput = new PrintStream(OutputStream.nullOutputStream(), true, StandardCharsets.UTF_8);
				PrintStream suppressedError = new PrintStream(OutputStream.nullOutputStream(), true, StandardCharsets.UTF_8)) {
