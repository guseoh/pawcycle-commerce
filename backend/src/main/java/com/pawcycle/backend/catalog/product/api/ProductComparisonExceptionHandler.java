...
class ProductComparisonExceptionHandler { @ExceptionHandler(ProductComparisonException.class) ResponseEntity<ApiErrorResponse> handle(ProductComparisonException e){return ResponseEntity.status(e.status()).body(ApiErrorResponse.withoutFieldErrors(e.code(),e.getMessage()));} }
