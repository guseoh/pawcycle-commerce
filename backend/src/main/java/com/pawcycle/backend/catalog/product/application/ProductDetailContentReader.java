...
public class ProductDetailContentReader {
    ...
    public List<ProductDetailSectionView> visibleSections(long productId) {
        return jdbc.query("""
                ...
                """, (rs, rowNum) -> new ProductDetailSectionView(
                rs.getLong("id"), rs.getString("title"), rs.getString("body"),
                rs.getInt("display_order"), rs.getBoolean("visible"),
    ...
    public ProductTrustView trust(long productId) {
        return jdbc.queryForObject("""
                ...
                FROM reviews r
                WHERE r.product_id=?
                """, (rs, rowNum) -> new ProductTrustView(
                rs.getBigDecimal("average_rating"), rs.getLong("review_count"), rs.getLong("question_count")),
                productId, productId);
    }
