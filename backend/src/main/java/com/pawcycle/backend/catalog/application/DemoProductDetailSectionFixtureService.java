...
public class DemoProductDetailSectionFixtureService {
  ...
  private Map<String, Long> findProducts(Set<String> catalogKeys) {
    ...
    for (Map<String, Object> row :
        jdbc.queryForList(
            "SELECT id,catalog_key FROM products WHERE catalog_key IN ("
                + placeholders
                + ") FOR UPDATE",
            catalogKeys.toArray())) {
  ...
  private void ensureSection(long productId, DetailSectionFixture fixture) {
    List<Map<String, Object>> rows =
        jdbc.queryForList(
            "SELECT title,body,display_order,visible FROM product_detail_sections WHERE"
                + " product_id=? AND display_order=? FOR UPDATE",
            productId,
    ...
    if (rows.isEmpty()) {
      ...
      jdbc.update(
          "INSERT INTO"
              + " product_detail_sections(product_id,title,body,display_order,visible,created_at,updated_at)"
              + " VALUES (?,?,?,?,?,?,?)",
          productId,
          fixture.title(),
          fixture.body(),
    ...
    if (rows.size() != 1 || !matches(rows.getFirst(), fixture)) {
      throw fixtureError(
          "상품 상세 section 충돌: product_id=" + productId + ", displayOrder=" + fixture.displayOrder());
    }
  }
  ...
  private Map<String, List<DetailSectionFixture>> validateAndGroup(DetailSectionManifest manifest) {
    ...
    for (DetailSectionFixture section : manifest.sections()) {
      if (section == null
          || section.catalogKey() == null
          || !section.catalogKey().startsWith("demo-")
          || section.title() == null
          || section.title().isBlank()
          || section.title().length() > 200
          || section.body() == null
          || section.body().isBlank()
      ...
      grouped
          .computeIfAbsent(section.catalogKey(), ignored -> new java.util.ArrayList<>())
          .add(section);
    }
  ...
  private DetailSectionManifest loadFixture() {
    try {
      String location =
          fixtureLocation == null || fixtureLocation.isBlank()
              ? DEFAULT_FIXTURE_LOCATION
              : fixtureLocation;
  ...
  private CatalogManifestImportException fixtureError(String message, Throwable cause) {
    return new CatalogManifestImportException(
        "Demo Product Detail section fixture 오류: " + message, cause);
  }
  ...
  private record DetailSectionFixture(
      String catalogKey, String title, String body, int displayOrder, boolean visible) {}
}
