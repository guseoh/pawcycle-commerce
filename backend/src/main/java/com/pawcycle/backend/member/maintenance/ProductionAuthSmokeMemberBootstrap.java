...
public final class ProductionAuthSmokeMemberBootstrap {
  ...
  private static final List<String> ENFORCED_KEYS =
      ENFORCED_ARGUMENTS.stream().map(ProductionAuthSmokeMemberBootstrap::optionKey).toList();
  ...
  static int runIfRequested(
    ...
    if (!enabled.present() || enabled.ambiguous() || !"true".equals(enabled.value())) {
      error.println(ERROR_MESSAGE);
      return FAILURE;
    }
  ...
  private static ResolvedProperty resolve(
    ...
    if (!commandLineValues.isEmpty()) {
      return new ResolvedProperty(
          true, commandLineValues.get(commandLineValues.size() - 1), commandLineValues.size() != 1);
    }
