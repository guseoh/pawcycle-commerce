...
class RecommendationRepository {
  ...
  RecommendationRepository(JdbcTemplate jdbc) {
    this.jdbc = jdbc;
  }
        ...
        jdbc.query(
            ...
            (rs, row) ->
                new RecommendationCandidate(
                    ...
                    rs.getString(4),
                    rs.getString(5),
                    new RecommendationCandidate.Category(
                        rs.getLong(6), rs.getString(7), rs.getString(8)),
                    new RecommendationCandidate.Brand(
    ...
    return Set.copyOf(
        jdbc.queryForList(
            "SELECT DISTINCT sku.product_id FROM subscriptions subscription JOIN"
                ...
                + " subscription.pet_id=? AND subscription.mvp2_managed=true AND"
  ...
  MemberSignals memberSignals(long memberId, long petId) {
    ...
    Map<String, Integer> subscriptionCategories =
        categoryCounts(
            ...
            JOIN categories category ON category.id=product.category_id WHERE subscription.member_id=? AND subscription.pet_id=? AND subscription.mvp2_managed=true AND subscription.status='ACTIVE' GROUP BY category.slug\
            """,
            memberId,
            petId);
    Map<String, Integer> subscriptionBrands =
        categoryCounts(
            ...
            JOIN subscription_snapshot_items item ON item.snapshot_id=snapshot.id JOIN skus sku ON sku.id=item.sku_id JOIN products product ON product.id=sku.product_id
            JOIN brands brand ON brand.id=product.brand_id WHERE subscription.member_id=? AND subscription.pet_id=? AND subscription.mvp2_managed=true AND subscription.status='ACTIVE' GROUP BY brand.slug\
            """,
            memberId,
            petId);
  ...
  record TrendScore(long recent, long previous) {
    long delta() {
