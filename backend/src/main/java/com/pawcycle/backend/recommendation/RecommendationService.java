...
class RecommendationService {
  ...
  RecommendationResponse recommend(long memberId, long petId) {
    ...
    List<RecommendationCandidate> initialCandidates =
        sortedCandidates(petType, preferences, excluded, signals);
    ...
    int aiInputSize =
        exploration
            ? Math.min(9, initialCandidates.size())
            : Math.min(10, initialCandidates.size());
    List<RecommendationAiClient.AiRecommendation> aiRecommendations;
    boolean providerFallback = false;
    try {
      aiRecommendations =
          aiClient.recommend(List.copyOf(initialCandidates.subList(0, aiInputSize)), preferences);
    ...
    List<RecommendationCandidate> latestCandidates =
        sortedCandidates(petType, preferences, excluded, signals);
    if (latestCandidates.isEmpty()) {
      metrics.fallback();
      return new RecommendationResponse(UUID.randomUUID().toString(), List.of());
    }
    List<RecommendationCandidate> personalized =
        latestCandidates.subList(0, Math.min(desired, latestCandidates.size()));
    ...
    if (exploration && latestCandidates.size() > result.size()) {
      RecommendationCandidate explorationCandidate =
          explorationCandidate(memberId, petId, latestCandidates, result);
        ...
        result =
            append(
                ...
                toItem(
                    ...
                    fallbackReason(explorationCandidate, preferences),
                    "EXPLORATION"));
    }
    return new RecommendationResponse(UUID.randomUUID().toString(), result);
  }

  private List<RecommendationCandidate> sortedCandidates(
      String petType, List<String> preferences, Set<Long> excluded, MemberSignalsView signals) {
    ...
    return candidates.stream()
        .filter(candidate -> !excluded.contains(candidate.productId()))
        .filter(this::isNonMedical)
        .sorted(
            Comparator.comparingLong(
                    (RecommendationCandidate candidate) -> score(candidate, signals))
                .reversed()
                .thenComparing(
  ...
  private long score(RecommendationCandidate candidate, MemberSignalsView signals) {
    ...
    score +=
        Math.max(0, signals.value().purchases().getOrDefault(candidate.productId(), 0))
            * RecommendationPolicy.PURCHASE_PRODUCT;
    score +=
        Math.max(0, signals.value().wishlists().getOrDefault(candidate.productId(), 0))
            * RecommendationPolicy.WISHLIST_PRODUCT;
    score +=
        RecommendationPolicy.cap(signals.value().clicks().getOrDefault(candidate.productId(), 0))
            * RecommendationPolicy.CLICK_PRODUCT;
    score +=
        RecommendationPolicy.cap(signals.value().views().getOrDefault(candidate.productId(), 0))
            * RecommendationPolicy.VIEW_PRODUCT;
    score +=
        contentScore(
            signals.value().purchaseCategories(), category, RecommendationPolicy.PURCHASE_CATEGORY);
    score +=
        contentScore(
            signals.value().wishlistCategories(), category, RecommendationPolicy.WISHLIST_CATEGORY);
    score +=
        contentScore(
            signals.value().subscriptionCategories(),
            category,
            RecommendationPolicy.SUBSCRIPTION_CATEGORY);
    score +=
        contentScore(
            signals.value().filterCategories(),
            category,
            RecommendationPolicy.FILTER_CATEGORY,
            true);
    score +=
        contentScore(signals.value().purchaseBrands(), brand, RecommendationPolicy.PURCHASE_BRAND);
    score +=
        contentScore(signals.value().wishlistBrands(), brand, RecommendationPolicy.WISHLIST_BRAND);
    score +=
        contentScore(
            signals.value().subscriptionBrands(), brand, RecommendationPolicy.SUBSCRIPTION_BRAND);
    score +=
        contentScore(
            signals.value().filterBrands(), brand, RecommendationPolicy.FILTER_BRAND, true);
    for (String facet : candidate.facets()) {
      score +=
          contentScore(
              signals.value().purchaseFacets(), facet, RecommendationPolicy.PURCHASE_FACET);
      score +=
          contentScore(
              signals.value().wishlistFacets(), facet, RecommendationPolicy.WISHLIST_FACET);
      score +=
          contentScore(
              signals.value().subscriptionFacets(), facet, RecommendationPolicy.SUBSCRIPTION_FACET);
      score +=
          contentScore(
              signals.value().filterFacets(), facet, RecommendationPolicy.FILTER_FACET, true);
    }
    return score;
  }

  private long contentScore(Map<String, Integer> values, String key, int weight) {
    return contentScore(values, key, weight, false);
  }

  private long contentScore(Map<String, Integer> values, String key, int weight, boolean capped) {
    if (key == null) return 0;
    int occurrences = Math.max(0, values.getOrDefault(key, 0));
    return (long) (capped ? RecommendationPolicy.cap(occurrences) : occurrences) * weight;
  }
  ...
  private List<RecommendationItem> mergeAiAndFallback(
    ...
    for (RecommendationAiClient.AiRecommendation ai :
        aiRecommendations == null
            ? List.<RecommendationAiClient.AiRecommendation>of()
            : aiRecommendations) {
    ...
    for (RecommendationCandidate candidate : general) {
      ...
      if (selected.add(candidate.productId())) {
        fallbackUsed = true;
        result.add(toItem(candidate, fallbackReason(candidate, preferences), strategy));
      }
    }
  ...
  private RecommendationCandidate explorationCandidate(
      long memberId,
      long petId,
    ...
    List<RecommendationCandidate> choices =
        candidates.stream().filter(candidate -> !ids.contains(candidate.productId())).toList();
    if (choices.isEmpty()) return null;
    Set<Long> exposed = safeSet(repository.exposedProductIds(memberId, 7));
    List<RecommendationCandidate> fresh =
        choices.stream().filter(candidate -> !exposed.contains(candidate.productId())).toList();
  ...
  private List<RecommendationItem> append(List<RecommendationItem> items, RecommendationItem item) {
    List<RecommendationItem> result = new ArrayList<>(items);
    result.add(item);
    return List.copyOf(result);
  }
  ...
  private List<String> preferences(long memberId, long petId) {
    ...
    addAll(all, repository.subscriptionCategorySlugs(memberId, petId));
    addAll(all, repository.purchaseCategorySlugs(memberId));
    addAll(all, repository.wishlistCategorySlugs(memberId));
  ...
  private boolean isNonMedical(RecommendationCandidate candidate) {
    return containsNoMedicalTerm(candidate.name())
        ...
        && (candidate.category() == null
            || (containsNoMedicalTerm(candidate.category().name())
                && containsNoMedicalTerm(candidate.category().slug())))
        && (candidate.brand() == null
            || (containsNoMedicalTerm(candidate.brand().name())
                && containsNoMedicalTerm(candidate.brand().slug())));
  }
  ...
  private ReasonSelection safeReason(
      String reason, RecommendationCandidate candidate, List<String> preferences) {
    if (reason == null
        || reason.isBlank()
        || reason.length() > 100
        || !containsNoMedicalTerm(reason)
        || reason
            .chars()
            .noneMatch(
                character ->
