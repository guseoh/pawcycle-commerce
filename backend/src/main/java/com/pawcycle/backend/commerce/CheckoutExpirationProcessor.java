...
class CheckoutExpirationProcessor {
  ...
  public boolean expire(long paymentId) {
    Map<String, Object> payment =
        one(
            "SELECT payment.id,payment.order_id,payment.status,orders.status AS order_status FROM"
                + " payments payment JOIN orders ON orders.id=payment.order_id WHERE payment.id=?"
                + " FOR UPDATE",
            paymentId);
    if (payment == null
        || !"READY".equals(payment.get("status"))
        || !"PAYMENT_PENDING".equals(payment.get("order_status"))) return false;
    ...
    jdbc.update(
        "UPDATE payments SET"
            + " status='FAILED',provider_status='EXPIRED',failure_code='CHECKOUT_EXPIRED',failed_at=?"
            + " WHERE id=?",
        now,
        paymentId);
