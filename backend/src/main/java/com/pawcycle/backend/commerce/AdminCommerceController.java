...
class AdminCommerceController {
	private final CommerceService commerce; private final AdminCommerceService adminCommerce; private final AdminAuditService audits; private final DeliveryService deliveries; private final ReturnService returns; private final RefundService refunds; private final PaymentReconciliationService payments; private final SubscriptionBillingService billing; private final AdminOrderQueryService orders; private final OperationsQueryService operations;
	AdminCommerceController(CommerceService commerce,AdminCommerceService adminCommerce,AdminAuditService audits,DeliveryService deliveries,ReturnService returns,RefundService refunds,PaymentReconciliationService payments,SubscriptionBillingService billing,AdminOrderQueryService orders,OperationsQueryService operations){this.commerce=commerce;this.adminCommerce=adminCommerce;this.audits=audits;this.deliveries=deliveries;this.returns=returns;this.refunds=refunds;this.payments=payments;this.billing=billing;this.orders=orders;this.operations=operations;}
	@GetMapping("/inventories") List<Map<String,Object>> inventories(){return commerce.inventories();}
	@PostMapping("/inventories/{skuId}/adjustments") ResponseEntity<Void> adjust(@AuthenticationPrincipal AuthenticatedMemberPrincipal p,@PathVariable long skuId,@Valid @RequestBody CommerceRequests.Adjustment r){adminCommerce.adjustInventory(p.memberId(),skuId,r.delta());return ResponseEntity.noContent().build();}
	@GetMapping("/coupons") List<Map<String,Object>> coupons(){return commerce.coupons();}
	@PostMapping("/coupons") ResponseEntity<Map<String,Object>> createCoupon(@AuthenticationPrincipal AuthenticatedMemberPrincipal p,@Valid @RequestBody CommerceRequests.Coupon request){long id=adminCommerce.createCoupon(p.memberId(),request.legacyPayload());return ResponseEntity.created(URI.create("/api/admin/coupons/"+id)).body(Map.of("couponId",id));}
	@PatchMapping("/coupons/{couponId}") ResponseEntity<Void> patchCoupon(@AuthenticationPrincipal AuthenticatedMemberPrincipal p,@PathVariable long couponId,@Valid @RequestBody CommerceRequests.Coupon request){adminCommerce.updateCoupon(p.memberId(),couponId,request.legacyPayload());return ResponseEntity.noContent().build();}
	@PostMapping("/coupons/{couponId}/issues") ResponseEntity<Void> issueCoupon(@AuthenticationPrincipal AuthenticatedMemberPrincipal p,@PathVariable long couponId,@Valid @RequestBody CommerceRequests.CouponIssue request){adminCommerce.issueCoupon(p.memberId(),couponId,request.memberId());return ResponseEntity.noContent().build();}
	@GetMapping("/membership-grades") List<Map<String,Object>> grades(){return commerce.membershipGrades();}
	@PostMapping("/membership-grades") ResponseEntity<Map<String,Object>> createGrade(@AuthenticationPrincipal AuthenticatedMemberPrincipal p,@Valid @RequestBody CommerceRequests.MembershipGrade request){long id=adminCommerce.createMembershipGrade(p.memberId(),request.legacyPayload());return ResponseEntity.created(URI.create("/api/admin/membership-grades/"+id)).body(Map.of("gradeId",id));}
	@PostMapping("/members/{memberId}/membership/evaluate") ResponseEntity<Void> evaluate(@AuthenticationPrincipal AuthenticatedMemberPrincipal p,@PathVariable long memberId){adminCommerce.evaluateMembership(p.memberId(),memberId);return ResponseEntity.noContent().build();}
