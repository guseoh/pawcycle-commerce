...
class CheckoutController {
    ...
    Map<String, Object> result =
        new LinkedHashMap<>(
            checkoutIdempotencyService.checkout(
                p.memberId(), key, r.addressId(), r.memberCouponId(), r.cartVersion()));
  ...
  Map<String, Object> confirm(
      @AuthenticationPrincipal AuthenticatedMemberPrincipal p,
      @Valid @RequestBody PaymentConfirmRequest r) {
    return commerce.confirm(p.memberId(), r.paymentKey(), r.providerOrderId(), r.amount());
  }

  @GetMapping("/orders")
  java.util.List<Map<String, Object>> orders(
      @AuthenticationPrincipal AuthenticatedMemberPrincipal p) {
    return commerce.orders(p.memberId());
  }

  @GetMapping("/orders/{orderId}")
  Map<String, Object> order(
      @AuthenticationPrincipal AuthenticatedMemberPrincipal p, @PathVariable long orderId) {
    return commerce.order(p.memberId(), orderId);
  }

  @PostMapping("/orders/{orderId}/reorder")
  Map<String, Object> reorder(
      @AuthenticationPrincipal AuthenticatedMemberPrincipal p,
      @PathVariable long orderId,
      @RequestHeader("Idempotency-Key") String key) {
    return commerce.reorder(p.memberId(), orderId, key);
  }
}
