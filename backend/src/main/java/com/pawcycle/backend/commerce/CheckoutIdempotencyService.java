...
public class CheckoutIdempotencyService {
    ...
    public Map<String, Object> checkout(long memberId, String idempotencyKey, long addressId, Long memberCouponId, Long requestedCartVersion) {
        ...
        long requestCartVersion = requestedCartVersion == null ? currentCartVersion : requestedCartVersion;
        Map<String, Object> result = commerce.checkout(memberId, idempotencyKey, addressId, memberCouponId, requestCartVersion);
        int updated = jdbc.update("""
                ...
                """, requestCartVersion, memberId, idempotencyKey);
        if (updated != 1) {
    ...
    private void verifyReplayIdentity(Map<String, Object> replay, long addressId, Long memberCouponId, Long requestedCartVersion) {
        ...
        long fingerprintCartVersion = requestedCartVersion == null ? originalCartVersion : requestedCartVersion;
    ...
    private long lockCurrentCartVersion(long memberId) {
        Map<String, Object> cart = one("SELECT version FROM carts WHERE member_id=? FOR UPDATE", memberId);
    ...
    private Map<String, Object> checkoutResponse(Map<String, Object> row) {
        ...
        List<Map<String, Object>> items = jdbc.queryForList(
                "SELECT product_name_snapshot AS product_name FROM order_items WHERE order_id=? ORDER BY id", orderId);
