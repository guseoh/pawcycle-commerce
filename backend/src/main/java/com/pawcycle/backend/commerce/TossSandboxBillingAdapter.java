...
class TossSandboxBillingAdapter implements TossBillingAdapter {
  @Override
  public BillingKeyResult issueBillingKey(String customerKey, String authKey) {
    if (authKey.startsWith("fail_"))
      throw new CommerceException(
          503, "PAYMENT_PROVIDER_UNAVAILABLE", "Toss Billing Provider가 현재 요청을 처리할 수 없습니다.");
    return new BillingKeyResult("sandbox-billing-" + UUID.randomUUID());
  }

  @Override
  public ChargeResult charge(String billingKey, String providerOrderId, BigDecimal amount) {
    return outcome(providerOrderId);
  }

  @Override
  public ChargeResult queryCharge(String providerOrderId) {
    return outcome(providerOrderId);
  }
