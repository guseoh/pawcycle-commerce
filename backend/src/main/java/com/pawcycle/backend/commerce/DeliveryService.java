...
public class DeliveryService {
	...
	public Map<String,Object> ship(Long adminId,long id,String carrier,String tracking) {
		...
		try { return tx.execute(status -> {
			...
			long member = jdbc.queryForObject("SELECT member_id FROM orders WHERE id=?", Long.class, ((Number)row.get("order_id")).longValue());
	...
	public Map<String,Object> complete(Long adminId,long id) { return transition(adminId,id,"SHIPPED","DELIVERED",null,"ORDER_DELIVERED","DELIVERY_COMPLETE"); }
	public Map<String,Object> fail(long id,String reason) { return fail(null,id,reason); }
	...
	private Map<String,Object> transition(Long adminId,long id,String from,String to,String failure,String notification,String auditAction) {
		...
		try { return tx.execute(status -> {
			...
			jdbc.update("UPDATE deliveries SET status=?,failure_reason=?," + column + "=? WHERE id=?", to, failure, now, id);
