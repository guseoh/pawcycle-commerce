...
public class InventoryService {
  ...
  public void adjust(long skuId, int delta) {
    ...
    if (available + delta < 0)
      throw new CommerceException(409, "INVENTORY_INSUFFICIENT", "재고가 부족합니다.");
    jdbc.update(
        "UPDATE inventories SET available_quantity=available_quantity+?,version=version+1 WHERE"
            + " sku_id=?",
        delta,
        skuId);
    if (delta != 0)
  ...
  private void restore(long skuId, int quantity, long sourceId, String type, String sourceColumn) {
    ...
    String sql =
        "INSERT INTO"
            + " inventory_movements(sku_id,payment_id,type,quantity,available_before,available_after,reserved_before,reserved_after,"
            + sourceColumn
            + ",source_id,created_at) VALUES (?,NULL,?,?,?,?,?,?,?,?,?)";
    jdbc.update(
        sql,
        skuId,
        type,
  ...
  private Map<String, Object> lockForAdjustment(long skuId) {
    Map<String, Object> inventory =
        one(
            ...
            skuId);
    if (inventory == null)
      throw new CommerceException(404, "INVENTORY_NOT_FOUND", "요청한 리소스를 찾을 수 없습니다.");
    return inventory;
  }
