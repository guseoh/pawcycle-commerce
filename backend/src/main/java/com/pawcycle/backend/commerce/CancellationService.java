    ...
    return tx.execute(
        status -> {
          ...
          Map<String, Object> existing =
              one(
                  "SELECT cancellation.id AS"
                      + " cancellationId,cancellation.status,cancellation.reason,cancellation.requested_at"
                      + " AS requestedAt,cancellation.completed_at AS completedAt FROM"
                      + " order_cancellations cancellation WHERE cancellation.order_id=? FOR"
                      + " UPDATE",
                  orderId);
          if (existing != null) return existing;
          Map<String, Object> delivery =
              one("SELECT id,status FROM deliveries WHERE order_id=? FOR UPDATE", orderId);
          ...
          jdbc.update(
              "UPDATE deliveries SET status='CANCELLED',cancelled_at=? WHERE id=?",
              now,
              delivery.get("id"));
          restore(orderId, id);
          jdbc.update(
              "INSERT INTO"
                  + " refunds(order_id,source,cancellation_id,status,amount,provider,idempotency_key,attempt_no,requested_at)"
                  + " SELECT ?,'CANCELLATION',?,'READY',payment_amount,'TOSS',?,1,? FROM orders"
                  + " WHERE id=?",
              orderId,
              id,
