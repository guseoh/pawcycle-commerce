...
public class NotificationService {
	...
	public List<Map<String,Object>> list(long memberId) { List<Map<String,Object>> result=jdbc.queryForList("SELECT id AS notificationId,type,reference_type AS referenceType,reference_id AS referenceId,read_at AS readAt,created_at AS createdAt FROM notifications WHERE member_id=? ORDER BY id DESC",memberId); for(Map<String,Object> item:result){if("SUBSCRIPTION_DELIVERY_REMINDER".equals(item.get("type"))){List<Map<String,Object>> context=jdbc.queryForList("SELECT subscription_id AS subscriptionId,scheduled_date AS scheduledDate FROM subscription_schedules WHERE id=?",item.get("referenceId"));if(!context.isEmpty()){item.put("subscriptionId",context.getFirst().get("subscriptionId"));item.put("scheduledDate",context.getFirst().get("scheduledDate"));}}}return result; }
