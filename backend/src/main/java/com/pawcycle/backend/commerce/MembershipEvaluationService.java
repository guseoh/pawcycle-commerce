...
public class MembershipEvaluationService {
	...
	public MembershipEvaluationService(JdbcTemplate jdbc, Clock clock) { this.jdbc = jdbc; this.clock = clock; }
	...
	public void evaluate(long memberId) {
		...
		Timestamp now = now();
		BigDecimal amount = jdbc.queryForObject("SELECT COALESCE(SUM(payment_amount),0) FROM orders WHERE member_id=? AND status='PAID' AND paid_at>=?",
				BigDecimal.class, memberId, Timestamp.from(clock.instant().minus(365, ChronoUnit.DAYS)));
		...
		if (before == null) jdbc.update("INSERT INTO member_memberships(member_id,grade_id,evaluated_purchase_amount,evaluated_at) VALUES (?,?,?,?)", memberId, number(grade, "id"), amount, now);
		else jdbc.update("UPDATE member_memberships SET grade_id=?,evaluated_purchase_amount=?,evaluated_at=? WHERE member_id=?", number(grade, "id"), amount, now, memberId);
		if (before == null || number(before, "grade_id") != number(grade, "id")) {
			jdbc.update("INSERT INTO membership_histories(member_id,from_grade_id,to_grade_id,evaluated_purchase_amount,changed_at) VALUES (?,?,?,?,?)", memberId, before == null ? null : number(before, "grade_id"), number(grade, "id"), amount, now);
			if (grade.get("benefit_coupon_id") != null) jdbc.update("INSERT INTO member_coupons(member_id,coupon_id,status,issued_at) VALUES (?,?,'AVAILABLE',?)", memberId, number(grade, "benefit_coupon_id"), now);
		}
	}
