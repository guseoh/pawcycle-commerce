 ...
 */
@Service
public class LegacyMvp2MigrationService {
	private static final long JSON_SAFE_MAX = 9_007_199_254_740_991L;
	private final JdbcTemplate jdbc;

	public LegacyMvp2MigrationService(JdbcTemplate jdbc) { this.jdbc = jdbc; }
	...
	public LegacyPreflight preflight() {
		List<Map<String, Object>> invalid = jdbc.queryForList("""
			...
			""", JSON_SAFE_MAX);
		return new LegacyPreflight(invalid.isEmpty(), invalid.size());
	}
	...
	public void migrateAfterSourceWriteFreeze(boolean sourceWritesFrozen) {
		if (!sourceWritesFrozen) throw new IllegalStateException("legacy source writes must be frozen before migration");
		LegacyPreflight preflight = preflight();
	...
	private void migrateOne(Map<String, Object> row) {
		...
		jdbc.update("INSERT INTO plan_items(plan_version_id,sku_id,quantity) VALUES (?,?,?)", versionId, skuId, quantity);
		jdbc.update("INSERT INTO plan_version_delivery_cycles(plan_version_id,delivery_cycle_weeks) VALUES (?,?)", versionId, cycle);
		jdbc.update("UPDATE subscription_plans SET current_plan_version_id=? WHERE id=?", versionId, planId);
		jdbc.update("INSERT INTO subscription_snapshots(subscription_id,source_plan_version_id,package_total_krw,delivery_cycle_weeks) VALUES (?,?,?,?)", subscriptionId, versionId, total, cycle);
		long snapshotId = jdbc.queryForObject("SELECT LAST_INSERT_ID()", Long.class);
		jdbc.update("INSERT INTO subscription_snapshot_items(snapshot_id,sku_id,quantity) VALUES (?,?,?)", snapshotId, skuId, quantity);
