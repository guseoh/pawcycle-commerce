...
public class SubscriptionExceptionHandler {
  ...
  private static final Set<String> REQUEST_FIELDS =
      Set.of("skuId", "quantity", "deliveryCycleWeeks");
  ...
  ResponseEntity<ApiErrorResponse> handleValidation(MethodArgumentNotValidException exception) {
    ...
    return ResponseEntity.badRequest()
        .body(new ApiErrorResponse("VALIDATION_FAILED", "입력 내용을 확인해 주세요.", fieldErrors));
  }
            ...
            new ApiErrorResponse(
                ...
                List.of(
                    new FieldErrorResponse(
                        resolveJsonErrorField(exception), "요청 값의 형식을 확인해 주세요."))));
  }
  ...
  ResponseEntity<ApiErrorResponse> handleSkuNotFound(SkuNotFoundException exception) {
    return ResponseEntity.status(HttpStatus.NOT_FOUND)
        .body(ApiErrorResponse.withoutFieldErrors("SKU_NOT_FOUND", exception.getMessage()));
  }
  ...
  ResponseEntity<ApiErrorResponse> handleSkuNotSubscribable(SkuNotSubscribableException exception) {
    return ResponseEntity.status(HttpStatus.CONFLICT)
        .body(ApiErrorResponse.withoutFieldErrors("SKU_NOT_SUBSCRIBABLE", exception.getMessage()));
  }

  @ExceptionHandler(SubscriptionNotFoundException.class)
  ResponseEntity<ApiErrorResponse> handleSubscriptionNotFound(
      SubscriptionNotFoundException exception) {
    return ResponseEntity.status(HttpStatus.NOT_FOUND)
        .body(
            ApiErrorResponse.withoutFieldErrors("SUBSCRIPTION_NOT_FOUND", exception.getMessage()));
  }
  ...
  ResponseEntity<ApiErrorResponse> handleCreateFailed(SubscriptionCreateFailedException exception) {
    log.error("Unexpected exception while creating subscription", exception);
    return ResponseEntity.internalServerError()
        .body(ApiErrorResponse.withoutFieldErrors("SUBSCRIPTION_CREATE_FAILED", "구독을 생성하지 못했습니다."));
  }

  @ExceptionHandler(SubscriptionListUnavailableException.class)
  ResponseEntity<ApiErrorResponse> handleListUnavailable(
      SubscriptionListUnavailableException exception) {
    log.error("Unexpected exception while querying subscription list", exception);
    return ResponseEntity.internalServerError()
        .body(
  ...
  ResponseEntity<ApiErrorResponse> handleDetailUnavailable(
      SubscriptionDetailUnavailableException exception) {
    log.error("Unexpected exception while querying subscription detail", exception);
    return ResponseEntity.internalServerError()
        .body(
            ApiErrorResponse.withoutFieldErrors(
