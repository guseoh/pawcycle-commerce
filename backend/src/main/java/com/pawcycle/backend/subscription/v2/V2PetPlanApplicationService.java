...
class V2PetPlanApplicationService {
	...
	private Map<String,Object> pet(V2SubscriptionData.Pet pet){Map<String,Object> result=new LinkedHashMap<>();result.put("petId",pet.id());result.put("name",pet.name());result.put("petType",pet.petType());result.put("breed",pet.breed());result.put("weightKg",pet.weightKg());result.put("profileComplete",pet.profileComplete());return result;}
	private String nullableText(Object value,String field,int max){if(value==null)return null;if(!(value instanceof String))throw support.validation(field);String text=((String)value).trim();if(text.isBlank()||text.codePointCount(0,text.length())>max||text.chars().anyMatch(Character::isISOControl))throw support.validation(field);return text;}
	private BigDecimal nullableWeight(Object value){if(value==null)return null;if(!(value instanceof Number))throw support.validation("weightKg");try{BigDecimal weight=new BigDecimal(value.toString());if(weight.scale()>2)throw support.validation("weightKg");weight=weight.setScale(2, RoundingMode.UNNECESSARY);if(weight.signum()<=0||weight.compareTo(new BigDecimal("200.00"))>0)throw support.validation("weightKg");return weight;}catch(NumberFormatException|ArithmeticException exception){throw support.validation("weightKg");}}
