...
public class V2SubscriptionService {
	...
	private Map<String,Object> availableVersion(Map<String,Object> pet,long versionId,int cycle) { LocalDate today=today(); Map<String,Object> v=one("SELECT p.id plan_id,p.name plan_name,p.current_plan_version_id,v.id version_id,p.target_pet_type,p.on_sale,p.sale_starts_on,p.sale_ends_on,v.package_price_krw,v.is_migration_only FROM subscription_plans p JOIN plan_versions v ON v.plan_id=p.id WHERE v.id=?",versionId).orElseThrow(() -> new V2ApiException(404,"PLAN_VERSION_NOT_FOUND","PlanVersion을 찾을 수 없습니다.")); if(!pet.get("pet_type").equals(v.get("target_pet_type"))) throw new V2ApiException(409,"PLAN_PET_TYPE_MISMATCH","Pet 종과 Plan이 호환되지 않습니다."); if(longValue(v,"current_plan_version_id") != versionId || v.get("plan_name")==null || !Boolean.TRUE.equals(v.get("on_sale")) || Boolean.TRUE.equals(v.get("is_migration_only")) || (v.get("sale_starts_on") != null && ((java.sql.Date)v.get("sale_starts_on")).toLocalDate().isAfter(today)) || (v.get("sale_ends_on") != null && ((java.sql.Date)v.get("sale_ends_on")).toLocalDate().isBefore(today))) throw new V2ApiException(409,"PLAN_NOT_AVAILABLE","판매 가능한 PlanVersion이 아닙니다."); Integer allowed=jdbc.queryForObject("SELECT COUNT(*) FROM plan_version_delivery_cycles WHERE plan_version_id=? AND delivery_cycle_weeks=?",Integer.class,versionId,cycle); if(allowed==0) throw new V2ApiException(409,"DELIVERY_CYCLE_NOT_ALLOWED","허용되지 않은 배송 주기입니다."); return v; }
	...
	private Object canonical(Object value) { if(value instanceof Map<?,?> map) { Map<String,Object> sorted=new java.util.TreeMap<>(); map.forEach((key,item) -> sorted.put(String.valueOf(key),canonical(item))); return sorted; } if(value instanceof List<?> list) return list.stream().map(this::canonical).toList(); if(value instanceof Number number) return new java.math.BigDecimal(number.toString()).stripTrailingZeros(); return value; }
