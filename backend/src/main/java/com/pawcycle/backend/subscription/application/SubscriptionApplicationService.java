...
public class SubscriptionApplicationService {
  ...
  public SubscriptionCreateResult create(
      Long memberId, Long skuId, int quantity, int deliveryCycleWeeks) {
    try {
      ...
      Subscription subscription =
          subscriptionRepository.save(
              Subscription.create(member, sku, quantity, deliveryCycleWeeks, createdDate));
  ...
  public SubscriptionListView findOwnedSubscriptions(Long memberId) {
    try {
      List<SubscriptionSummary> subscriptions =
          subscriptionRepository.findAllOwnedWithCatalogOrderByIdDesc(memberId).stream()
              .map(this::toSummary)
              .toList();
  ...
  public SubscriptionDetailView findOwnedSubscription(Long memberId, Long subscriptionId) {
    try {
      return subscriptionRepository
          .findOwnedWithCatalog(subscriptionId, memberId)
          .map(this::toDetail)
