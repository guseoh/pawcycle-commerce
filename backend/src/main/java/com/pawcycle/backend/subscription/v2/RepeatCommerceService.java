...
public class RepeatCommerceService {
	...
	private int chooseCycle(List<Integer> allowed,long medianDays,int current){long closestDistance=allowed.stream().mapToLong(value->Math.abs(value*7L-medianDays)).min().orElse(Long.MAX_VALUE);List<Integer> tied=allowed.stream().filter(value->Math.abs(value*7L-medianDays)==closestDistance).toList();if(tied.contains(current))return current;return tied.stream().max(Integer::compareTo).orElse(current);}
