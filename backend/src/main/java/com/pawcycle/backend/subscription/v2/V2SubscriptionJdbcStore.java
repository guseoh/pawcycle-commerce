...
class V2SubscriptionJdbcStore {
  ...
  V2SubscriptionData.PlanVersion findPlanVersion(long versionId) {
    return one(
        ...
        .orElseThrow(
            () -> new V2ApiException(404, "PLAN_VERSION_NOT_FOUND", "PlanVersion을 찾을 수 없습니다."));
  }
  ...
  V2SubscriptionData.AddonSku findEligibleAddonSku(long skuId) {
    return one(
        ...
        .map(
            row ->
                new V2SubscriptionData.AddonSku(
                    skuId,
  ...
  long insertSubscription(
    ...
    jdbc.update(
        "INSERT INTO"
            + " subscriptions(member_id,sku_id,quantity,delivery_cycle_weeks,created_date,next_order_date,pet_id,status,version,current_snapshot_id,legacy_api_visible,mvp2_managed)"
  ...
  V2SubscriptionData.Subscription lockOwnedSubscription(long memberId, long subscriptionId) {
    return one(
            "SELECT id,member_id,status,version,pet_id,delivery_cycle_weeks,current_snapshot_id"
                + " FROM subscriptions WHERE id=? AND member_id=? AND mvp2_managed=true FOR UPDATE",
            subscriptionId,
            memberId)
        .map(this::subscription)
        .orElseThrow(
            () -> new V2ApiException(404, "SUBSCRIPTION_NOT_FOUND", "Subscription을 찾을 수 없습니다."));
  }

  V2SubscriptionData.Subscription findOwnedSubscription(long memberId, long subscriptionId) {
    return one(
            "SELECT id,member_id,status,version,pet_id,delivery_cycle_weeks,current_snapshot_id"
                + " FROM subscriptions WHERE id=? AND member_id=? AND mvp2_managed=true",
            subscriptionId,
            memberId)
        .map(this::subscription)
        .orElseThrow(
  ...
  void updatePet(
    ...
    if (jdbc.update(
            ...
            arguments.toArray())
        != 1) throw new V2ApiException(404, "PET_NOT_FOUND", "Pet을 찾을 수 없습니다.");
  }

  V2SubscriptionData.Page<V2SubscriptionData.Pet> findPets(long memberId, int page, int size) {
    long total =
        jdbc.queryForObject("SELECT COUNT(*) FROM pets WHERE member_id=?", Long.class, memberId);
    List<V2SubscriptionData.Pet> items =
        jdbc.query(
            "SELECT id,name,pet_type,breed,weight_kg FROM pets WHERE member_id=? ORDER BY id ASC"
                + " LIMIT ? OFFSET ?",
            (rs, n) ->
                new V2SubscriptionData.Pet(
                    rs.getLong("id"),
                    rs.getString("name"),
            ...
            size,
            page * size);
    return new V2SubscriptionData.Page<>(page, size, total, items);
  }

  V2SubscriptionData.Page<V2SubscriptionData.PlanVersion> findSalePlanVersions(
    ...
    List<V2SubscriptionData.PlanVersion> items =
        jdbc.query(
            "SELECT p.id plan_id,p.name"
                + " plan_name,p.current_plan_version_id,p.target_pet_type,p.on_sale,p.sale_starts_on,p.sale_ends_on,v.id"
            ...
            (rs, n) ->
                new V2SubscriptionData.PlanVersion(
                    rs.getLong("plan_id"),
                    rs.getString("plan_name"),
                    (Long) rs.getObject("current_plan_version_id"),
  ...
  List<V2SubscriptionData.Item> findPlanItems(long versionId) {
    return jdbc.query(
        "SELECT sku_id,quantity FROM plan_items WHERE plan_version_id=? ORDER BY sku_id",
        (rs, n) -> new V2SubscriptionData.Item(rs.getLong("sku_id"), rs.getInt("quantity")),
        versionId);
  }
  ...
  Map<Long, List<V2SubscriptionData.Item>> findPlanItems(List<Long> versionIds) {
    return groupedItems(
  ...
  V2SubscriptionData.Snapshot findSnapshot(long snapshotId) {
    V2SubscriptionData.Snapshot base =
        one(
            ...
            .map(
                row ->
                    new V2SubscriptionData.Snapshot(
                        longValue(row, "id"),
                        ...
                        List.of()))
            .orElseThrow();
    return new V2SubscriptionData.Snapshot(
        base.id(),
        base.planVersionId(),
  ...
  private List<V2SubscriptionData.Item> findPlanItemsForSnapshot(long snapshotId) {
    return jdbc.query(
  ...
  V2SubscriptionData.StoredIdempotencyResult lockCommandResult(
  ...
  V2SubscriptionData.Schedule lockNextScheduled(long subscriptionId) {
    return one(
        ...
        .map(
            row ->
                new V2SubscriptionData.Schedule(
                    longValue(row, "id"), date(row.get("scheduled_date"))))
        .orElseThrow(
            () ->
                new V2ApiException(409, "SUBSCRIPTION_COMMAND_NOT_ALLOWED", "다음 Schedule이 없습니다."));
  }
  ...
  Optional<V2SubscriptionData.PendingChange> findPendingChange(long subscriptionId) {
    return one(
        ...
        .map(
            row ->
                new V2SubscriptionData.PendingChange(
  ...
  void deleteScheduleAddon(long scheduleId, long skuId) {
    if (jdbc.update(
            ...
            scheduleId,
            skuId)
        != 1) throw new V2ApiException(404, "ADDON_NOT_FOUND", "Add-on을 찾을 수 없습니다.");
  }
  ...
  List<V2SubscriptionData.ScheduleAddon> findScheduleAddons(long scheduleId) {
    return jdbc.query(
  ...
  private V2SubscriptionData.Pet pet(Map<String, Object> row) {
    return new V2SubscriptionData.Pet(
        longValue(row, "id"),
        (String) row.get("name"),
  ...
  private V2SubscriptionData.PlanVersion planVersion(Map<String, Object> row) {
    return new V2SubscriptionData.PlanVersion(
  ...
  private V2SubscriptionData.Subscription subscription(Map<String, Object> row) {
    return new V2SubscriptionData.Subscription(
        longValue(row, "id"),
  ...
  private V2SubscriptionData.StoredIdempotencyResult storedIdempotency(Map<String, Object> row) {
    return new V2SubscriptionData.StoredIdempotencyResult(
  ...
  V2SubscriptionData.Page<V2SubscriptionData.Subscription> findSubscriptions(
      long memberId, int page, int size) {
    long total =
        jdbc.queryForObject(
            "SELECT COUNT(*) FROM subscriptions WHERE member_id=? AND mvp2_managed=true",
            Long.class,
            memberId);
    List<V2SubscriptionData.Subscription> items =
        jdbc.query(
            "SELECT id,member_id,status,version,pet_id,delivery_cycle_weeks,current_snapshot_id"
                + " FROM subscriptions WHERE member_id=? AND mvp2_managed=true ORDER BY id DESC"
                + " LIMIT ? OFFSET ?",
            (rs, n) ->
                new V2SubscriptionData.Subscription(
                    rs.getLong("id"),
                    rs.getLong("member_id"),
                    rs.getString("status"),
                    rs.getLong("version"),
                    (Long) rs.getObject("pet_id"),
  ...
  Map<Long, V2SubscriptionData.Pet> findOwnedPets(long memberId, List<Long> ids) {
    if (ids.isEmpty()) return Map.of();
    Map<Long, V2SubscriptionData.Pet> result = new HashMap<>();
    jdbc.query(
  ...
  Map<Long, V2SubscriptionData.SnapshotBase> findSnapshots(List<Long> ids) {
    if (ids.isEmpty()) return Map.of();
    Map<Long, V2SubscriptionData.SnapshotBase> result = new HashMap<>();
    jdbc.query(
        "SELECT id,source_plan_version_id,package_total_krw,delivery_cycle_weeks FROM"
        ...
        rs -> {
          result.put(
              rs.getLong("id"),
              new V2SubscriptionData.SnapshotBase(
                  rs.getLong("id"),
  ...
  Map<Long, List<V2SubscriptionData.Item>> findSnapshotItems(List<Long> snapshotIds) {
    return groupedItems(
  ...
  Optional<V2SubscriptionData.NextDeliverySchedule> findNextDeliverySchedule(long subscriptionId) {
    return one(
            "SELECT"
  ...
  List<V2SubscriptionData.ItemDetail> findSnapshotItemDetails(long snapshotId) {
    return jdbc.query(
        ...
        (rs, n) ->
            new V2SubscriptionData.ItemDetail(
                rs.getLong("sku_id"),
  ...
  V2SubscriptionData.Page<V2SubscriptionData.ScheduleView> findScheduleViews(
    ...
    long total =
        jdbc.queryForObject(
            ...
            Long.class,
            subscriptionId);
    List<V2SubscriptionData.ScheduleView> items =
        jdbc.query(
            ...
            (rs, n) ->
                new V2SubscriptionData.ScheduleView(
                    rs.getLong("id"),
                    rs.getDate("scheduled_date").toLocalDate(),
                    rs.getString("status"),
                    (Long) rs.getObject("effective_snapshot_id")),
            subscriptionId,
            size,
            page * size);
    return new V2SubscriptionData.Page<>(page, size, total, items);
  ...
  V2SubscriptionData.Page<V2SubscriptionData.CommandHistory> findCommandHistory(
      long subscriptionId, int page, int size) {
    long total =
        jdbc.queryForObject(
            ...
            Long.class,
            subscriptionId);
    List<V2SubscriptionData.CommandHistory> items =
        jdbc.query(
            ...
            (rs, n) ->
                new V2SubscriptionData.CommandHistory(
  ...
  List<Long> activeSubscriptionIds() {
    return jdbc.queryForList(
        "SELECT id FROM subscriptions WHERE mvp2_managed=true AND status='ACTIVE' ORDER BY id",
        Long.class);
  }

  Optional<V2SubscriptionData.Subscription> lockActiveSubscription(long subscriptionId) {
    return one(
            "SELECT id,member_id,status,version,pet_id,delivery_cycle_weeks,current_snapshot_id"
                + " FROM subscriptions WHERE id=? AND mvp2_managed=true AND status='ACTIVE' FOR"
                + " UPDATE",
            subscriptionId)
        .map(this::subscription);
  }
  ...
  List<V2SubscriptionData.Schedule> futureSchedulesForUpdate(long subscriptionId, LocalDate today) {
    return jdbc.query(
  ...
  Optional<V2SubscriptionData.ProcessedSchedule> lastProcessedSchedule(long subscriptionId) {
    return one(
        ...
        .map(
            row ->
                new V2SubscriptionData.ProcessedSchedule(
  ...
  private Map<Long, List<V2SubscriptionData.Item>> groupedItems(
      String prefix, List<Long> ids, String key) {
    if (ids.isEmpty()) return Map.of();
    Map<Long, List<V2SubscriptionData.Item>> result = new HashMap<>();
    jdbc.query(
        ...
        (org.springframework.jdbc.core.RowCallbackHandler)
            rs ->
                result
                    ...
                    .add(new V2SubscriptionData.Item(rs.getLong("sku_id"), rs.getInt("quantity"))),
        ids.toArray());
    return result;
  }
