...
class V2SubscriptionJdbcStore {
	...
	V2SubscriptionData.Pet findOwnedPet(long memberId, long petId) {
		return one("SELECT id,name,pet_type,breed,weight_kg FROM pets WHERE id=? AND member_id=?", petId, memberId)
				.map(this::pet)
	...
	boolean planContainsSku(long versionId, long skuId) { return jdbc.queryForObject("SELECT COUNT(*) FROM plan_items WHERE plan_version_id=? AND sku_id=?", Integer.class, versionId, skuId) > 0; }
	boolean scheduleAddonConflicts(long scheduleId, long versionId) { return jdbc.queryForObject("SELECT COUNT(*) FROM subscription_schedule_addons addon JOIN plan_items item ON item.sku_id=addon.sku_id WHERE addon.schedule_id=? AND item.plan_version_id=?", Integer.class, scheduleId, versionId) > 0; }
	V2SubscriptionData.AddonSku findEligibleAddonSku(long skuId) {
		return one("SELECT sku.id sku_id,product.id product_id,product.name product_name,sku.name sku_name,sku.price,product.display_status,category.active category_active,brand.active brand_active,sku.status sku_status,COALESCE(inventory.available_quantity,0) available_quantity FROM skus sku JOIN products product ON product.id=sku.product_id JOIN categories category ON category.id=product.category_id JOIN brands brand ON brand.id=product.brand_id LEFT JOIN inventories inventory ON inventory.sku_id=sku.id WHERE sku.id=?", skuId)
				.map(row -> new V2SubscriptionData.AddonSku(skuId, longValue(row, "product_id"), (String) row.get("product_name"), (String) row.get("sku_name"), (java.math.BigDecimal) row.get("price"), "ACTIVE".equals(row.get("sku_status")) && "PUBLIC".equals(row.get("display_status")) && Boolean.TRUE.equals(row.get("category_active")) && Boolean.TRUE.equals(row.get("brand_active")) && intValue(row, "available_quantity") > 0))
				.orElseThrow(() -> new V2ApiException(404, "ADDON_NOT_FOUND", "Add-on을 찾을 수 없습니다."));
	}
	...
	void updatePet(long memberId, long petId, String name, boolean namePresent, String breed, boolean breedPresent, java.math.BigDecimal weightKg, boolean weightPresent) {
		List<String> columns = new ArrayList<>();
		List<Object> arguments = new ArrayList<>();
		if (namePresent) { columns.add("name=?"); arguments.add(name); }
		if (breedPresent) { columns.add("breed=?"); arguments.add(breed); }
		if (weightPresent) { columns.add("weight_kg=?"); arguments.add(weightKg); }
		arguments.add(petId);
		arguments.add(memberId);
		if (jdbc.update("UPDATE pets SET " + String.join(",", columns) + " WHERE id=? AND member_id=?", arguments.toArray()) != 1) throw new V2ApiException(404, "PET_NOT_FOUND", "Pet을 찾을 수 없습니다.");
	}
	V2SubscriptionData.Page<V2SubscriptionData.Pet> findPets(long memberId, int page, int size) { long total = jdbc.queryForObject("SELECT COUNT(*) FROM pets WHERE member_id=?", Long.class, memberId); List<V2SubscriptionData.Pet> items = jdbc.query("SELECT id,name,pet_type,breed,weight_kg FROM pets WHERE member_id=? ORDER BY id ASC LIMIT ? OFFSET ?", (rs,n) -> new V2SubscriptionData.Pet(rs.getLong("id"),rs.getString("name"),rs.getString("pet_type"),rs.getString("breed"),rs.getBigDecimal("weight_kg")), memberId,size,page*size); return new V2SubscriptionData.Page<>(page,size,total,items); }
	...
	V2SubscriptionData.Schedule lockNextScheduled(long subscriptionId) {
		return one("SELECT schedule.id,schedule.scheduled_date FROM subscription_schedules schedule LEFT JOIN subscription_orders existing_order ON existing_order.schedule_id=schedule.id WHERE schedule.subscription_id=? AND (schedule.status='SCHEDULED' OR (schedule.status='HELD' AND schedule.hold_reason='ORDER_STOCK_UNAVAILABLE')) AND existing_order.id IS NULL ORDER BY schedule.scheduled_date,schedule.id LIMIT 1 FOR UPDATE", subscriptionId)
	...
	void setScheduleStatus(long scheduleId, String status) { jdbc.update("UPDATE subscription_schedules SET status=?,hold_reason=CASE WHEN ?='HELD' THEN hold_reason ELSE NULL END WHERE id=?", status, status, scheduleId); }
	int scheduleAddonCount(long scheduleId) { return jdbc.queryForObject("SELECT COUNT(*) FROM subscription_schedule_addons WHERE schedule_id=?", Integer.class, scheduleId); }
	boolean hasScheduleAddon(long scheduleId, long skuId) { return jdbc.queryForObject("SELECT COUNT(*) FROM subscription_schedule_addons WHERE schedule_id=? AND sku_id=?", Integer.class, scheduleId, skuId) > 0; }
	void upsertScheduleAddon(long scheduleId, long skuId, int quantity, java.math.BigDecimal price) { jdbc.update("INSERT INTO subscription_schedule_addons(schedule_id,sku_id,quantity,unit_price_krw,created_at,updated_at) VALUES (?,?,?, ?,UTC_TIMESTAMP(6),UTC_TIMESTAMP(6)) ON DUPLICATE KEY UPDATE quantity=VALUES(quantity),unit_price_krw=VALUES(unit_price_krw),updated_at=UTC_TIMESTAMP(6)", scheduleId, skuId, quantity, price); }
	void deleteScheduleAddon(long scheduleId, long skuId) { if (jdbc.update("DELETE FROM subscription_schedule_addons WHERE schedule_id=? AND sku_id=?", scheduleId, skuId) != 1) throw new V2ApiException(404, "ADDON_NOT_FOUND", "Add-on을 찾을 수 없습니다."); }
	void deleteScheduleAddons(long subscriptionId) { jdbc.update("DELETE addon FROM subscription_schedule_addons addon JOIN subscription_schedules schedule ON schedule.id=addon.schedule_id WHERE schedule.subscription_id=? AND schedule.status IN ('SCHEDULED','HELD')", subscriptionId); }
	void moveScheduleAddons(long fromScheduleId, long toScheduleId) { jdbc.update("UPDATE subscription_schedule_addons SET schedule_id=? WHERE schedule_id=?", toScheduleId, fromScheduleId); }
	List<V2SubscriptionData.ScheduleAddon> findScheduleAddons(long scheduleId) { return jdbc.query("SELECT addon.schedule_id,addon.sku_id,sku.product_id,product.name product_name,sku.name sku_name,addon.quantity,addon.unit_price_krw FROM subscription_schedule_addons addon JOIN skus sku ON sku.id=addon.sku_id JOIN products product ON product.id=sku.product_id WHERE addon.schedule_id=? ORDER BY addon.sku_id", (rs,n)->new V2SubscriptionData.ScheduleAddon(rs.getLong(1),rs.getLong(2),rs.getLong(3),rs.getString(4),rs.getString(5),rs.getInt(6),rs.getBigDecimal(7)), scheduleId); }
	...
	void rescheduleHeld(long scheduleId, LocalDate date) { jdbc.update("UPDATE subscription_schedules SET scheduled_date=?,status='SCHEDULED',hold_reason=NULL WHERE id=?", date, scheduleId); }
	...
	void deleteDeliveryReminder(long scheduleId) { jdbc.update("DELETE FROM notifications WHERE type='SUBSCRIPTION_DELIVERY_REMINDER' AND reference_type='SCHEDULE' AND reference_id=?", scheduleId); }
	void deleteDeliveryReminders(long subscriptionId) { jdbc.update("DELETE notification FROM notifications notification JOIN subscription_schedules schedule ON schedule.id=notification.reference_id AND notification.reference_type='SCHEDULE' WHERE notification.type='SUBSCRIPTION_DELIVERY_REMINDER' AND schedule.subscription_id=?", subscriptionId); }

	private V2SubscriptionData.Pet pet(Map<String,Object> row) { return new V2SubscriptionData.Pet(longValue(row,"id"), (String) row.get("name"), (String) row.get("pet_type"), (String) row.get("breed"), (java.math.BigDecimal) row.get("weight_kg")); }
	...
	Map<Long,V2SubscriptionData.Pet> findOwnedPets(long memberId,List<Long> ids) { if(ids.isEmpty())return Map.of(); Map<Long,V2SubscriptionData.Pet> result=new HashMap<>(); jdbc.query("SELECT id,name,pet_type,breed,weight_kg FROM pets WHERE member_id=? AND id IN "+placeholders(ids.size()),rs->{result.put(rs.getLong("id"),new V2SubscriptionData.Pet(rs.getLong("id"),rs.getString("name"),rs.getString("pet_type"),rs.getString("breed"),rs.getBigDecimal("weight_kg")));},withLeading(memberId,ids)); return result; }
