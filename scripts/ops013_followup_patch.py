...
def patch_backup_script() -> None:
    ...
    replace_once(
        ...
        '  local container="$1"\n'
        '  local manifest="$2"\n'
        '  local schema_file="$WORK_ROOT/source-schema"',
        'write_restore_manifest() {\n'
        '  local manifest="$1"\n'
        '  local schema_file="$WORK_DIR/snapshot-schema"',
    )
    # The prior replacement intentionally expects WORK_DIR paths below; normalize the remaining names and queries.
    replace_once(path, '  local flyway_file="$WORK_DIR/source-flyway"', '  local flyway_file="$WORK_DIR/snapshot-flyway"')
    replace_once(path, '  local count_file="$WORK_DIR/source-count"', '  local count_file="$WORK_DIR/snapshot-count"')
    replace_once(
        path,
...
def patch_tests() -> None:
    ...
    replace_once(path, 'REGION="ap-northeast-2"\nPREFIX=', 'REGION="ap-northeast-2"\nEXPECTED_BUCKET_OWNER="123456789012"\nPREFIX=')
    replace_once(
        path,
...
def patch_runbook() -> None:
    ...
    # Add owner guard to manual contract reads.
    target = ROOT / path
    text = target.read_text(encoding="utf-8")
    text = text.replace(
    ...
    # There are two run commands; replace the second occurrence too after the first mutation.
    target = ROOT / path
    text = target.read_text(encoding="utf-8")
    old = 'sudo --preserve-env=PAWCYCLE_BACKUP_BUCKET,PAWCYCLE_BACKUP_REGION,PAWCYCLE_BACKUP_PREFIX \\\n'
    if old in text:
        text = text.replace(old, 'sudo --preserve-env=PAWCYCLE_BACKUP_BUCKET,PAWCYCLE_BACKUP_REGION,PAWCYCLE_BACKUP_PREFIX,PAWCYCLE_BACKUP_EXPECTED_BUCKET_OWNER \\\n', 1)
        target.write_text(text, encoding="utf-8")
    replace_once(
        path,
...
def patch_report_and_handoff() -> None:
...
def main() -> None:
    ...
    patch_validator()
    patch_runbook()
    patch_report_and_handoff()
    print("OPS-013 follow-up patch applied")
