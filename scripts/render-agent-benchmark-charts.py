    ...
    for path in paths:
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            if row.get("record_type") == "result" and row.get("counts_toward_independent_repetition"):
                grouped[row["comparison_arm"]][row["scenario"]].append(row)
    return grouped
...
def render(grouped: dict[str, dict[str, list[dict]]], output: Path, title: str) -> None:
    ...
    sections = [(135, "Duration median (seconds)"), (375, "Tool calls median"), (615, "Accuracy pass count")]
    ...
    for index, scenario in enumerate(SCENARIOS):
        duration_y = 175 + index * 48
        tools_y = 415 + index * 48
        accuracy_y = 655 + index * 38
        parts.append(text(55, duration_y + 16, scenario, 14, "bold"))
        parts.append(text(55, tools_y + 16, scenario, 14, "bold"))
        parts.append(text(55, accuracy_y + 16, scenario, 14, "bold"))
        for arm_index, arm in enumerate(arms):
            ...
            if value is None:
                ...
                parts.append(f'<rect x="115" y="{duration_y + offset}" width="{bar}" height="12" fill="{color}"/>')
            ...
            parts.append(f'<rect x="115" y="{tools_y + offset}" width="{tool_bar}" height="12" fill="{color}"/>')
            ...
            parts.append(f'<rect x="115" y="{accuracy_y + offset}" width="{accuracy_bar}" height="12" fill="{color}"/>')
