...
AI가 코드를 생성하는 것만으로 작업이 완료된 것으로 보지 않는다. 입력 문서, 작업 범위, 검증, 적용 방법, 보고서, 인수인계, Git 기록, 다음 작업이 함께 남아야 한다.
...
- 필수 검증이 통과하고 작업 보고서와 인수인계가 작성되면 Codex는 commit과 push를 수행한다.
